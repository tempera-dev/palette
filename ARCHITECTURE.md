...
| **LLM gateway / proxy** (cache, failover, 100+ models) | **Braintrust AI Proxy** (GA) | **Patchbay** `beater-gateway` (§20.10 #7.3) | every proxied call is natively traced with **zero SDK** and online-scorable (§20.6); **opt-in &amp; complementary to OTLP, never the sole instrumentation** (a proxy alone misses non-LLM agent steps — the §26.0 Helicone lesson) and it carries its own hot-path availability SLO (§16) |
