...
Palette is a Rust-first, open-source agent observability, replay, and eval platform.
...
1. Ship one Rust binary first.
   Crates are modular, but the MVP runs as one `paletted` process. Later service
...
8. No cloud dependency in OSS.
   The open-source edition must run without calling Palette Cloud. Self-host
...
13. Zero-code OTLP bootstrap is the default onboarding.
    ...
    Palette with **no Palette SDK and no code edits** (§15, §20.8). The native SDK is
...
```text
docker compose up
  paletted       # one Rust process: API, OTLP, jobs, eval, replay   [built]
...
As of `origin/main`, `paletted` boots with SQLite stores, a `SqliteDurableBus`,
...
Palette is not one artifact — it is a server, a documentation site, seven
...
| **CLI** (`palettectl`) | resolves `/v1` operations from the spec at runtime (`palette api`), plus local fixtures/smoke | the server image, and a standalone binary | tracks server/spec | `cargo run -p palettectl` or the released binary; used in CI smoke + local dev | [built] |
...
The **project** is **Palette**. Its **crates are collectively the "beat-boxes"**, and
...
underlying Cargo crate (`palette-*`) is given in parentheses on first use and is the
...
`palette-*` paths today; the **physical directory rename to the beat names is a
...
The top-level binary (`paletted`), foundational primitives, and the canonical event
...
code never grew (`palette-normalize`, `palette-store-ch`, `palette-sdk`,
`palette-telemetry`), the note explains where that responsibility actually lives.

```text
palette/
  Cargo.toml
  crates/
    palette-core/          # Downbeat — IDs, entity types, typed money, clocks, tenant scope
    palette-schema/        # Beatmap [CHANGED] canonical event/run/span/eval schemas, mappings,
                          #   rollups, conventions PLUS `sampling_weight` on the keep path
                          #   and WEIGHTED roll-ups/aggregates (§9, §13); DatasetCase `split`
    palette-otlp/          # Upbeat (with palette-ingest) — tonic/prost OTLP HTTP/gRPC
                          #   receive/export AND the OTLP/OpenInference/GenAI -> canonical
                          #   normalizer (there is no separate palette-normalize crate)
    palette-temporal/      # Syncopation — Temporal workflow-history -> canonical span normalization
    palette-ingest/        # Upbeat [CHANGED] auth, quota, raw append, normalization, tail-sampling
                          #   PLUS recording `sampling_weight = 1/keep_probability` on every
                          #   kept span so downstream aggregates can be unbiased (§9)
    palette-store/         # Groove [CHANGED] TraceStore, MetadataStore, ArtifactStore, QuotaLimiter
    ...
    palette-store-conformance/ # Soundcheck — shared trait-conformance test suite run against every backend
    palette-store-memory/  # Sample — in-memory TraceStore/MetadataStore/QuotaLimiter for tests/dev
    palette-store-sql/     # Vinyl — SQLite stores (runtime default) PLUS PgTraceStore and
                          #   ClickHouseTraceStore (implemented, NOT yet runtime-wired);
                          #   ClickHouse lives here, not in a palette-store-ch crate
    palette-store-obj/     # Crate — FsArtifactStore (filesystem) for artifacts/raw envelopes
    palette-bus/           # Drumbeat — SqliteDurableBus (the durable bus today); NATS/Kafka are [planned]
    palette-eval/          # Backbeat (with palette-judge/-stats) — evaluator catalog, scoring
                          #   contracts, paired comparison, aggregation
                          #   [CHANGED] hardcoded-z `compare_paired_scores` is DELETED;
                          #   it now delegates to Backbeat/palette-stats (§10.3, §20.5)
    palette-calibration/   # Tuning [CHANGED] judge-vs-human agreement + Cohen's-kappa reports
    ...
    palette-usage/         # Tempo — usage ledger, billing meters, spend summaries
    palette-audit/         # Logbook — privileged access audit events and readback
    palette-sandbox/       # Soundproof — Wasmtime/WASI Component Model evaluator runtime
    palette-secrets/       # Stash — opaque provider-secret refs, BYOK metadata, revocation
    palette-security/      # Vault — crypto primitives: Argon2 keys, ChaCha20 envelope, signed webhooks
    palette-judge/         # Backbeat (with palette-eval/-stats) — LLM/embedding judge broker, BYOK, calibration
    palette-replay/        # Rewind [CHANGED] cassettes + deterministic replay PLUS
    ...
    palette-datasets/      # Encore [CHANGED] datasets, versions, examples, trace promotion PLUS a
                          #   seeded-hash Train/Dev/Test `split` on DatasetCase + min-sample
                          #   gate + contamination guard (§5.3, §6.4); bulk promote-from-query
    palette-experiments/   # Palette Experiments (with the §21 RSI tools) — candidate-vs-baseline
                          #   comparisons and statistics
    palette-gates/         # Cue [CHANGED] CI/CD gates and policy evaluation; the deploy-gate
                          #   number now comes from Backbeat/palette-stats (real p-value + power +
                          #   FWER/FDR) and a gate accepts only on the frozen Test split (§10.3)
    palette-human/         # Setlist — review queues, annotations, human labels
    palette-search/        # Crate Dig — Tantivy full-text index over spans
    palette-archive/       # Cold Storage — Parquet cold-tier archive (Arrow/DataFusion read path)
    palette-alerts/        # Offbeat — alert evaluation over trace/score signals
    palette-auth/          # Backstage — API keys, JWT/session, RBAC types, audit scopes
    palette-accounts/      # Guestlist — users, password auth, browser sessions, org membership
    palette-oauth/         # Wristband — OAuth 2.1 core: clients, PKCE codes, access/refresh tokens
    palette-oauth-server/  # Door — OAuth 2.1 HTTP surface (wired into Palette/paletted)
    palette-mcp/           # Palette MCP [CHANGED] MCP server exposing every /v1 operation as a tool,
    ...
    palette-browser/       # Liveset — browser-agent observability contract (shared foundation)
    palette-browser-cdp/         # Liveset: DJ Deck — Chrome DevTools Protocol backend
    palette-browser-playwright/  # Liveset: Turntable — Playwright driver backend
    palette-browser-webdriver/   # Liveset: Mixer — WebDriver/fantoccini backend
    palette-browser-capture/     # Liveset: Bootleg — console + network + DOM capture per browser step
    palette-browser-harness/     # Liveset: Roadie — browser-agent run harness
    palette-api/           # Mixing Board [CHANGED] axum routers, OpenAPI, SSE/read APIs PLUS the
    ...
    palette-stats/         # Backbeat (with palette-eval/-judge) [planned, NEW] over `statrs`: real
    ...
    palette-scorers/       # Riff [planned] custom-scorer registry over the Soundproof WASI sandbox (§20.5)
    palette-online/        # Backspin [planned] online-eval scoring worker (§20.6)
    palette-prompts/       # Mixdown [planned] prompt registry/versioning/playground (§20.6)
    palette-rbac/          # All-Access [planned] role/permission resolution inside authorize() (§20.7)
    palette-identity/      # Passport [planned] OIDC/SAML/SCIM (§20.7)
    palette-billing/       # Bandwidth [planned] plans/subscriptions/Stripe metered sync (§20.7)
    palette-bench/         # Tech Rider [planned] criterion benches + load fixtures (§20.2)
    palette-guardrails/    # Bouncer [planned] runtime guardrail/firewall: pre/post input+output
                          #   checks (injection/PII/toxicity/topic), block|redact|allow|flag (§20.10 7.1)
    palette-gateway/       # Patchbay [planned] OpenAI-compatible LLM proxy for app traffic:
                          #   caching, BYOK failover/load-balance, native tracing + online scoring (§20.10 7.3)
    palette-insights/      # Medley [planned] failure discovery: cluster failing traces -> named
                          #   FailureIssues w/ counterfactual root-cause + one-click promote (§20.10 7.4)
  bins/
    paletted/              # Palette — default all-in-one binary (also holds Heartbeat: metrics.rs / Prometheus facade)
    palettectl/            # Stomp Box — CLI: init, ingest test, eval run, gate, export
    palette-worker/        # [planned] later thin bin over worker modules
    palette-ingestd/       # [planned] later thin bin over ingest modules
  sdks/
    rust/                 # native Rust SDK + tracing layers (a standalone package,
                          #   excluded from the workspace; there is no palette-sdk crate)
...
Metrics, health, and SLO instrumentation are NOT a separate `palette-telemetry`
crate; they live in `bins/paletted/src/metrics.rs` (the Prometheus facade) and
...
SDK packages stay out of the core build/test graph; it is not a `palette-sdk`
workspace crate.

Browser-observability family note: `palette-browser*` is a six-crate family that
turns browser-driving agents into first-class observed agents. `palette-browser`
...
| ClickHouse | driven over its HTTP interface via `reqwest` (no native driver), in `palette-store-sql` — there is no separate `clickhouse`-crate dependency |
...
| statistics | `statrs` (distributions/CDFs for p-values, Wilson, power) in the new `palette-stats`; bootstrap/permutation are hand-rolled over a seeded RNG |
...
pub struct CanonicalSpan {
    ...
    // probabilistic sampling. WEIGHTED roll-ups/aggregates (§13, palette-store)
...
Leaf types: `SessionId`/`ThreadId`/`UserId` are `palette-core` ID newtypes
...
**Pre-1.0 schema-evolution stance.** Palette is not deployed anywhere yet, so the
canonical and API schemas evolve *freely*: a change bumps `schema_version`
(`CANONICAL_SCHEMA_VERSION` / `RAW_SCHEMA_VERSION` in `palette-schema`) and
...
An agent is not a scalar. Palette measures it along many **typed dimensions**, each
...
from wishful thinking. For each, how Palette checks or relaxes it:
...
- **Judge-calibration validity.** Judge-derived dimensions assume the §10.1.1
  ...
  periodic judge-vs-human agreement (`palette-calibration`, Cohen's kappa);
...
`EVALUATOR`, `PROMPT`. The `palette-otlp` table maps nine of them into §5.2 today
...
normalizer. For those, Palette exposes a **declarative MAPPING importer** on the
...
- `SqliteTraceStore` in `palette-store-sql`: **[built, runtime default]** — the
  only `TraceStore` `paletted` actually constructs today.
- `InMemoryTraceStore` in `palette-store-memory`: **[built]** — used by tests and
  for ephemeral dev.
- `PgTraceStore` (Postgres, `tokio-postgres`) and `ClickHouseTraceStore`
  (ClickHouse over HTTP via `reqwest`), both in `palette-store-sql`:
  **[built but NOT runtime-wired]** — the types and trait impls exist and pass
  the `palette-store-conformance` suite, but `paletted` has no backend selector,
  so neither is reachable from the running service yet (§20.2 #0.1).
- `ParquetTraceArchive` in `palette-archive`: **[built, local-fs only]** — cold
...
`palette-store-conformance`, so a newly wired backend must satisfy the same
...
`palette_store::query_runs_by_materializing_spans` as a dev/local fallback. That
...
| Cold traces | Parquet + Arrow/DataFusion, local-fs (`palette-archive`) | → Parquet on object store, scheduled demotion | long retention and export |
| Full text | Tantivy (`palette-search`) | → Tantivy or managed equivalent | prompt/output/error search |
...
the `paletted` background worker consume only that lane, write through
...
- **At-least-once delivery reconciled by idempotency keys (the dedup algorithm).**
  ...
  built by `make_idempotency_key` (`palette-schema`) as
...
- **Inverse-probability sampling weights on the keep path (honesty invariant §1
  ...
  downstream aggregates (§13, `palette-store`) are **weighted by default** and an
  ...
  `Var(T̂) = Σ_i (1−pᵢ)/pᵢ² · yᵢ²`, which `palette-stats` reports as the weighted
...
lies. Palette's judge broker therefore implements a debiasing protocol as a
...
Because the magnitude and even the *sign* are model-specific, Palette never hard-
...
In Palette this calibration **lives in the judge broker** (alongside the existing
`palette-calibration` agreement/kappa reporting), is fit from a **human reference-
...
operating points, to be confirmed by Palette's own measurement (below): a **minimum
...
as load-bearing in a production gate** — they should be measured against Palette's own
...
eval folklore relied on were measured to fail and Palette must not assume either:

- *"Pairwise comparison is strictly better than pointwise scoring"* — **refuted**
  (held in 0 of 3 tested settings). Palette treats pairwise vs pointwise as an
...
Palette's own calibration reports (§10.1.1, §10.3) are the source of truth for any
...
`compare_paired_scores` in `palette-eval` computes a paired delta, a sample
...
call into `palette-stats` that selects the correct test (below). This restores the
...
assumption-aware algorithm spec. It lives in a new **`palette-stats`** crate
(built on `statrs`; §20.5 #3.4) that `palette-experiments`, the gate runner
...
*fixed-horizon* core of `palette-stats` — Wilson + bootstrap CIs, clustered SEs,
...
inflate false wins. `palette-stats` computes **clustered standard errors** with
...
- **Bounded / continuous metrics** (judge scores in [0,1], latency, cost):
  ...
  **percentile** interval; for skewed metrics (cost, latency) `palette-stats` uses
...
test is chosen by the data, and `palette-stats` records which assumption justified
the choice:
...
or slices, raw per-comparison alpha inflates false wins. `palette-stats` applies
...
can return *pass*, `palette-stats` checks that the comparison was adequately
...
why this is tractable for Palette's metrics. This ties directly to §13 alert
...
**7. The exact formulas `palette-stats` implements.** So the layer is buildable
...
- **Wilson score interval** for a proportion `p̂ = k/n` at level `z = Φ^{-1}(1−α/2)`:
  ...
  or `p̂` near 0/1 — exactly Palette's regime).
...
- **Anytime-valid: mSPRT / confidence sequence (the e-process).** For a stream of
  bounded scores testing `H₀: μ = μ₀`, `palette-stats` maintains a non-negative
...
`palette-stats` and the §10.1.1 calibration):
...
- judge calibration artifact: judge-vs-human agreement, confusion counts, Cohen's
  ...
  (`palette-calibration` already persists kappa/agreement).
...
- **Tool-call correctness is execution-based, never AST/syntactic.** A scorer that
  ...
  *effect*; Palette scores the call by replaying/executing it (deterministic lane,
...
metrics; Palette adds them to `EVALUATOR_CATALOG` with the *same* assumption/CI
...
- **G-Eval-style task-agnostic judge**: CoT + auto-generated evaluation steps from a
  ...
  held-out labels). This is what separates Palette's auto-rubric from a vibes-rubric.
...
- **Agent / score calibration (this section)** asks whether a *probabilistic
  ...
  `palette-calibration` and ties directly to agent dimension #7 (self-calibration,
  §6.3).
...
- **Expected Calibration Error (ECE)** — the binned gap between confidence and
  ...
  **M = 10 equal-width bins** over [0,1] (`B_m = (\,(m-1)/10,\ m/10\,]`); with
  ...
  bins, and `palette-stats` reports which was used so the number is comparable
  across runs.
- **Reliability curve** — the per-bin confidence-vs-accuracy plot the dashboard
  renders, the visual form of ECE.
- **Cohen's kappa becomes a secondary signal.** The existing `palette-calibration`
...
**Persisted recalibration map.** From the reliability data `palette-calibration`
...
`attribute_failure` first-error stub has been retired. Current `palette-replay`
...
midpoint, recurse left if it flips, right if it does not. `palette-stats` is not
...
- `Simulator` *(Understudy, Phase 7 §20.10 #7.9)*: a user-simulator (persona + goal)
  ...
  simulation benchmark (Galileo-style) expressible on Palette's own substrate — built
...
- **failure discovery** (Medley, §20.10 #7.4): unsupervised clustering of failing
  ...
  Behavior Discovery / Patronus Percival) over Palette's already-rigorous counterfactual
...
user must reach fast is the moment Palette shows them a *failing* agent behavior with
...
v0 `palettectl smoke` target) but is no longer the headline DX number.

The **default** onboarding path is **zero-code OTLP bootstrap** (§1 #13, §20.8):
point a standards-based OTLP exporter at Palette via environment variables, with no
Palette SDK and no code edits. The native SDK is an accelerator offered second, not
the adoption gate.

Required onboarding paths (zero-code OTLP first):

- **zero-code env-var OTLP bootstrap (DEFAULT)** — any OpenInference/OpenLLMetry/
  OTel app exports to Palette by setting `PALETTE_*`/OTLP env vars; no code, no SDK
  (§20.8 #6.2)
...
- `palettectl smoke` to emit a known trace and verify ingestion
...
- **Quickstart tutorial** — zero-code OTLP bootstrap (§15) to first scored failure,
  step by step.
- **"Palette in Claude Code & Codex" setup guide** — the §21.5b OAuth connect flow
...
Palette dogfoods itself. Hosted cannot launch without dashboards and alerts for:
...
- `palettectl smoke` proves time-to-first-trace.
...
candidate change at the fork point, re-score) and §21 Palette Experiments `simulate` (which
...
This section is the concrete, technical execution plan to take Palette from its
...
| Security, multi-tenancy & hosted ops | 38% | OAuth 2.1 + accounts/sessions now exist (`palette-oauth`/`-oauth-server`/`-accounts`, wired into `paletted`) but enforced RBAC, SSO/SAML/SCIM are absent; RBAC data model never consulted by `authorize()`; audit covers one action; no deletion/retention/billing/backups |
...
(PKCE, accounts, sessions) wired into `paletted`; quota limiter; single-source
...
| --- | --- | --- | --- | --- | --- |
| 0.1 | Columnar store wired into `paletted` | `ClickHouseTraceStore`/`PgTraceStore` implemented but dead code; runtime hardcodes `SqliteTraceStore` | Add `TraceStoreBackend` env/CLI arg (`sqlite\|postgres\|clickhouse`) + `build_trace_store(cfg) -> Arc<dyn TraceStore>` in `palette-store-sql`; thread through `ApiState` and the ingest/query bins; non-ignored compose integration test booting `paletted` on ClickHouse | L | docker |
...
| 0.3 | Measured query p95 SLOs | no `benches/`, no criterion, no load test, no SLO evidence | New `palette-bench` crate: criterion benches for `write_batch` throughput + `query_*` latency on seeded 1M/10M/100M-span fixtures; `xtask loadgen` emitting OTLP at sustained RPS → p50/p95/p99; codify §16 SLOs + CI regression gate | XL | evidence |
| 0.4 | Runtime retention/TTL | TTL exists only as ClickHouse DDL that never runs | `RetentionPolicy{hot_days,archive_days}` in `palette-core`/`palette-schema`; retention sweeper (extend `palette-archive`) on an interval in `paletted` demoting-then-deleting expired hot rows; `GET/PUT /v1/projects/:id/retention` **[contract]** | L | design |
| 0.5 | Automated cold-tier archival | `ParquetTraceArchive` exists, local-fs only | Write partitioned append-only Parquet (`tenant/project/yyyymm/uuid`) to object store via `palette-store-obj`; scheduled demotion job; DataFusion read path over cold files | L | design |
| 0.6 | Backend-agnostic migrations + re-normalization | versioned framework exists for SQLite only | Generalize the `SqliteMigration` version/checksum `Migrator` to ClickHouse + Postgres (`_palette_schema_migrations` on each); `xtask renormalize` reprojecting historical `RawEnvelope`s to a new canonical version | L | none |

Acceptance: `paletted --trace-store clickhouse` boots and serves traces; a 10M-span
...
| 1.3 | Multimodal (image/audio/file) I/O | stringified scalars only | `MediaArtifact{mime_type,uri-or-inline,role}` on canonical messages; parse OpenInference content-part `image_url`/`audio`; store large media via `palette-store-obj` with size caps + redaction class **[contract]** | L | design |
| 1.4 | Full-text over artifact-backed I/O | tantivy indexes only inline attrs, not artifact bodies | In `palette-search`, have the ingest processor resolve `input_ref`/`output_ref` via `ArtifactStore` and index their text into dedicated `input_body`/`output_body`/`error` fields; per-tenant shards | L | evidence |
...
| 1.6 | Sampling weights + weighted aggregates (**honesty fix**, §9, §1 #9) | tail-sampling keeps/drops but records no weight; roll-ups average kept spans (biased) | Add `sampling_weight: f64` to `CanonicalSpan` (`palette-schema`); stamp `1/keep_probability` on the keep path in `palette-ingest`; make `palette-store` roll-up/aggregate queries weighted (Horvitz-Thompson); label any unweighted view biased **[contract]** | M | contract |
| 1.7 | DatasetCase Train/Dev/Test split + contamination guard (§5.4, §6.4) | `DatasetCase` has no split; no held-out discipline | Add `split: DatasetSplit` (seeded hash off `dataset_version_seed ++ case_id`) to `DatasetCase` (`palette-datasets`/`palette-schema`); min-sample gate; near-dup contamination detection train↔test; gates/RSI read Test-only **[contract]** | M | contract |
...
artifacts; a stock OTel JSON exporter ingests with no Palette SDK.
...
| 2.5 | Human annotation queues + inline scoring UI | full `palette-human` backend, no UI | `web/dashboard/app/review` (queue + task inbox) + inline `AnnotationPanel` on span detail posting `submitReviewAnnotation`; keyboard labeling | L | none |
...
| 3.3 | Custom scorer registry | WASI sandbox runs components, no upload/registry | `palette-scorers` (or extend `palette-eval`): `ScorerStore` (upload component bytes → `Sha256Hash`, version, list/get) on `palette-store-obj`+sqlite; `/v1/scorers` CRUD **[contract]**; resolve by `wasm_hash` into the sandbox; add memory/epoch limits to `SandboxConfig` | XL | contract |
| 3.4 | Real statistics module (**correctness fix**, §10.3) | single paired normal-approx, **hardcoded z (1.96/2.576), Bonferroni-only, no p-value → nominal alpha ≠ actual alpha** | New `palette-stats` on `statrs`: paired-t / bootstrap-percentile / Wilson CIs; test selection `{PairedT, McNemarExact, WilcoxonSignedRank, Bootstrap}` with real `p_value`; Holm-Bonferroni + Benjamini-Hochberg; `power.rs` (`required_sample_size`, `achieved_power`); **DELETE `compare_paired_scores`'s hardcoded-z path + `StatisticalTest::PairedNormalApproximation`**, route `palette-eval`/`palette-experiments`/`palette-gates` through `palette-stats`. mSPRT/confidence-sequences are the REQUIRED online follow-on (Phase 4, §10.3 #6) | L | none |
...
| 3.6 | CI integration | none | `sdks/python/palette/pytest_plugin.py` (`@palette.eval` marker running cases through the API, asserting via `GatePolicy`); TS vitest reporter; `palette eval` gating CLI subcommand | L | contract |
| 3.7 | Agent/score calibration (proper scoring, §10.5) | `palette-calibration` reports kappa/agreement only; the continuous confidence signal is stored then discarded | Persist the probability signal; add Brier + ECE + reliability curve and a persisted, versioned isotonic/Platt recalibration map to `palette-calibration` (kappa demoted to secondary); pin the map into `EvalResult` repro metadata; reliability-curve UI; feeds agent dim #7 (§6.3) on the Test split **[contract]** | M | contract |
...
custom uploaded scorer runs sandboxed; `pytest`/`palette eval` fails CI on
regression.
...
| # | Requirement | Now | Target / concrete task | Effort | Blocker |
| --- | --- | --- | --- | --- | --- |
| 4.1 | Online evals that score | sampling decision only, never scored | `palette-online` worker (or `palette-temporal` workflow) consuming tail-sampled traces, running configured deterministic+judge evaluators, persisting online-tagged `EvalResult`s (weighted by `sampling_weight`, §9); `GET /v1/online/.../scores` timeseries **[contract]** | XL | design |
...
| 4.3 | Actual webhook delivery | `WebhookDelivery` computed, never sent | delivery worker POSTing with retry/backoff + `palette-security` HMAC signature; persist attempts/status; delivery-history endpoint | M | evidence |
...
| 4.5 | Baseline/anomaly/drift alerting (anytime-valid) | static threshold only | `AlertCondition{AbsoluteThreshold, BaselineDeviation, Drift}` with rolling EWMA/z-score/percentile baseline over recent project scores, decided against a **`palette-stats` confidence sequence (mSPRT)** not a fixed-N test — the REQUIRED online follow-on of §10.3 #6 (peeking a fixed-horizon test inflates false alerts 5–10×) | L | design |
...
| 5.2 | Human identity + enforced RBAC | `RoleBinding` data model never consulted by `authorize()` | `palette-rbac` (or extend `palette-auth`): `Role`/`Permission` + `resolve_permissions(principal, scope)` called inside `authorize()` on every mutating route; users + memberships; member/role-grant endpoints **[contract]**; conformance tests | XL | contract |
| 5.3 | SSO / SAML / SCIM / OIDC | none | `palette-identity`: OIDC (auth-code+PKCE) + SAML2 SP + SCIM 2.0 `/Users`/`/Groups`; session/JWT issuance; per-org IdP config; enforced-SSO toggle; JIT provisioning | XL | design |
...
| 5.7 | Comprehensive tamper-evident audit | covers `PiiUnmask` plus API-key/provider-secret create/revoke; role/config/export/login/auth-failure still missing | expand `AuditAction` (key/secret/role/config/export/login/auth-failure); emit from `palette-auth`/`palette-secrets`/RBAC/login; hash-chained tamper-evident column; `GET /v1/audit-events` **[contract]** | L | contract |
| 5.8 | Billing / usage ledger | idempotent ledger exists; no plans/invoicing | meters for ingest/storage/eval/judge; per-org rollups; `palette-billing` (plan/subscription + Stripe metered sync) linked to `QuotaLimiter` | L | contract |
| 5.9 | Backups + restore drills | none | hosted on Postgres+object store with PITR; `palettectl backup`/`restore` for self-host; CI restore-drill job with documented RPO/RTO | L | evidence |
| 5.10 | SLO dashboards + dogfooding | Prometheus facade exists | Grafana dashboard JSON + Prometheus alert rules under `ops/`; self-trace OTLP exporter so `paletted` traces into a Palette project; load test producing the §16 numbers | M | evidence |
...
| # | Requirement | Now | Target / concrete task | Effort | Blocker |
| --- | --- | --- | --- | --- | --- |
| 6.1 | Auto-instrumentation (OpenAI/Anthropic) | one-line `wrap_*` wrappers only | `palette.auto.instrument(providers=[...])` monkeypatching `openai`/`anthropic` (incl streaming + tool calls) in py + ts | L | none |
| 6.2 | Zero-code env-var bootstrap (**DEFAULT onboarding**, §1 #13, §15) | `palettectl ingest test` verifies live OTLP ingest and prints the exporter env block; env-var-only auto-instrumented app path is still planned | `opentelemetry-distro`/configurator (py) + TS `--require` preload reading `PALETTE_*` env, setting OTLP exporter+headers, enabling installed auto-instrumentors; promoted to the documented first path | M | none |
...
`palettectl quickstart` demonstrates **time to first scored failure** under the §15
...
- `palette-bench` — criterion benches + load-test fixtures (Phase 0).
- `palette-stats` — CIs, test selection, p-values, power, FWER/FDR (Phase 3); the
...
- `palette-scorers` — custom-scorer registry over the WASI sandbox (Phase 3).
- `palette-online` — online-eval scoring worker (Phase 4).
- `palette-prompts` — prompt registry/versioning/playground (Phase 4).
- `palette-rbac` — role/permission resolution wired into `authorize()` (Phase 5).
- `palette-identity` — OIDC/SAML/SCIM (Phase 5).
- `palette-billing` — plans/subscriptions/Stripe metered sync (Phase 5).
- `palette-guardrails` — Bouncer, runtime guardrail/firewall (Phase 7, §20.10 #7.1).
- `palette-gateway` — Patchbay, OpenAI-compatible LLM proxy + caching + failover (Phase 7, §20.10 #7.3).
- `palette-insights` — Medley, failure clustering/discovery + named issues (Phase 7, §20.10 #7.4).

(Phase 7 also folds work into existing crates rather than new ones: distilled house
judges into `palette-judge` (#7.2); optimizer strategies into `palette-experiments`
(#7.6); AutoRubric/G-Eval into `palette-mcp`/`palette-eval` (#7.7); conversation/
trajectory scorers into `palette-eval` (#7.8); the user-simulator into the harness
(#7.9); embedding-drift into `palette-alerts` (#7.5). The GQL query language (#7.11) is
**conditional** — deferred per §26.3 unless a measured expressiveness limit forces it,
then into `palette-api`/`palette-store`.)

**The beat-boxes rename (pre-1.0 follow-up, cross-cutting).** §4 establishes the
beat-themed names now; the **physical directory rename** of each `palette-*` crate to
...
then, reach for a component by its `palette-*` path and reason about it by its beat
name.
...
Langfuse, Arize, Comet Opik, Patronus, and Galileo ship and Palette does not yet —
**without** weakening any §1 honesty/statistics invariant. Every item is built *on top
of* Palette's existing rigor (§10.3 statistics, §10.1.1/§10.5 calibration, §5.4/§6.4
...
| 7.12 | **BYOC data-plane / control-plane split + Terraform** | §3.2 hosted cells, no formal BYOC topology | Formalize: customer-hosted **data plane** (ingest + stores + workers + judge/gateway, all payloads) vs Palette-hosted **control plane** (dashboard/auth/metadata only, no payloads); Terraform modules (AWS/GCP/Azure); documented residency boundary. Extends §20.7 #5.4/#5.6 | Braintrust hybrid, LangSmith BYOC | XL | design |
...
green, "anything they can do, Palette can do — and gates it correctly" becomes an
*auditable* claim (each row → a §22 test), not a slogan (§26.4). Until then it is a
plan, marked `[planned]` throughout.

## 21. Planned: Recursive Self-Improvement (folded into `palette-mcp`)

This is the recursive-self-improvement (RSI) loop layered on the Palette
...
autonomy guardrails. It reuses Palette for traces, evals, judges, datasets, replay,
and statistics; it does not reinvent them.

**Architecture decision: the improvement loop is FOLDED INTO `palette-mcp`, not a
...
foundation (`palette-credits`, `palette-mcp-improve`) already exists on the
`feat/mcp-improve-foundation` branch, but the architecture now prefers folding
improve into `palette-mcp` over shipping a separate improve server, and **defers**
...
HTTP endpoints already exist on `main` (`palette-oauth`/`palette-oauth-server` wired
into `paletted`: `/.well-known/oauth-authorization-server`, `/oauth/authorize`,
...
surface. `paletted mcp --stdio` now exposes the same tool set locally over stdin /
...
- **Standards + reuse at the edge.** Scoring is Palette's existing LLM-judge +
  deterministic WASI evals; statistics are `palette-stats`; memory/tools are
...
### 21.1 The improvement tools (added to `palette-mcp`)

The RSI tool-belt is a set of tools on the existing `palette-mcp` server (not a
...
- `simulate` — run N candidate iterations through Palette's harness (§12) on the
  **Train** split (and Dev for tuning), scoring with the frozen evaluator
  ...
  verifier gain vs judge gain, position-bias-cancelled) **with a `palette-stats`
...
Palette's statistics (FWER/FDR, power, held-out Test) are exactly what stop an
...
per-operation tools (§20), `palette-mcp` exposes a small set of **outcome-shaped
...
split) and `gate_candidate` (run experiment → `palette-stats` CI on Test → return
...
**typed decomposition** with a `palette-stats` interval on each component:
...
`palette-stats` CI, and acceptance (§21.3) reads these on the **Test** split only.
...
goal + params + few examples
  ...
  -> collect traces/evals (Palette) + classify failures
  ...
  -> evaluate on the untouched TEST split -> palette-stats CI + power check (§10.3)
...
untouched Test split (§5.4, §6.4), and a *pass* requires a real `palette-stats`
...
§21.3, on the same `palette-stats`/Backbeat statistics and `palette-eval` scorers,
...
> PALETTE's own data before any threshold is trusted. The scaling-law coefficients
...
> detector (4c) are **[deferred]** — not enabled until PALETTE-specific calibration
...
- **KL-distance scaling law (closed-form peak) [deferred — requires calibration data].** Over-optimization scales with the
  ...
  to PALETTE's OWN proxy-vs-KL data** — Gao's published coefficients are **not
  ...
  until α, β are fit to measured PALETTE proxy-vs-KL data; using published
...
- **Info-bottleneck latent-outlier detector (secondary) [deferred — requires calibration data].** A CSI-style
  ...
  **Not enabled until a bottleneck model is trained and validated on PALETTE's own
...
HALT the whole RSI job (and ROLL BACK to the best checkpoint) when divergence is detected:
    ...
    drift d = √D_KL(π‖π_init) past the fitted scaling-law peak  (Signal 4b KL law — DEFERRED until α,β fit to PALETTE data),  OR
    a CSI latent-outlier alarm                                  (Signal 4c — DEFERRED until bottleneck model trained on PALETTE data).
```
...
- **Backbeat (`palette-stats`)** supplies every CI/p-value/power the signals gate on;
  **`palette-eval`** supplies the scorers (deterministic + rotated judge) for the OOD
...
- **Single-preprint evidence.** Signals 3 and 4's headline numbers
  ...
  re-measured on PALETTE's data before a threshold ships.
...
### 21.5b Palette in your coding agent (Claude Code & Codex)

The hosted `palette-mcp` is meant to be attached **inside the coding agent the
...
**OAuth 2.1 resource server**; no Palette-specific auth dance. A coding agent
discovers and authenticates over the standard chain (each step a real endpoint on
`main`, in `crates/palette-oauth-server`):
...
6. Authenticated **streamable-HTTP** `/mcp` calls — the agent now invokes Palette's
   MCP tools with the bearer token. The `/mcp` endpoint is `POST/GET /mcp` over
   axum (`crates/palette-mcp`) and is **[built]**; the full streamable-HTTP
...
```bash
claude mcp add --transport http palette <palette-mcp-url>/mcp
# first tool call opens the dashboard login, completes the OAuth chain above,
# then Palette's tools are available inside Claude Code.
```

**Setup — Codex.** Add Palette as an MCP server in Codex's MCP configuration using
its remote-MCP support, pointing at the same `<palette-mcp-url>/mcp`. Codex performs
...
the URL + HTTP transport are the only Palette-specific values.)

**Local `stdio` transport is the alternate path and is `[built]` for the basic
tool-catalog smoke.** `paletted mcp --stdio` serves newline-delimited JSON-RPC for
...
- **Backbeat real statistics** — every score carries a confidence interval, with
  ...
  **anytime-valid** sequential testing (`palette-stats`, §10.3, §21.2). A "win" is a
...
- **Calibration** — Brier / ECE / reliability curves and judge-vs-human agreement
  (`palette-calibration`, §10.5, §6.3 #7) so the readings are trustworthy.
...
`palettectl quickstart` (§20.8 #6.4) boots compose, provisions a tenant/key, and
...
> **[built]** (`crates/palette-oauth-server`, `crates/palette-mcp`, `paletted mcp
...
### 21.6b Deferred: Agent Studio (`palette-studio`)
...
### 21.6c Deferred: auto-provisioned tool-belt (`palette-toolbelt`)
...
improvement into `palette-mcp` (§21) and treating credits as a later commercial
...
Requires (when productized): a metering/credits service (`palette-credits`, MVP
exists on branch) over the existing `palette-usage` ledger (§10 usage records) +
...
- **`palette-mcp` (CHANGED, primary)** — the improvement tools (§21.1) and composite
  recipes are added here; the loop is *not* a separate server. stdio + streamable-
  HTTP/OAuth 2.1 transports (§21 intro).
- **`palette-stats` (NEW, §10.3)** — supplies the CI/p-value/power the loop gates on.
- **`palette-replay` (CHANGED, §11)** — forked replay backs `simulate` and root-cause.
- **`palette-mcp-improve` (DEFERRED / branch foundation)** — exists on
  `feat/mcp-improve-foundation` as the MVP; its logic folds into `palette-mcp` rather
  than shipping standalone.
- **`palette-credits` (DEFERRED / branch foundation)** — metering exists on branch;
  productization deferred (§21.7).
- **`palette-toolbelt` (DEFERRED, §21.6c)**, **`palette-studio` (DEFERRED, §21.6b)** —
...
- **MVP:** the improvement tools on `palette-mcp` — `index_agent`/`propose_change`/
  `simulate`/`apply_change` — wired to Palette evals/judge/harness/`palette-stats`,
...
| A4 | **Nominal alpha = actual alpha** (the gate's stated error rate is its true rate) | §1 #9/#11, §10.3 | the hardcoded-z path is deleted; real p-values from `palette-stats` | n/a — this is an invariant, not a relaxable assumption; gate is wrong if violated | conformance: simulate many null comparisons, assert empirical false-positive rate ≈ alpha (calibration test) |
...
it.** Commands assume `paletted` on `:8080` (API) / `:4317`/`:4318` (OTLP) and the
...
| **conformance** | the *same* suite run against every backend impl | `palette-store-conformance` (trait suite) |
| **e2e** | live `paletted`: ingest → query → eval → gate → UI | `palettectl` fixtures, `gate1-live-smoke`, `gate2-*`, Playwright |
| **load / bench** | throughput + p95 SLO evidence | `palette-bench` (criterion), `xtask loadgen` **[planned]** |
...
**Ingest (`palette-ingest`, `palette-otlp`, `palette-temporal`).**
...
**Store (`palette-store*`).**
*Tests:* the `palette-store-conformance` suite is the contract — run it against
...
pushdown (§20.2 #0.2). load — `palette-bench` write/query p95 on 1M/10M-span
fixtures (§20.2 #0.3).
*Verify:* `[built]` `cargo test -p palette-store-conformance --workspace` (the
`storage-backends` CI gate runs this); `[planned]`
`cargo run -p paletted -- --trace-store clickhouse` boots and serves.

**Evals / judge (`palette-eval`, `palette-judge`, `palette-sandbox`, `palette-scorers`).**
...
```bash
cargo run -q -p palettectl -- judge-fixture --data-dir /tmp/palette-judge          # encrypted BYOK + cached judge + ledger
cargo run -q -p palettectl -- judge-dataset-fixture --data-dir /tmp/palette-jds    # judge eval over a dataset version
```

**Statistics (`palette-stats`, §10.3).**
...
*Verify:* `[planned]` `cargo test -p palette-stats` (alpha-calibration +
power-refusal tests); until then the gate uses the deleted-and-wrong path and
must not be trusted (§10.3).

**Experiments + gates (`palette-experiments`, `palette-gates`).**
...
```bash
cargo run -q -p palettectl -- judge-experiment-fixture --data-dir /tmp/palette-jx  # judge-backed gate
cargo run -q -p palettectl -- gate-run-fixture --data-dir /tmp/palette-gate        # CI gate passes on no-regression
! cargo run -q -p palettectl -- gate-run --data-dir /tmp/palette-gate \
    --tenant-id demo --project-id demo --gate-id main                            # gate BLOCKS a regression (non-zero exit)
```

**Calibration (`palette-calibration`, §10.5).**
...
*Verify:* `[built]`
`cargo run -q -p palettectl -- calibration-fixture --data-dir /tmp/palette-cal`
(kappa/agreement today; Brier/ECE/map is §20.5 #3.7 `[planned]`).

**Replay & attribution (`palette-replay`, §11).**
...
*Verify:* `[built]`
`cargo run -q -p palettectl -- replay-fixture --data-dir /tmp/palette-replay`
...
each client round-trips a real call against `paletted`.
*Verify:* `[built]`
...
*Verify:* `[planned]` `cargo test -p palette-eval dimensions::` exercising the
§6.3 estimator vector against a seeded multi-dimension fixture.

**RSI loop (`palette-mcp` improvement tools, §21).**
...
reads the untouched Test split and requires a real `palette-stats` p-value at
...
*Verify:* `[planned]` `cargo run -p paletted -- --auth-mode required` then an
...
cargo run -q -p palettectl -- judge-fixture --data-dir /tmp/palette-judge
cargo run -q -p palettectl -- gate-run-fixture --data-dir /tmp/palette-gate
...
- **v0 Substrate** → ingest + store + `palettectl smoke` rows (§22.1) all `[built]`.
...
| **`storage-backends`** | the `palette-store-conformance` trait suite runs against every backend (in-memory, SQLite today; Postgres/ClickHouse as wired, §20.2 #0.1), incl. tenant-isolation (A20) and the `#[ignore]`d container-backed store tests | `cargo test -p palette-store-conformance --workspace`; `cargo test -p palette-store-sql -- --ignored` |
...
| **`browser`** | the `palette-browser*` family (Liveset) builds and its driver/capture tests pass | `cargo test` over the browser crates |
| **`gate1-live-smoke`** | a live `paletted` ingest → query round-trip (`paletted --test live_smoke`); the zero-code-bootstrap and `quickstart` acceptance map to §24 Group A rows | `cargo run -p palettectl -- smoke …` |
...
- **Benchmarks / load (`palette-bench`, `xtask loadgen`) [planned].** Scale/p95 SLO
...
1. **Contract drift — `spec → 7 SDKs → MCP → CLI → docs`** (the `sdk-contract`
   ...
   breaking check. **MCP tools and the `palette api` CLI are not separately
...
2. **Semantic-convention drift — `palette-schema (Beatmap) → conventions.json → 5
   ...
   env-var names have one source — the `conventions` module of `palette-schema` —
...
- **`deploy-backend`** — on push to `main` touching `crates/**`/`bins/**`/Dockerfile/
  `fly.toml`, deploys `paletted` (Fly.io; runtime secrets live in `fly secrets`, the
...
and RSI episodes at once — behind a **low-latency** MCP (Palette MCP) and SDK surface.
...
- **Tokio async runtime, BOUNDED parallelism.** Every fan-out (eval batch, judge
  ...
  is the explicit concurrency cap. *Lives in:* Backbeat (`palette-eval`/`palette-judge`),
  Palette Experiments (`palette-experiments` + RSI tools), Upbeat (`palette-ingest`). *Target:*
...
- **`spawn_blocking` for CPU-bound work.** WASI scorer execution (Soundproof,
  `palette-sandbox`), hashing (request-hash, payload-hash), and any sync compute run
...
- **Backpressure via bounded channels.** In-process smoothing uses **bounded Tokio
  ...
  (`palette-bus`) + the ingest path (`palette-ingest`).

### 23.2 Low-latency MCP (Palette MCP, `palette-mcp`)
...
### 23.4 Parallel eval / judge / RSI (Backbeat + Palette Experiments)

- **Judge broker: pooled, cached, rate-capped, batched.** The judge broker
  (`palette-judge`) holds **per-provider connection pools**, a **request-hash cache**
...
- **WASI instance pool with fuel/epoch limits.** Soundproof (`palette-sandbox`,
  ...
  **fuel** *and* **epoch** deadlines (`consume_fuel(true)` + epoch interruption) and
  ...
  (`palette-sandbox`); the pool + epoch deadline is the scale hardening.
...
### 23.5 Storage & query (Groove, `palette-store*`)
...
| --- | --- | --- | --- |
| judge result | hash(judge prompt + model + params) | Backbeat (`palette-judge`) | determinism (A15) + spend (§16) |
| calibration map | (evaluator, dataset version) | Tuning (`palette-calibration`) | avoid re-fitting isotonic/Platt per call |
| embedding | hash(text + embed model) | Backbeat (judge/OOD probe, §21.4) | OOD novelty + judge embeddings reused |
| MCP spec/op | spec version | Palette MCP (`palette-mcp`) | fast `tools/list`/dispatch (§23.2) |
...
- **Memory bounds.** Hard caps on in-memory structures: the replay **TailBuffer**
  ...
  unbounded input. *Lives in:* Rewind (`palette-replay`), Crate (`palette-store-obj`),
  Drumbeat (`palette-bus`).
...
- **Retry with backoff + jitter; DLQ + poison isolation; idempotency.** Transient
  ...
  (idempotency keys, §8.4). *Lives in:* Drumbeat (`palette-bus`) + Upbeat
  (`palette-ingest`).
...
- **Stateless API + MCP horizontally behind a load balancer.** Mixing Board
  (`palette-api`) and Palette MCP (`palette-mcp`) hold no session state (auth is token-
...
- **Stateful workers in cells; Drumbeat distributes work.** Eval/judge/replay/RSI
  workers run in **cells** and pull from the Drumbeat bus (`palette-bus` →
...
- **Heartbeat metrics.** Heartbeat (`paletted` `metrics.rs`/`metrics_http.rs`, §16)
...
- **Bench + load-test SLO gates.** Tech Rider (`palette-bench` [planned], §22.5
...
| Real statistics | method-appropriate CI, real p-value, test-selection, Holm-BH/FDR, anytime-valid | §22.1 Statistics row (A2–A8); `cargo test -p palette-stats` | planned (hardcoded-z path deleted; `palette-stats` not yet built, §10.3) |
| Candidate-vs-baseline gate blocks a regression | the gate **exits non-zero** on a real confidence-bound regression | §22.1 Experiments+gates; `! palettectl gate-run …` (non-zero exit) `[built]` | partial (built on the deleted-stats path; trustworthy once `palette-stats` lands) |
...
| MCP deployable < 5 min | stdio **and** hosted streamable-HTTP/OAuth, connecting from Claude Code/Cursor/ChatGPT/Codex with `tools/list`+`tools/call` | §22.1 MCP row; `curl POST /mcp` `[built]`; `paletted mcp --stdio` `[built]` | partial (basic transports built; end-client attach proof remains) |
...
| Capability | Binary done-criterion | Verified-by | Status |
| --- | --- | --- | --- |
| Orgs / projects / envs | org/project/environment entities + scoping enforced | §22.1 Store row; `palette-store-conformance` boundaries `[built]` | built |
...
| SSO / SAML / SCIM | enterprise login JIT-provisions a user (OIDC/SAML/SCIM) | §22.1 Hosted control plane (SSO JIT); Passport (`palette-identity` [planned]) | planned |
...
| Quotas / rate-limits / billing ledger | per-tenant quota + rolling-window limit + a usage/billing ledger | §22.1 + Tempo (`palette-usage`) `[built]` ledger; Bandwidth (`palette-billing` [planned]) | partial |
...
| Capability | Binary done-criterion | Verified-by | Status |
| --- | --- | --- | --- |
| palette-bench SLO gates green | criterion + loadgen meet §16 SLOs and gate regressions | §22.5 advisory bench gate → required; Tech Rider (`palette-bench` [planned], §23.10) | planned |
...
| Capability | Binary done-criterion | Verified-by | Status |
| --- | --- | --- | --- |
| Runtime guardrails enforce | a prompt-injection attempt is **blocked in `p95 < 200ms`**, emits a `guardrail.check` span, and auto-builds a red-team dataset (R18.1) | §20.10 #7.1; `palette-guardrails` hook test + `POST /v1/guardrails/check` full-stack | planned |
...
| Failure discovery → named issues | failing traces **auto-cluster into named issues**, each with a counterfactual root cause + weighted frequency (R18.4) | §20.10 #7.4; `palette-insights` clustering + `GET /v1/insights/issues` test | planned |
| Embedding/distribution drift alert | a drift alert fires on an **anytime-valid confidence sequence** (not fixed-N), weighted by §9 (R18.5) | §20.10 #7.5; drift test on `palette-alerts` + mSPRT decision | planned |
...
verification each, exactly how much of Palette is actually finished. This ties
...
the MCP server (§21), and the `palettectl` CLI. This is a direct consequence of the
...
- **OSS self-host — static/SSR build, no cloud.** The same Next.js app builds into
  ...
  and points at the local `paletted`. No Palette Cloud call is required to run it
...
- **The GENERATED OpenAPI client is the ONLY data contract.** The front-end never
  ...
  (`web/dashboard/openapi/palette-read-api.json`), and `web/dashboard/lib/api.ts`
...
- **Auth — OAuth 2.1 session + org/project/env switcher.** Login/session run
  against the **`palette-oauth-server`** authorization server (+ `palette-oauth`,
  `palette-accounts`) — the same RFC 9728→8414→7591→authorize/token chain the
...
| `/experiments/[id]` **Palette Experiments** | A/B candidate-vs-baseline | per-case score table; baseline-vs-candidate **deltas with `ci_low/ci_high`**; **gate badge** (pass/fail/**inconclusive**); trace deep-links; per-slice segments | `ExperimentRunReport`; `GET /v1/experiments/:tenant/:project` (§20.4 #2.3, §20.5 #3.5) | **[needs read-API]** |
...
| `/review` **Setlist** | human review / annotation queues | task inbox; inline `AnnotationPanel` on span detail; **keyboard labeling**; `submitReviewAnnotation` | `palette-human` read + `submitReviewAnnotation` (§20.4 #2.5) | **[planned]** (backend built; UI not; no new read-API needed) |
...
| `/evolution` **Palette Experiments RSI** | agent-evolution / RSI episodes | propose→simulate→accept timeline; **§21.4 guardrail-signal views** (held-out gap, **OOD-probe** result, smoothness/sensitivity, EvalStop), per-episode reward + CI; the OOD-reject verdict | §21 RSI tool results + §21.4 guardrail signals | **[planned]** (entire §21 is planned) |
...
| `/docs` **front-of-house docs** | in-app API/MCP reference | renders the committed OpenAPI snapshot (Scalar) → cannot drift (§22.5); MCP + quickstart guides | the committed `palette-api.json` (§3.3 docs row) | **[built]** (`/docs`, `/docs/mcp`, `/docs/quickstarts`) |
| `/connect` **coding-agent connect** | the §21.5b Claude-Code / Codex / ChatGPT connect screen | shows `claude mcp add …` + Codex MCP config; triggers the OAuth browser flow; verifies `tools/list` | `palette-oauth-server` connect chain (§21.5b) | **[partial]** (OAuth server + `/mcp` built; the connect *screen* + e2e verify planned) |
...
| **S3 — Experiments & gates** | `/experiments/[id]` (CIs + gate badge + segments) | §20.4 #2.3, §20.5 #3.5 (and trustworthy stats need `palette-stats`, §20.5 #3.4) | **[needs read-API]** |
...
- **Honest markers** ([built]/[partial]/[planned]/[needs read-API]) and the
  ...
  **Soundstage**, and each screen carries its beat-box (Encore, Backbeat,
  Palette Experiments, Setlist, Mixdown, Rewind, Crate Dig, Tempo/Heartbeat) crosswalk.
...
Palette already holds**, and a **small, high-leverage set of targeted
...
| Incumbent | Trace store | Self-host shape | Sourced weakness Palette attacks |
| --- | --- | --- | --- |
...
teardowns — is the exact intersection Palette is built on: **Rust-first +
...
| --- | --- | --- | --- | --- |
| O1 | **Embedded columnar HOT tier** (DataFusion/Arrow over Parquet) as a first-class `TraceStore` backend, not cold-only | Brainstore/adb proprietary; Langfuse/Helicone need a ClickHouse cluster; Phoenix row-store 200M ceiling | Groove + Cold Storage (`palette-store-sql`/`palette-archive`) | [planned] |
| O2 | **Synchronous, immediately-consistent ingest** as a stated guarantee (direct mode) | Langfuse/Braintrust async queue lag ("where's my trace?") | Upbeat (`palette-ingest`) + §16 SLO | [partial] (direct mode built; guarantee not codified) |
| O3 | **Content-addressed (blake3 CAS) artifact/cassette store** with structural dedup of repeated prompts/tool-schemas/bodies | Braintrust per-GB tax on tens-of-MB spans; all incumbents store full bodies | Crate (`palette-store-obj`) | [planned] |
| O4 | **Anytime-valid early-stop of eval/experiment runs** — stop once the gate is statistically decided at power | every incumbent runs **fixed-N** eval suites and bills every judge call | Backbeat (`palette-stats` mSPRT, reusing §10.3 #6) | [planned] |
| O5 | **Verifier-first eval cascade** — cheap deterministic WASI scorers first, judge only on the unresolved/disputed remainder | Judgment/Phoenix/Braintrust route everything through the LLM judge | Backbeat (`palette-eval`/`palette-judge`) | [planned] |
...
| O7 | **blake3 on the hot hashing path** (payload/idempotency), SHA-256 only where an external contract requires it | n/a (pure micro-opt; resolves the §9 hash description) | Downbeat/Upbeat (`palette-core`/`palette-ingest`) | [planned] |
| O8 | **Stock-OTel interop completeness** — the churn-prone new `gen_ai.*` names + the full OpenInference kind/attribute set (incl. `RERANKER`, `graph.node.*`, `llm.cost.*`) | adoption friction; "lights up any already-instrumented app with zero code" is the cheapest, highest-leverage interop win | Upbeat (`palette-otlp` normalizer, §7) | [partial] |
...
DataFusion/Arrow/Parquet confined to the **cold** archive (`palette-archive`). The
...
away as Apache-2.0 in one binary, on deps Palette already vendors (`arrow`/`parquet`/
`datafusion` in `palette-archive`). It collapses Langfuse's biggest complaint (the
...
`palette-store-obj` stores each distinct body **once** and references it everywhere
...
Palette's footprint shrinks on the most repetitive workloads — a margin that compounds
with O1 (smaller columnar payloads) and §11 cassettes (replay bodies dedup against
trace bodies).

**O4 — early-stopping evals is a cost margin no incumbent has.** Palette already
...
and bill every call; Palette returns the *same* error-controlled verdict for a fraction
of the judge spend, and — because the stop is anytime-valid — without the peeking
inflation a naive early-stop would cause (A7/A8). This reuses `palette-stats`; it is a
...
**Phase 7 (§20.10)**, and the *moat* is the same one §26.1 names: Palette's plan is to
...
> **Status (honest, per §4 convention): every "Palette's answer" below is `[planned]`
...
| Incumbent product surface | Who ships it (strongest) | Palette's answer *(all [planned], §24.4)* | The rigor edge (why ours is designed to be correct, not just present) |
| --- | --- | --- | --- |
| **Runtime guardrails / firewall** (block injection/PII/toxicity at the decision layer) | Galileo **Protect**, Opik Guardrails, Patronus, Arize/Guardrails-AI | **Bouncer** `palette-guardrails` (§20.10 #7.1) | every enforcement emits an observable `guardrail.check` span (§5.2) and is itself an eval; blocked attempts auto-build a red-team dataset — enforcement is *measured*, not a black box |
...
| **Named optimizers** (MIPRO/GEPA/evolutionary/Bayesian) | Comet **Opik Optimizer** (GA), DSPy, LangSmith **Polly** | `OptimizerStrategy` in `palette-experiments` (§20.10 #7.6) | the same optimizers, but every candidate flows through Palette's held-out + multiplicity-corrected gate — un-defers §21's evolutionary search *safely* |
...
| **Embeddings / drift analysis** (UMAP, centroid/eval drift) | **Arize** (ML-observability heritage) | embedding-drift in `palette-alerts` + Medley (§20.10 #7.5) | drift decided on an **anytime-valid confidence sequence** (§10.3 #6), not a fixed-N peek; reported with §9 weighting, never a bare distance |
...
| **NL trace-analysis copilot** | LangSmith **Polly**, Arize **Alyx**, Opik **OpikAssist**, Braintrust **Loop**, Judgment Agent | **`palette-mcp` inside your coding agent** (§21.5b) | not a walled-garden chatbot — the *entire quantitative stack* (stats + calibration + held-out gate) attached to Claude Code / Codex / any MCP client, so the agent's self-improvement is statistically gated, not vibes |
| **Prompt mgmt: runtime rollback + client caching** | **Langfuse**, LangSmith Prompt Hub | **Mixdown** `palette-prompts` (§20.6 #4.7) | already planned; add Langfuse's client-side cache + rollback-without-redeploy semantics as `PromptVersion` resolution properties |
...
agents** (LangSmith Deployment / LangGraph Platform) — Palette *observes, evaluates, and
improves* agents; it does not host the customer's agent runtime. Hosting is a different
product with a different failure surface; staying out of it keeps Palette framework- and
...
The net thesis: **for every product surface an incumbent is known for, Palette has a
...
- **O3/O7** extend §5.3 (raw-envelope hashing) and §11 (cassettes), and resolve the §9
  ...
  (corrected in §9).
- **O4/O5/O6** extend §10.3 (`palette-stats`), §10.1 (lanes), §21.2 (typed reward), and
...
- **§26.4 (product-surface parity)** maps 1:1 onto **Phase 7 (§20.10)**: each row's
  "Palette's answer" is a §20.10 item, registered in §4 (new crates Bouncer/Patchbay/
...
| **E3** | Mid-trace span substitution | inject a recorded/synthetic response at `(seq, kind, request_hash)` and replay forward; one engine in `palette-replay` | bisection, fault injection, ablation, sensitivity, regret — **one build serves §27.4 + the `attribute_failure` work** |
...
- **Convergence / diminishing-returns stop signal.** Over the last `N` deltas in
  ...
  Uses the real-statistics work tracked in `palette-stats`. [planned]
...
2. **Judge score-distribution drift** (E2) — ceiling-clustering (>N% of cases at
   ...
   cross-checked against `palette-calibration` judge↔human agreement to separate
...
`attribute_failure` work (§11); convergence + real CIs use `palette-stats` (§20.5);
...
*other axis*: **how the agent ecosystem plugs into Palette, and how Palette sources and
...
**passive**: Palette is not present inside any framework's tool/handoff lifecycle, and
...
**ingest (passive) → adapter (present in the lifecycle) → bidirectional (Palette's
...
| --- | --- | --- | --- |
| Generic OTLP / OpenInference / OpenLLMetry ingest | **[built]** | `palette-otlp`, `palette-ingest` (§7) | the zero-code floor; any instrumented app lights up |
| OTel `gen_ai.*` semantic-convention completeness | **[partial]** | `palette-otlp` (§26 O8) | churn-prone new names + full OpenInference kind/attr set; #93 |
| browser-use / Stagehand SDKs | **[built]** | `sdks/python-browser-use`, `sdks/ts-stagehand` | the one shipped first-class agent vertical |
| Temporal history → canonical spans | **[built]** | `palette-temporal` | sub-agent steps map cleanly to canonical spans |
...
The `SourceImporter` trait (`palette-temporal/src/lib.rs:256`) + unified `POST /v1/import`
endpoint are **[built]** but only `temporal_history` + `native` are wired
(`paletted/src/main.rs`). Importers for **LangSmith / Langfuse / Braintrust / Phoenix**
...
| **Composio / MCP directories** (outbound: expose `gate_candidate`/`promote_failures` etc. *as* registry tools) | **[planned]** | PARTNER / BUILD | makes Palette's loop callable from any agent; depends on #277 composite tools |
| **`palette-mcp` attach inside coding agents** (Claude Code / Codex / ChatGPT connectors, OAuth 2.1 RFC 9728→8414→7591) | **[partial]** | BUILD | connect flow built (§21.5b); **stdio transport + e2e verify still missing (#277)** |
