...
def _patch_client_class(module: Any, class_name: str, wrapper: Wrapper, provider: str) -> InstrumentationResult:
    ...
    if getattr(client_class, "_palette_auto_instrumented", False):
    ...
    def __init__(self: Any, *args: Any, **kwargs: Any) -> None:
        ...
        if getattr(self, "_palette_wrapped", False):
            return
        try:
            wrapper(self)
        except Exception as exc:  # noqa: BLE001 - instrumentation must fail open.
            self._palette_auto_instrument_error = repr(exc)

    client_class.__init__ = __init__
    client_class._palette_original_init = original_init
    client_class._palette_auto_instrumented = True
...
def instrument(providers: Optional[Iterable[str]] = None) -> Tuple[InstrumentationResult, ...]:
    """Patch installed provider clients so new instances emit Palette spans.
