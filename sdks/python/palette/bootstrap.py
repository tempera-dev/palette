...
select the ``palette`` OpenTelemetry configurator to initialize tracing.
"""
...
def _provider_request(env: Mapping[str, str]) -> Optional[Tuple[str, ...]]:
    requested = env.get("PALETTE_AUTO_INSTRUMENT", "")
    normalized = requested.strip().lower()
    providers = env.get("PALETTE_AUTO_INSTRUMENT_PROVIDERS")
...
def _apply_auto_instrumentation(env: Mapping[str, str]) -> Tuple[AutoInstrumentationStatus, ...]:
    ...
    try:
        auto = importlib.import_module("palette.auto")
    except ImportError:
    ...
    if instrument is None:
        return (
            AutoInstrumentationStatus(
                provider="providers",
                instrumented=False,
                status="palette.auto.instrument is not available",
            ),
        )
...
def bootstrap_from_env(*, force: bool = False) -> BootstrapResult:
    """Initialize Palette tracing from ``PALETTE_*`` environment variables.

    The existing ``palette.tracing.init`` resolver owns connection defaults and
...
class PaletteConfigurator(_ConfiguratorBase):
