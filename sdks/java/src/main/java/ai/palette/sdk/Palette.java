...
/**
 * Ergonomic entry point for tracing arbitrary code as Palette spans.
 *
 * <p>Call {@link #init(PaletteConfig)} once at process start, then wrap work in
 * {@link #observe(String, String, Supplier)}.
 */
public final class Palette {

    private static final String TRACER_NAME = "palette.sdk";

    private static volatile OpenTelemetrySdk sdk;
    private static volatile SdkTracerProvider provider;
    private static volatile PaletteConfig config;
    private static volatile Tracer tracer;
    private static final AtomicLong SEQ = new AtomicLong(0);

    private Palette() {}

    /** Initialize the Palette tracer with an OTLP/HTTP exporter. Call once at startup. */
    public static synchronized void init(PaletteConfig cfg) {
     ...
     */
    public static synchronized void init(PaletteConfig cfg, SpanProcessor processor) {
        configure(cfg, processor);
    }

    private static void configure(PaletteConfig cfg, SpanProcessor processor) {
