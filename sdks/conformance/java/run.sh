...
#   1. Install the generated client (ai.palette:palette-client) into the local repo.
#   2. Build a tiny module that depends on it and round-trips health +
#      createDataset + listTraces against a live paletted, asserting typed shapes.
set -euo pipefail
...
java -jar "$here/target/palette-conformance.jar"
