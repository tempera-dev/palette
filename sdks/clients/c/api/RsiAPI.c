...
RsiAPI_gateOptimizationCandidate(apiClient_t *apiClient, char *tenant_id, char *project_id, gate_candidate_request_t *gate_candidate_request, char *authorization, char *x_beater_api_key, char *x_beater_project_id, char *x_beater_environment_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/v1/rsi/{tenant_id}/{project_id}/gate-candidate");
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_gate_candidate_request = NULL;
    if (gate_candidate_request != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_gate_candidate_request = gate_candidate_request_convertToJSON(gate_candidate_request);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_gate_candidate_request);
    ...
    //    printf("%s\n","Gate an optimization candidate against the held-out Test split and the anti-overfitting guardrail");
    //}
    // uncomment below to debug the error response
    //if (apiClient->response_code == 400) {
    //    printf("%s\n","Invalid request, scope, or under-powered split");
    //}
    // uncomment below to debug the error response
    ...
    gate_candidate_response_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *RsiAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = gate_candidate_response_parseFromJSON(RsiAPIlocalVarJSON);
        cJSON_Delete(RsiAPIlocalVarJSON);
    ...
    if (localVarSingleItemJSON_gate_candidate_request) {
        cJSON_Delete(localVarSingleItemJSON_gate_candidate_request);
        localVarSingleItemJSON_gate_candidate_request = NULL;
    }
