...
ConnectAPI_connectGetPaletteConnectStatus(apiClient_t *apiClient, char *tenant_id, char *project_id, char *authorization, char *x_palette_api_key, char *x_palette_project_id, char *x_palette_environment_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/v1/connect/status/{tenant_id}/{project_id}");
    ...
    //    printf("%s\n","Palette product connection status for account-console setup");
    //}
    // uncomment below to debug the error response
    ...
    palette_connect_status_response_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *ConnectAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = palette_connect_status_response_parseFromJSON(ConnectAPIlocalVarJSON);
        cJSON_Delete(ConnectAPIlocalVarJSON);
