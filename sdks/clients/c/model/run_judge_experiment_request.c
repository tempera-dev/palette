...
#include <string.h>
#include <stdio.h>
#include "run_judge_experiment_request.h"



static run_judge_experiment_request_t *run_judge_experiment_request_create_internal(
    ...
    run_judge_experiment_request_t *run_judge_experiment_request_local_var = malloc(sizeof(run_judge_experiment_request_t));
    if (!run_judge_experiment_request_local_var) {
        return NULL;
    }
    run_judge_experiment_request_local_var->baseline_outputs = baseline_outputs;
    run_judge_experiment_request_local_var->baseline_release_id = baseline_release_id;
    run_judge_experiment_request_local_var->candidate_outputs = candidate_outputs;
    run_judge_experiment_request_local_var->candidate_release_id = candidate_release_id;
    run_judge_experiment_request_local_var->evaluator_id = evaluator_id;
    run_judge_experiment_request_local_var->evaluator_version_id = evaluator_version_id;
    run_judge_experiment_request_local_var->gate_policy = gate_policy;
    run_judge_experiment_request_local_var->kind = kind;
    run_judge_experiment_request_local_var->provider_secret_id = provider_secret_id;

    run_judge_experiment_request_local_var->_library_owned = 1;
    return run_judge_experiment_request_local_var;
}
...
__attribute__((deprecated)) run_judge_experiment_request_t *run_judge_experiment_request_create(
    ...
    return run_judge_experiment_request_create_internal (
        baseline_outputs,
        baseline_release_id,
        candidate_outputs,
...
void run_judge_experiment_request_free(run_judge_experiment_request_t *run_judge_experiment_request) {
    if(NULL == run_judge_experiment_request){
        return ;
    }
    if(run_judge_experiment_request->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "run_judge_experiment_request_free");
        return ;
    }
    listEntry_t *listEntry;
    if (run_judge_experiment_request->baseline_outputs) {
        list_ForEach(listEntry, run_judge_experiment_request->baseline_outputs) {
            case_output_override_request_free(listEntry->data);
        }
        list_freeList(run_judge_experiment_request->baseline_outputs);
        run_judge_experiment_request->baseline_outputs = NULL;
    }
    if (run_judge_experiment_request->baseline_release_id) {
        free(run_judge_experiment_request->baseline_release_id);
        run_judge_experiment_request->baseline_release_id = NULL;
    }
    if (run_judge_experiment_request->candidate_outputs) {
        list_ForEach(listEntry, run_judge_experiment_request->candidate_outputs) {
            case_output_override_request_free(listEntry->data);
        }
        list_freeList(run_judge_experiment_request->candidate_outputs);
        run_judge_experiment_request->candidate_outputs = NULL;
    }
    if (run_judge_experiment_request->candidate_release_id) {
        free(run_judge_experiment_request->candidate_release_id);
        run_judge_experiment_request->candidate_release_id = NULL;
    }
    if (run_judge_experiment_request->evaluator_id) {
        free(run_judge_experiment_request->evaluator_id);
        run_judge_experiment_request->evaluator_id = NULL;
    }
    if (run_judge_experiment_request->evaluator_version_id) {
        free(run_judge_experiment_request->evaluator_version_id);
        run_judge_experiment_request->evaluator_version_id = NULL;
    }
    if (run_judge_experiment_request->gate_policy) {
        gate_policy_free(run_judge_experiment_request->gate_policy);
        run_judge_experiment_request->gate_policy = NULL;
    }
    if (run_judge_experiment_request->kind) {
        evaluator_kind_free(run_judge_experiment_request->kind);
        run_judge_experiment_request->kind = NULL;
    }
    if (run_judge_experiment_request->provider_secret_id) {
        free(run_judge_experiment_request->provider_secret_id);
        run_judge_experiment_request->provider_secret_id = NULL;
    }
    free(run_judge_experiment_request);
}
...
cJSON *run_judge_experiment_request_convertToJSON(run_judge_experiment_request_t *run_judge_experiment_request) {
    ...
    if (run_judge_experiment_request->baseline_outputs) {
    list_ForEach(baseline_outputsListEntry, run_judge_experiment_request->baseline_outputs) {
    ...
    if (run_judge_experiment_request->candidate_outputs) {
    list_ForEach(candidate_outputsListEntry, run_judge_experiment_request->candidate_outputs) {
    ...
    if(run_judge_experiment_request->gate_policy) {
    cJSON *gate_policy_local_JSON = gate_policy_convertToJSON(run_judge_experiment_request->gate_policy);
    ...
    // run_judge_experiment_request->kind
    if (!run_judge_experiment_request->kind) {
        goto fail;
    }
    cJSON *kind_local_JSON = evaluator_kind_convertToJSON(run_judge_experiment_request->kind);
...
run_judge_experiment_request_t *run_judge_experiment_request_parseFromJSON(cJSON *run_judge_experiment_requestJSON){
    ...
    run_judge_experiment_request_t *run_judge_experiment_request_local_var = NULL;

    // define the local list for run_judge_experiment_request->baseline_outputs
    list_t *baseline_outputsList = NULL;

    // define the local list for run_judge_experiment_request->candidate_outputs
    list_t *candidate_outputsList = NULL;

    // define the local variable for run_judge_experiment_request->gate_policy
    gate_policy_t *gate_policy_local_nonprim = NULL;

    // define the local variable for run_judge_experiment_request->kind
    ...
    // run_judge_experiment_request->kind
    cJSON *kind = cJSON_GetObjectItemCaseSensitive(run_judge_experiment_requestJSON, "kind");
    ...
    run_judge_experiment_request_local_var = run_judge_experiment_request_create_internal (
