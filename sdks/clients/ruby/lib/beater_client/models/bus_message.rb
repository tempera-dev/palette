  ...
  class BusMessage
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BeaterClient::BusMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BeaterClient::BusMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'enqueued_at')
        self.enqueued_at = attributes[:'enqueued_at']
      else
    ...
    def list_invalid_properties
      ...
      if @attempts < 0
        invalid_properties.push('invalid value for "attempts", must be greater than or equal to 0.')
      end

      if @enqueued_at.nil?
        invalid_properties.push('invalid value for "enqueued_at", enqueued_at cannot be nil.')
      end
    ...
    def attempts=(attempts)
      ...
      if attempts < 0
        fail ArgumentError, 'invalid value for "attempts", must be greater than or equal to 0.'
      end
    ...
    def hash
      [attempts, enqueued_at, idempotency_key, kind, max_attempts, message_id, payload, project_id, tenant_id].hash
    end
