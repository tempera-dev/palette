=begin
#Beater API

#Agent observability, evaluation, gating, and human-review APIs for Beater

The version of the OpenAPI document: 0.1.0

Generated by: https://openapi-generator.tech
Generator version: 7.11.0

=end

require 'spec_helper'
require 'json'

# Unit tests for BeaterClient::SearchApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'SearchApi' do
  before do
    # run before each test
    @api_instance = BeaterClient::SearchApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of SearchApi' do
    it 'should create an instance of SearchApi' do
      expect(@api_instance).to be_instance_of(BeaterClient::SearchApi)
    end
  end

  # unit tests for search_spans
  # @param tenant_id tenant_id
  # @param [Hash] opts the optional parameters
  # @option opts [String] :q 
  # @option opts [String] :project_id 
  # @option opts [String] :environment_id 
  # @option opts [String] :trace_id 
  # @option opts [String] :span_id 
  # @option opts [String] :kind 
  # @option opts [String] :status 
  # @option opts [String] :model 
  # @option opts [String] :tool 
  # @option opts [Integer] :limit 
  # @option opts [String] :authorization Bearer API token for strict auth
  # @option opts [String] :x_beater_api_key API key alternative for strict auth
  # @option opts [String] :x_beater_project_id Strict-auth project scope
  # @option opts [String] :x_beater_environment_id Strict-auth environment scope
  # @return [SearchResponse]
  describe 'search_spans test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

end
