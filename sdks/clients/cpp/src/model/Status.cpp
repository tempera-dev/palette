...
bool Status::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t(U("error"))))
    {
        ...
        {
            std::shared_ptr<Status_error> refVal_setError;
...
std::shared_ptr<Status_error> Status::getError() const
{
    return m_Error;
}


void Status::setError(const std::shared_ptr<Status_error>& value)
{
    m_Error = value;
    m_ErrorIsSet = true;
}

bool Status::errorIsSet() const
{
    return m_ErrorIsSet;
}

void Status::unsetError()
{
    m_ErrorIsSet = false;
}
