...
export class ConnectApi extends runtime.BaseAPI {
    ...
    async connectGetStatusRaw(requestParameters: ConnectGetStatusRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<PaletteConnectStatusResponse>> {
        ...
        const response = await this.request({
            path: `/v1/connect/status/{tenantId}/{projectId}`.replace(`{${"tenantId"}}`, encodeURIComponent(String(requestParameters['tenantId']))).replace(`{${"projectId"}}`, encodeURIComponent(String(requestParameters['projectId']))),
            method: 'GET',
