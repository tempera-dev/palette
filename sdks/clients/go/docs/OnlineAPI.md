...
> SamplingDecision DecideOnlineSampling(ctx, tenantId, projectId, traceId).OnlineSamplingPolicy(onlineSamplingPolicy).Authorization(authorization).XBeaterApiKey(xBeaterApiKey).XBeaterProjectId(xBeaterProjectId).XBeaterEnvironmentId(xBeaterEnvironmentId).Execute()
...
func main() {
	...
	resp, r, err := apiClient.OnlineAPI.DecideOnlineSampling(context.Background(), tenantId, projectId, traceId).OnlineSamplingPolicy(onlineSamplingPolicy).Authorization(authorization).XBeaterApiKey(xBeaterApiKey).XBeaterProjectId(xBeaterProjectId).XBeaterEnvironmentId(xBeaterEnvironmentId).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `OnlineAPI.DecideOnlineSampling``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `DecideOnlineSampling`: SamplingDecision
	fmt.Fprintf(os.Stdout, "Response from `OnlineAPI.DecideOnlineSampling`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiDecideOnlineSamplingRequest struct via the builder pattern
