/*
Beater API

Agent observability, evaluation, gating, and human-review APIs for Beater

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package beaterclient

import (
	"encoding/json"
	"time"
	"bytes"
	"fmt"
)

// checks if the PageRunSummaryItemsInner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PageRunSummaryItemsInner{}

// PageRunSummaryItemsInner struct for PageRunSummaryItemsInner
type PageRunSummaryItemsInner struct {
	DurationMs NullableInt64 `json:"duration_ms,omitempty"`
	EndedAt NullableTime `json:"ended_at,omitempty"`
	FirstSpanName string `json:"first_span_name"`
	Models []ModelRef `json:"models"`
	ProjectId string `json:"project_id"`
	ReleaseIds []string `json:"release_ids"`
	SpanCount int32 `json:"span_count"`
	StartedAt time.Time `json:"started_at"`
	Status SpanStatus `json:"status"`
	TenantId string `json:"tenant_id"`
	TotalCost NullableMoney `json:"total_cost,omitempty"`
	TraceId string `json:"trace_id"`
}

type _PageRunSummaryItemsInner PageRunSummaryItemsInner

// NewPageRunSummaryItemsInner instantiates a new PageRunSummaryItemsInner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPageRunSummaryItemsInner(firstSpanName string, models []ModelRef, projectId string, releaseIds []string, spanCount int32, startedAt time.Time, status SpanStatus, tenantId string, traceId string) *PageRunSummaryItemsInner {
	this := PageRunSummaryItemsInner{}
	this.FirstSpanName = firstSpanName
	this.Models = models
	this.ProjectId = projectId
	this.ReleaseIds = releaseIds
	this.SpanCount = spanCount
	this.StartedAt = startedAt
	this.Status = status
	this.TenantId = tenantId
	this.TraceId = traceId
	return &this
}

// NewPageRunSummaryItemsInnerWithDefaults instantiates a new PageRunSummaryItemsInner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPageRunSummaryItemsInnerWithDefaults() *PageRunSummaryItemsInner {
	this := PageRunSummaryItemsInner{}
	return &this
}

// GetDurationMs returns the DurationMs field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PageRunSummaryItemsInner) GetDurationMs() int64 {
	if o == nil || IsNil(o.DurationMs.Get()) {
		var ret int64
		return ret
	}
	return *o.DurationMs.Get()
}

// GetDurationMsOk returns a tuple with the DurationMs field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PageRunSummaryItemsInner) GetDurationMsOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return o.DurationMs.Get(), o.DurationMs.IsSet()
}

// HasDurationMs returns a boolean if a field has been set.
func (o *PageRunSummaryItemsInner) HasDurationMs() bool {
	if o != nil && o.DurationMs.IsSet() {
		return true
	}

	return false
}

// SetDurationMs gets a reference to the given NullableInt64 and assigns it to the DurationMs field.
func (o *PageRunSummaryItemsInner) SetDurationMs(v int64) {
	o.DurationMs.Set(&v)
}
// SetDurationMsNil sets the value for DurationMs to be an explicit nil
func (o *PageRunSummaryItemsInner) SetDurationMsNil() {
	o.DurationMs.Set(nil)
}

// UnsetDurationMs ensures that no value is present for DurationMs, not even an explicit nil
func (o *PageRunSummaryItemsInner) UnsetDurationMs() {
	o.DurationMs.Unset()
}

// GetEndedAt returns the EndedAt field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PageRunSummaryItemsInner) GetEndedAt() time.Time {
	if o == nil || IsNil(o.EndedAt.Get()) {
		var ret time.Time
		return ret
	}
	return *o.EndedAt.Get()
}

// GetEndedAtOk returns a tuple with the EndedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PageRunSummaryItemsInner) GetEndedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return o.EndedAt.Get(), o.EndedAt.IsSet()
}

// HasEndedAt returns a boolean if a field has been set.
func (o *PageRunSummaryItemsInner) HasEndedAt() bool {
	if o != nil && o.EndedAt.IsSet() {
		return true
	}

	return false
}

// SetEndedAt gets a reference to the given NullableTime and assigns it to the EndedAt field.
func (o *PageRunSummaryItemsInner) SetEndedAt(v time.Time) {
	o.EndedAt.Set(&v)
}
// SetEndedAtNil sets the value for EndedAt to be an explicit nil
func (o *PageRunSummaryItemsInner) SetEndedAtNil() {
	o.EndedAt.Set(nil)
}

// UnsetEndedAt ensures that no value is present for EndedAt, not even an explicit nil
func (o *PageRunSummaryItemsInner) UnsetEndedAt() {
	o.EndedAt.Unset()
}

// GetFirstSpanName returns the FirstSpanName field value
func (o *PageRunSummaryItemsInner) GetFirstSpanName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.FirstSpanName
}

// GetFirstSpanNameOk returns a tuple with the FirstSpanName field value
// and a boolean to check if the value has been set.
func (o *PageRunSummaryItemsInner) GetFirstSpanNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.FirstSpanName, true
}

// SetFirstSpanName sets field value
func (o *PageRunSummaryItemsInner) SetFirstSpanName(v string) {
	o.FirstSpanName = v
}

// GetModels returns the Models field value
func (o *PageRunSummaryItemsInner) GetModels() []ModelRef {
	if o == nil {
		var ret []ModelRef
		return ret
	}

	return o.Models
}

// GetModelsOk returns a tuple with the Models field value
// and a boolean to check if the value has been set.
func (o *PageRunSummaryItemsInner) GetModelsOk() ([]ModelRef, bool) {
	if o == nil {
		return nil, false
	}
	return o.Models, true
}

// SetModels sets field value
func (o *PageRunSummaryItemsInner) SetModels(v []ModelRef) {
	o.Models = v
}

// GetProjectId returns the ProjectId field value
func (o *PageRunSummaryItemsInner) GetProjectId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ProjectId
}

// GetProjectIdOk returns a tuple with the ProjectId field value
// and a boolean to check if the value has been set.
func (o *PageRunSummaryItemsInner) GetProjectIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ProjectId, true
}

// SetProjectId sets field value
func (o *PageRunSummaryItemsInner) SetProjectId(v string) {
	o.ProjectId = v
}

// GetReleaseIds returns the ReleaseIds field value
func (o *PageRunSummaryItemsInner) GetReleaseIds() []string {
	if o == nil {
		var ret []string
		return ret
	}

	return o.ReleaseIds
}

// GetReleaseIdsOk returns a tuple with the ReleaseIds field value
// and a boolean to check if the value has been set.
func (o *PageRunSummaryItemsInner) GetReleaseIdsOk() ([]string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ReleaseIds, true
}

// SetReleaseIds sets field value
func (o *PageRunSummaryItemsInner) SetReleaseIds(v []string) {
	o.ReleaseIds = v
}

// GetSpanCount returns the SpanCount field value
func (o *PageRunSummaryItemsInner) GetSpanCount() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.SpanCount
}

// GetSpanCountOk returns a tuple with the SpanCount field value
// and a boolean to check if the value has been set.
func (o *PageRunSummaryItemsInner) GetSpanCountOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SpanCount, true
}

// SetSpanCount sets field value
func (o *PageRunSummaryItemsInner) SetSpanCount(v int32) {
	o.SpanCount = v
}

// GetStartedAt returns the StartedAt field value
func (o *PageRunSummaryItemsInner) GetStartedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.StartedAt
}

// GetStartedAtOk returns a tuple with the StartedAt field value
// and a boolean to check if the value has been set.
func (o *PageRunSummaryItemsInner) GetStartedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.StartedAt, true
}

// SetStartedAt sets field value
func (o *PageRunSummaryItemsInner) SetStartedAt(v time.Time) {
	o.StartedAt = v
}

// GetStatus returns the Status field value
func (o *PageRunSummaryItemsInner) GetStatus() SpanStatus {
	if o == nil {
		var ret SpanStatus
		return ret
	}

	return o.Status
}

// GetStatusOk returns a tuple with the Status field value
// and a boolean to check if the value has been set.
func (o *PageRunSummaryItemsInner) GetStatusOk() (*SpanStatus, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Status, true
}

// SetStatus sets field value
func (o *PageRunSummaryItemsInner) SetStatus(v SpanStatus) {
	o.Status = v
}

// GetTenantId returns the TenantId field value
func (o *PageRunSummaryItemsInner) GetTenantId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TenantId
}

// GetTenantIdOk returns a tuple with the TenantId field value
// and a boolean to check if the value has been set.
func (o *PageRunSummaryItemsInner) GetTenantIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TenantId, true
}

// SetTenantId sets field value
func (o *PageRunSummaryItemsInner) SetTenantId(v string) {
	o.TenantId = v
}

// GetTotalCost returns the TotalCost field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PageRunSummaryItemsInner) GetTotalCost() Money {
	if o == nil || IsNil(o.TotalCost.Get()) {
		var ret Money
		return ret
	}
	return *o.TotalCost.Get()
}

// GetTotalCostOk returns a tuple with the TotalCost field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PageRunSummaryItemsInner) GetTotalCostOk() (*Money, bool) {
	if o == nil {
		return nil, false
	}
	return o.TotalCost.Get(), o.TotalCost.IsSet()
}

// HasTotalCost returns a boolean if a field has been set.
func (o *PageRunSummaryItemsInner) HasTotalCost() bool {
	if o != nil && o.TotalCost.IsSet() {
		return true
	}

	return false
}

// SetTotalCost gets a reference to the given NullableMoney and assigns it to the TotalCost field.
func (o *PageRunSummaryItemsInner) SetTotalCost(v Money) {
	o.TotalCost.Set(&v)
}
// SetTotalCostNil sets the value for TotalCost to be an explicit nil
func (o *PageRunSummaryItemsInner) SetTotalCostNil() {
	o.TotalCost.Set(nil)
}

// UnsetTotalCost ensures that no value is present for TotalCost, not even an explicit nil
func (o *PageRunSummaryItemsInner) UnsetTotalCost() {
	o.TotalCost.Unset()
}

// GetTraceId returns the TraceId field value
func (o *PageRunSummaryItemsInner) GetTraceId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TraceId
}

// GetTraceIdOk returns a tuple with the TraceId field value
// and a boolean to check if the value has been set.
func (o *PageRunSummaryItemsInner) GetTraceIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TraceId, true
}

// SetTraceId sets field value
func (o *PageRunSummaryItemsInner) SetTraceId(v string) {
	o.TraceId = v
}

func (o PageRunSummaryItemsInner) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PageRunSummaryItemsInner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.DurationMs.IsSet() {
		toSerialize["duration_ms"] = o.DurationMs.Get()
	}
	if o.EndedAt.IsSet() {
		toSerialize["ended_at"] = o.EndedAt.Get()
	}
	toSerialize["first_span_name"] = o.FirstSpanName
	toSerialize["models"] = o.Models
	toSerialize["project_id"] = o.ProjectId
	toSerialize["release_ids"] = o.ReleaseIds
	toSerialize["span_count"] = o.SpanCount
	toSerialize["started_at"] = o.StartedAt
	toSerialize["status"] = o.Status
	toSerialize["tenant_id"] = o.TenantId
	if o.TotalCost.IsSet() {
		toSerialize["total_cost"] = o.TotalCost.Get()
	}
	toSerialize["trace_id"] = o.TraceId
	return toSerialize, nil
}

func (o *PageRunSummaryItemsInner) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"first_span_name",
		"models",
		"project_id",
		"release_ids",
		"span_count",
		"started_at",
		"status",
		"tenant_id",
		"trace_id",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varPageRunSummaryItemsInner := _PageRunSummaryItemsInner{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varPageRunSummaryItemsInner)

	if err != nil {
		return err
	}

	*o = PageRunSummaryItemsInner(varPageRunSummaryItemsInner)

	return err
}

type NullablePageRunSummaryItemsInner struct {
	value *PageRunSummaryItemsInner
	isSet bool
}

func (v NullablePageRunSummaryItemsInner) Get() *PageRunSummaryItemsInner {
	return v.value
}

func (v *NullablePageRunSummaryItemsInner) Set(val *PageRunSummaryItemsInner) {
	v.value = val
	v.isSet = true
}

func (v NullablePageRunSummaryItemsInner) IsSet() bool {
	return v.isSet
}

func (v *NullablePageRunSummaryItemsInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePageRunSummaryItemsInner(val *PageRunSummaryItemsInner) *NullablePageRunSummaryItemsInner {
	return &NullablePageRunSummaryItemsInner{value: val, isSet: true}
}

func (v NullablePageRunSummaryItemsInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePageRunSummaryItemsInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


