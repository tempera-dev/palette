# coding: utf-8

"""
    Beater API

    Agent observability, evaluation, gating, and human-review APIs for Beater

    The version of the OpenAPI document: 0.1.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from beater_client.models.gate_candidate_request import GateCandidateRequest

class TestGateCandidateRequest(unittest.TestCase):
    """GateCandidateRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> GateCandidateRequest:
        """Test GateCandidateRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `GateCandidateRequest`
        """
        model = GateCandidateRequest()
        if include_optional:
            return GateCandidateRequest(
                candidate = beater_client.models.gate_candidate_change_request.GateCandidateChangeRequest(
                    description = '', 
                    kind = '', 
                    proposed_by = '', 
                    rationale = '', 
                    target = '', ),
                gate_policy = beater_client.models.gate_policy.GatePolicy(
                    alpha = 1.337, 
                    comparison_count = 0, 
                    max_regression = 1.337, 
                    min_sample_size = 0, ),
                overfit_confidence = 1.337,
                overfit_resamples = 0,
                overfit_seed = 0,
                overfit_tolerance = 1.337,
                scores = [
                    beater_client.models.gate_case_score_request.GateCaseScoreRequest(
                        baseline_score = 1.337, 
                        candidate_score = 1.337, 
                        split = '', )
                    ]
            )
        else:
            return GateCandidateRequest(
                candidate = beater_client.models.gate_candidate_change_request.GateCandidateChangeRequest(
                    description = '', 
                    kind = '', 
                    proposed_by = '', 
                    rationale = '', 
                    target = '', ),
                scores = [
                    beater_client.models.gate_case_score_request.GateCaseScoreRequest(
                        baseline_score = 1.337, 
                        candidate_score = 1.337, 
                        split = '', )
                    ],
        )
        """

    def testGateCandidateRequest(self):
        """Test GateCandidateRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
