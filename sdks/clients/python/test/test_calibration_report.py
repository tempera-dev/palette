# coding: utf-8

"""
    Palette API

    Agent observability, evaluation, gating, and human-review APIs for Palette

    The version of the OpenAPI document: 0.1.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from palette_client.models.calibration_report import CalibrationReport

class TestCalibrationReport(unittest.TestCase):
    """CalibrationReport unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CalibrationReport:
        """Test CalibrationReport
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CalibrationReport`
        """
        model = CalibrationReport()
        if include_optional:
            return CalibrationReport(
                brier_score = 1.337,
                calibration_report_id = '',
                cohen_kappa = 1.337,
                cohen_kappa_ci_high = 1.337,
                cohen_kappa_ci_low = 1.337,
                confusion = palette_client.models.calibration_confusion.CalibrationConfusion(
                    human_fail_judge_fail = 0, 
                    human_fail_judge_pass = 0, 
                    human_pass_judge_fail = 0, 
                    human_pass_judge_pass = 0, ),
                created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                dataset_id = '',
                dataset_version_id = '',
                eval_report_id = '',
                evaluator_version_id = '',
                expected_agreement = 1.337,
                expected_calibration_error = 1.337,
                items = [
                    palette_client.models.calibration_item.CalibrationItem(
                        agreed = True, 
                        dataset_case_id = '', 
                        evidence = null, 
                        human_label = 'pass', 
                        judge_label = 'pass', 
                        judge_result_label = '', 
                        judge_score = 1.337, )
                    ],
                observed_agreement = 1.337,
                observed_agreement_ci_high = 1.337,
                observed_agreement_ci_low = 1.337,
                policy = palette_client.models.calibration_policy.CalibrationPolicy(
                    pass_threshold = 1.337, ),
                project_id = '',
                reliability_bins = [
                    palette_client.models.reliability_bin.ReliabilityBin(
                        accuracy = 1.337, 
                        bin_index = 0, 
                        calibration_gap = 1.337, 
                        lower_bound = 1.337, 
                        mean_confidence = 1.337, 
                        sample_count = 0, 
                        upper_bound = 1.337, )
                    ],
                sample_count = 0,
                tenant_id = ''
            )
        else:
            return CalibrationReport(
                brier_score = 1.337,
                calibration_report_id = '',
                cohen_kappa = 1.337,
                confusion = palette_client.models.calibration_confusion.CalibrationConfusion(
                    human_fail_judge_fail = 0, 
                    human_fail_judge_pass = 0, 
                    human_pass_judge_fail = 0, 
                    human_pass_judge_pass = 0, ),
                created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                dataset_id = '',
                dataset_version_id = '',
                eval_report_id = '',
                evaluator_version_id = '',
                expected_agreement = 1.337,
                expected_calibration_error = 1.337,
                items = [
                    palette_client.models.calibration_item.CalibrationItem(
                        agreed = True, 
                        dataset_case_id = '', 
                        evidence = null, 
                        human_label = 'pass', 
                        judge_label = 'pass', 
                        judge_result_label = '', 
                        judge_score = 1.337, )
                    ],
                observed_agreement = 1.337,
                policy = palette_client.models.calibration_policy.CalibrationPolicy(
                    pass_threshold = 1.337, ),
                project_id = '',
                reliability_bins = [
                    palette_client.models.reliability_bin.ReliabilityBin(
                        accuracy = 1.337, 
                        bin_index = 0, 
                        calibration_gap = 1.337, 
                        lower_bound = 1.337, 
                        mean_confidence = 1.337, 
                        sample_count = 0, 
                        upper_bound = 1.337, )
                    ],
                sample_count = 0,
                tenant_id = '',
        )
        """

    def testCalibrationReport(self):
        """Test CalibrationReport"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
