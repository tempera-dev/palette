...
/// struct for passing parameters to the method [`gate_optimization_candidate`]
#[derive(Clone, Debug)]
pub struct GateOptimizationCandidateParams {
    /// tenant_id
    pub tenant_id: String,
    /// project_id
    pub project_id: String,
    pub gate_candidate_request: models::GateCandidateRequest,
...
/// struct for typed errors of method [`gate_optimization_candidate`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GateOptimizationCandidateError {
...
pub async fn gate_optimization_candidate(configuration: &configuration::Configuration, params: GateOptimizationCandidateParams) -> Result<models::GateCandidateResponse, Error<GateOptimizationCandidateError>> {

    let uri_str = format!("{}/v1/rsi/{tenant_id}/{project_id}/gate-candidate", configuration.base_path, tenant_id=crate::apis::urlencode(params.tenant_id), project_id=crate::apis::urlencode(params.project_id));
    ...
    req_builder = req_builder.json(&params.gate_candidate_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GateOptimizationCandidateError> = serde_json::from_str(&content).ok();
