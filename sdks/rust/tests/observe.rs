//! Offline tests: verify `observe`/`span` emit spans with the correct Palette
//! attributes, using the OTel in-memory exporter so no live paletted is needed.
...
use palette::{attr, span_kind, PaletteConfig, SPAN_KINDS};
...
fn setup() -> (MutexGuard<'static, ()>, InMemorySpanExporter) {
    ...
    // palette::init builds an OTLP exporter and sets the global provider, and also
...
fn observe_records_kind_release_and_output() {
    ...
    let result = palette::observe("call", span_kind::LLM_CALL, || {
    ...
    assert!(attr_str(span, attr::OUTPUT_VALUE)
        ...
        .unwrap_or(false));
    // palette.seq is set (>= 1).
    let seq = span
        .attributes
        .iter()
        .find(|kv| kv.key.as_str() == attr::SEQ)
        .map(|kv| kv.value.clone());
    assert!(seq.is_some(), "palette.seq should be set");
}
...
async fn observe_async_records_kind() {
    ...
    let n = palette::observe_async("arun", span_kind::AGENT_RUN, async { 21 * 2 }).await;
    assert_eq!(n, 42);
