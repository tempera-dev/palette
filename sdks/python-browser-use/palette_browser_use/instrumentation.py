...
``browser.*`` spans over OTLP into Palette.

The public surface is small and defensive:

- :class:`PaletteBrowserUseTracer` owns an OpenTelemetry ``TracerProvider`` and
...
ENV_ENDPOINT = "PALETTE_OTLP_ENDPOINT"
...
class PaletteBrowserUseTracer:
    """Owns an OTLP-exporting ``TracerProvider`` and the span mapping.

    Parameters
    ----------
    endpoint:
        OTLP/gRPC endpoint. Defaults to ``$PALETTE_OTLP_ENDPOINT`` or
        ``localhost:4317``.
    service_name:
        ``service.name`` resource attribute.
    span_processor:
        Inject a custom ``SpanProcessor`` (e.g. ``SimpleSpanProcessor`` wrapping
        an ``InMemorySpanExporter``) — used by the unit tests so no live Palette
    ...
    def __init__(
        ...
        self._tracer = self._provider.get_tracer("palette-browser-use")
        self._seq = 0
...
def make_hooks(
    tracer: Optional[PaletteBrowserUseTracer] = None,
    endpoint: Optional[str] = None,
    **tracer_kwargs: Any,
    ...
    """
    if tracer is None:
        tracer = PaletteBrowserUseTracer(endpoint=endpoint, **tracer_kwargs)
...
class _Instrumentation:
    ...
    tracer: PaletteBrowserUseTracer
...
def instrument(
    ...
    **tracer_kwargs: Any,
    ...
    Builds (or reuses) a :class:`PaletteBrowserUseTracer` and returns the run
...
def _make_register_callback(tracer: PaletteBrowserUseTracer, agent: Any):
