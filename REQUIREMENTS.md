...
| R18.3 | An OpenAI-compatible LLM gateway proxies app traffic with caching + failover, natively traced and online-scorable. | `beater-gateway` `POST /v1/gateway/chat/completions`; request-hash cache-hit test; BYOK failover/load-balance test; per-tenant budget via `QuotaLimiter`; auto-trace + online-score integration test proving zero-SDK instrumentation |
