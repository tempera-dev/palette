...
| R0.6 | CI can be blocked on regression gates. | Persisted `GateStore` tests, `/v1/gates` API test, `palettectl gate-run` nonzero exit test, deterministic and judge-backed experiment fixture tests, and example GitHub Actions workflow |
...
| ID | Requirement | Evidence required |
| --- | --- | --- |
| R1.1 | MVP ships as one all-in-one `paletted` binary. | Cargo workspace with `bins/paletted`; compose uses one Palette process by default; root Dockerfile builds `paletted` with multi-stage cargo-chef dependency caching |
...
| R1.3 | OSS runs without Palette Cloud. | Offline compose test with network calls disabled except configured providers |
| R1.4 | Vercel is used only for stateless/control-plane work. | `web/dashboard/vercel.json` deploys the stateless Next.js UI; stateful workers run outside Vercel or in `paletted`; Rust function config still required for hosted control plane |

## R2. Data Model and Schema Evolution

| ID | Requirement | Evidence required |
| --- | --- | --- |
| R2.1 | Canonical entities exist: Run, Span, Event, Artifact, DatasetVersion, Experiment, EvaluatorVersion, EvalResult, Gate, GateRun, ReviewQueue, ReviewTask, Annotation, CalibrationReport, UsageRecord, AuditEvent. | Schema definitions plus `migrations/sqlite`, `migrations/postgres`, and `migrations/clickhouse` schema contracts; `paletted` executes the SQLite contract through a checksummed migration ledger before opening local stores; Postgres/ClickHouse runtime backends still required |
...
| R2.4 | Old canonical spans can be migrated to a newer canonical schema version after schema changes. | Migration/replay test (`palette-replay` `reproject`) that reprojects an old canonical fixture into a new canonical version by reclassifying its attribute bag. NOTE: this is a canonical-attribute reclassification of an existing span, not yet a full re-normalization from the preserved raw bytes; raw bytes remain preserved (R2.2) for a future bytes-driven re-normalizer |
...
| ID | Requirement | Evidence required |
| --- | --- | --- |
| R3.1 | Product code depends on a `TraceStore` trait. | `palette-store` is trait/error/fake only; concrete SQLite/filesystem implementations live in `palette-store-sql` and `palette-store-obj`; `cargo tree -p palette-store` has no `rusqlite` |
| R3.2 | Local backend exists before ClickHouse assumptions leak. | `palette-store-sql` TraceStore conformance suite runs identical cases against SQLite and `InMemoryTraceStore`; `MetadataStore` conformance suite runs identical org/project/environment/RBAC cases against SQLite and `InMemoryMetadataStore`; `QuotaLimiter` conformance suite runs identical fixed-window cases against SQLite and `InMemoryQuotaLimiter`; filesystem artifact store has its own round-trip/hash test |
...
| R3.7 | Foundational time and money primitives are replay-safe. | `palette-core` exposes `Clock`, `SystemClock`, `FixedClock`, typed `Currency`, and checked `Money::try_add`/`try_sub`; `rg "Utc::now()" crates/palette-core crates/palette-schema` returns no matches |
| R3.8 | Canonical schema owns span/status mapping and run rollups. | `AgentSpanKind::parse`, `SpanStatus::{as_str,parse}`, `span_matches`, `span_summary`, and `roll_up_runs` live in `palette-schema`; storage/search/archive/API callers delegate to schema helpers |
...
| R4.2 | Durable buffer exists. | SQLite durable bus reopen/dedupe tests, `paletted` SQLite bus default, buffered trace-write queue; NATS JetStream and Vercel Queues adapters still required for scale/hosted GA |
| R4.3 | DLQ captures invalid or repeatedly failed events. | Bus DLQ/replay tests, ack/inflight retry tests, trace-write and trace-ingested worker invalid payload DLQ tests, DLQ replay API full-stack test, `palettectl bus-fixture` |
...
| R4.10 | ClickHouse or TraceStore outage does not silently drop accepted events. | `buffer_native` outage/recovery unit test, downstream-publish-after-store-write recovery test, SQLite bus unacked-inflight recovery test, trace-ingested restart→DLQ→replay recovery test, buffered ingest + scoped drain full-stack API test, DLQ replay API full-stack test, live `paletted` trace-write kill/restart/DLQ/replay smoke test over accepted buffered OTLP work, live `paletted` external TraceStore endpoint-unavailable→DLQ→replay smoke test proving the accepted event is not canonically written until replay, live `paletted` trace-ingested consumer-kill/restart/DLQ/replay smoke test, live `paletted` storage-failure accounting smoke test over explicit error + DLQ + recovered events with `lost=0`, `paletted` background trace-write drain worker, `paletted` background trace-ingested downstream worker, `palettectl smoke` OTLP trace-write/downstream drain test, live `paletted` OTLP HTTP/gRPC smoke test, and `palettectl ingest-outage-fixture` accounting over explicit error + DLQ + recovery with `lost=0` |
...
| R5.2 | LLM/embedding judges run through judge broker, never inside WASM. | `palette-judge` broker tests, HTTP provider mock-server tests, dataset and experiment judge eval tests, plus hosted API test for BYOK refs, cache hits, budget metering, and redacted ledger output |
...
| R5.6 | Judge spend is metered and budgeted. | `palette-usage` idempotent ledger tests, `/v1/usage` full-stack tests for direct/dataset/experiment judge spend, `palettectl usage-fixture`, and judge budget-failure tests |
...
| R6.5 | Judge calibration stores agreement with human labels. | Persisted calibration report schema, Cohen's kappa unit test, `/v1/calibrations` full-stack test, and `palettectl calibration-fixture` over judge-backed eval results |
...
| R8.3 | PII unmask is a separate audited scope. | `/v1/traces` full-stack test for default redaction, denied `pii_unmask`, allowed scoped unmask, and `/v1/audit` readback; `palette-audit` SQLite store test; `palettectl audit-fixture` |
...
| R11.2 | Zero-SDK OTLP onboarding works. | Live `paletted` OTLP HTTP/gRPC smoke test uses stock OpenTelemetry protobuf service types; OpenInference/OpenLLMetry/OTel fixture apps still required |
...
| ID | Requirement | Evidence required |
| --- | --- | --- |
| R13.1 | Palette emits its own traces, metrics, and logs. | Dogfood config and dashboards |
...
Before marking Palette "shipped", audit every requirement against direct
...
Palette's existing rigor**: a surface ships only with the held-out / calibrated /
multiplicity-corrected discipline (R5/R6) that makes its answer correct, not merely
present. Status today: all **[planned]**.

| ID | Requirement | Evidence required |
| --- | --- | --- |
| R18.1 | Runtime guardrails enforce (block/redact/allow/flag) on input and output. | `palette-guardrails` pre/post-hook tests (prompt-injection/jailbreak, PII/PHI, toxicity, topic, custom WASI); `p95 < 200ms` bench; each verdict emits an observable `guardrail.check` span; blocked attempt auto-promotes to a red-team dataset; `POST /v1/guardrails/check` full-stack test |
...
| R18.3 | An OpenAI-compatible LLM gateway proxies app traffic with caching + failover, natively traced and online-scorable; **any model, BYOK optional**. | `palette-gateway` `POST /v1/gateway/chat/completions`; **any-model routing test**; **BYOK-optional test** (managed-default fallback so no key is required on hosted; typed `NoKey` error in OSS); request-hash cache-hit test; multi-key failover test; per-tenant budget via `QuotaLimiter` + timeout; provider-key **redaction** test (never logged/returned); auto-trace (`llm.call` span) + online-score integration test proving zero-SDK instrumentation |
| R18.4 | Failing traces auto-cluster into named issues with a counterfactual root cause. | `palette-insights` clustering test; auto-name judge test; each `FailureIssue` carries a counterfactual earliest-flip root-cause span (R7.4) + sampling-weighted frequency; `GET /v1/insights/clusters\|issues`; one-click promote/guardrail/propose |
...
| R18.11 | BYOC data-plane / control-plane split with Terraform modules. | customer-hosted data plane (all payloads) vs Palette-hosted control plane (metadata only) topology; Terraform AWS/GCP/Azure modules; documented residency boundary (extends R8.2/R15) |
