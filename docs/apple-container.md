# Running Palette on Apple `container` or Docker

Palette builds and runs on either [Docker](https://www.docker.com/) or Apple's
...
```bash
export PALETTE_CONTAINER_RUNTIME=container   # or: docker
```

## Build

```bash
scripts/build-image.sh paletted:local
```
...
container its own IP, so `run-paletted.sh` resolves the address via
...
| Compose | `docker-compose.yml` supported | no compose — use `scripts/run-paletted.sh` |
...
- Multi-container topologies that use `docker-compose.yml` today run as before on
  ...
  `paletted` service is covered by `run-paletted.sh`). Compose-equivalent
