...
Palette can broker **250+ third-party tools** (GitHub, Gmail, Slack, web search,
...
```
crates/palette-composio          → typed Composio v3 client (ComposioClient trait)
crates/palette-api  /v1/connectors → 6 handlers (#[utoipa::path])
   → cargo xtask regen-spec      → OpenAPI + dashboard snapshot
   → scripts/regen-sdks.sh       → 7 SDK clients
   → palette-mcp build_tools()    → 6 MCP tools (auto, no hand-mirror)
```

Because the MCP catalog is derived from the `/v1` surface, an RSI agent session
calls Composio tools through **the same MCP path it already uses** for Palette's
own tools — no special-casing.

## Endpoints

All are tenant/project scoped. The Composio "entity" (which owns connections)
is keyed per project: `palette:{tenant_id}:{project_id}`.
...
This complements `palette-oauth-server` (which authorizes callers *into* Palette)
and the BYOK `provider-secrets` pattern — it does not replace either.

## Enabling

Set `COMPOSIO_API_KEY` in the `paletted` environment. Unset, the `/v1/connectors`
...
`eval:run` is only the coarse Palette scope for requesting a connector tool.
Before Palette calls Composio, it fetches the tool metadata, classifies the tool
risk, and applies Palette's connector policy. Read-only tools are allowed by
...
```bash
PALETTE_CONNECTOR_ALLOW_TOOLS=GITHUB_CREATE_AN_ISSUE,SLACK_SEND_MESSAGE
PALETTE_CONNECTOR_DENY_TOOLS=GITHUB_DELETE_REPOSITORY
```
...
2. **The `tool_set` lever.** `palette_composio::skill::tool_definition_json()`
...
snapshots the live catalog into `crates/palette-composio/catalog/toolkits.json`
...
- `cargo test -p palette-composio` — client mapping, skill scaffold, RSI entry.
  Live wire tests run only when `COMPOSIO_API_KEY` is set (else they skip).
- `cargo test -p palette-api --test connectors` — full router→handler→trait
