...
Composio continuously adds and updates apps and tools. Beater carries **no
local catalog snapshot** — `listConnectors` / `listConnectorTools` query the
live Composio catalog on every call, so new apps and tools surface immediately
without a Beater release. The trade-off is deliberate: catalog freshness comes
from the provider, while the execution decision stays with Beater (the
connector policy classifies each tool's risk at invoke time, so a
newly-published high-risk tool is deny-by-default until allowlisted).
