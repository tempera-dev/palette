...
def test_gate2_image_helpers_keep_public_ghcr_contract() -> None:
    ...
    for image_name in module.GATE2_IMAGE_NAMES:
        ...
        assert module.gate2_image_repo(image_name) == f"ghcr.io/tempera-dev/palette/{image_name}"
        assert (
            module.gate2_registry_repository(image_name)
            == f"tempera-dev/palette/{image_name}"
        )
        assert (
            module.gate2_image_ref(image_name, "abc123")
            == f"ghcr.io/tempera-dev/palette/{image_name}:abc123"
        )
        assert (
            module.gate2_image_digest_prefix(image_name)
            == f"ghcr.io/tempera-dev/palette/{image_name}@sha256:"
        )
...
def test_raw_public_preflight_command_pins_expected_commit() -> None:
    ...
    assert (
        "https://raw.githubusercontent.com/tempera-dev/palette/0123456789abcdef/"
...
def test_immutable_log_url_requires_actions_run_or_job() -> None:
    ...
    assert module.is_immutable_log_url("https://github.com/tempera-dev/palette/actions/runs/123")
    assert module.is_immutable_log_url(
        "https://github.com/tempera-dev/palette/actions/runs/123/job/456"
    )
    assert not module.is_immutable_log_url(
        "https://github.com/tempera-dev/palette/actions/workflows/gate2-proof-contract.yml"
    )
    assert not module.is_immutable_log_url("https://github.com/tempera-dev/palette/pull/123")
