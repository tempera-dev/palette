...
//!
//! The RSI loop (`palette-experiments`) only *proposes* changes; a candidate is
...
use palette_core::{DatasetCaseId, JsonHashError, Sha256Hash, sha256_json_hash};
...
/// `palette-stats`), and `Test` is the held-out gate set — ideally queried once,
/// at acceptance time.
