...
use palette_eval::{EvaluationCase, EvaluatorSpec, ScoreResult, evaluate_deterministic};
...
fn eval_result_from_score(
    ...
    Ok(EvalResult {
        ...
        reproducibility: EvalReproducibility {
            ...
            wasi_abi_version: Some("palette-deterministic-v1".to_string()),
            judge_model_id: None,
            judge_provider: None,
...
fn select_span<'a>(
    ...
    spans
        .iter()
        .find(|span| matches!(span.status, palette_schema::SpanStatus::Error))
        .or_else(|| spans.first())
...
mod tests {
    use super::*;
    use palette_core::{ArtifactId, JudgeCallId, Money, TokenCounts};
    use palette_eval::EvaluatorKind;
    ...
    async fn promotes_trace_case_versions_and_runs_deterministic_eval() {
        ...
        assert_eq!(result.reproducibility.normalizer_version, "palette-test-v1");
    ...
    fn fixture_span(tenant: &TenantId, project: &ProjectId) -> CanonicalSpan {
        ...
        CanonicalSpan {
            ...
            normalizer_version: "palette-test-v1".to_string(),
            tenant_id: tenant.clone(),
