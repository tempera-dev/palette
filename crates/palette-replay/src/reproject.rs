...
use palette_schema::{CanonicalSpan, RawEnvelope};
use serde_json::{Map, Value};
use std::collections::BTreeMap;

/// Second canonical schema version. Defined locally (not in `palette-schema`) so
...
pub const REPROJECTION_NORMALIZER_VERSION: &str = "palette-reproject-v2";
...
/// `palette_ingest::canonical_mapping::CANONICAL_PREFIXES` (the duplication is
...
/// identical to ingest's `palette_ingest::canonical_mapping::CANONICAL_EXACT_KEYS`
...
mod tests {
    ...
    /// ingest (`palette_ingest::canonical_mapping`) with no shared dependency.
    ...
    fn canonical_prefixes_match_ingest() {
        use palette_ingest::canonical_mapping;
        assert_eq!(
            CANONICAL_PREFIXES,
