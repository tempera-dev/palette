...
impl ComposioClient for FakeComposio {
    ...
    async fn get_tool(&self, tool_slug: &str) -> Result<ConnectorTool, ComposioError> {
        ...
        // GITHUB_DELETE_REPOSITORY deliberately answers with GITHUB_GET_REPOSITORY
        // metadata so the slug-mismatch defense in the invoke handler is exercised.
        if matches!(
            tool_slug,
            "GITHUB_GET_REPOSITORY" | "GITHUB_DELETE_REPOSITORY"
        ) {
