...
//! third-party apps (Gmail, GitHub, Slack, web search, …). Palette wraps it so
//! that an agent under evaluation — or the RSI loop's `tool_set` lever — can
//! reach those tools through Palette's own `/v1/connectors` surface (and, by
...
//!
//! * A Composio **entity** (`user_id`) owns connections. Palette keys it per
//!   project: `palette:{tenant_id}:{project_id}`.
...
/// Environment variable Palette reads to enable the Composio integration. When
...
/// Async interface over the Composio operations Palette needs. Implemented by
...
mod tests {
    ...
    //
    // Run locally with:  COMPOSIO_API_KEY=ak_... cargo test -p palette-composio -- --nocapture
    ...
    async fn live_connect_returns_login_url() {
        ...
        let link = client
            .connect("github", "palette:test:ci")
            .await
            .expect("connect");
