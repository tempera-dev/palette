...
//!   a *when to use* hint, the argument contract, and the exact Palette MCP call
...
pub fn skills_doc(tools: &[ConnectorTool]) -> String {
    let mut out = String::from(
        "# Connector tools (Composio)\n\nYou can call any tool below through the \
         Palette MCP tool `invokeConnectorTool` (or `POST /v1/connectors/{tenant}/{project}/invoke`). \
