//! palette-receipts — agent action receipts for transactions, external writes,
//! and user-visible changes (GitHub issue #269).
//!
//! Receipts are stored in an append-only SHA-256 hash chain, mirroring the
//! pattern in `palette-audit`: each record carries the `prev_hash` of the record
...
use palette_schema::RedactionClass;
...
pub enum ChainError {
    ...
    Hash(#[from] palette_core::JsonHashError),
