...
id_type!(UserId);
id_type!(SessionId);
id_type!(OAuthClientId);
id_type!(AuthCodeId);
id_type!(AccessTokenId);
id_type!(RefreshTokenId);
id_type!(TokenFamilyId);
id_type!(ProjectId);
...
///
/// Palette is opt-out: a self-hosted `paletted` reports **no** anonymous usage
/// telemetry to Palette Cloud unless the operator explicitly opts in. This type
...
/// `PALETTE_SELF_HOST_TELEMETRY`); until then no outbound endpoint is exposed, so
...
impl SelfHostTelemetryConfig {
    ...
    pub const ENV_VAR: &'static str = "PALETTE_SELF_HOST_TELEMETRY";
    ...
    pub const ENDPOINT: &'static str = "https://telemetry.palette.dev/v1/usage";
