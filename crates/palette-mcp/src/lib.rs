//! Model Context Protocol (MCP) server for the Palette API.
//!
...
//! auth (`Authorization: Bearer ...` / `x-palette-api-key` plus strict-auth
...
//! `Authorization`, `x-palette-api-key`, and strict-auth scope headers are
...
fn build_tools() -> Vec<ToolSpec> {
    ...
    for op in palette_api::openapi::operations(&doc) {
...
/// should treat them as having side effects beyond Palette's own store (cost, rate
...
fn required_api_scope(operation_id: &str, method: &str) -> &'static str {
    match operation_id {
        ...
        | "gates.create"
        | "gates.run"
        | "connectors.invokeTool" => "eval:run",

        "alerts.evaluate" | "online.decideSampling" => "trace:read",
        ...
        | "archive.archiveTrace" => "trace:write",
...
fn help_tool_json(advertise_oauth: bool) -> Value {
    ...
    let mut tool = json!({
        "name": HELP_TOOL_NAME,
        "description": "Discover Palette MCP tools. Call with no arguments for an \
        ...
        "annotations": annotations_for("GET", "help", "Discover Palette MCP tools"),
    });
...
///
/// Merge this into the main app, e.g. `palette_api::router(state).merge(
/// palette_mcp::router(state))`, so `/mcp` is served alongside the HTTP API and
...
async fn handle_mcp_get(State(state): State<ApiState>) -> Response {
    if state.oauth_metadata_url().is_some() {
        return oauth_unauthorized_response(
            ...
            "Authenticate before connecting to Palette MCP.".to_string(),
        );
    }
    (
...
fn server_info() -> Value {
    json!({ "name": "palette-mcp", "version": env!("CARGO_PKG_VERSION") })
}
...
async fn call_tool(
    ...
    for header_name in [http::header::AUTHORIZATION.as_str(), "x-palette-api-key"] {
        if let Some(value) = headers.get(header_name) {
            builder = builder.header(header_name, value);
        }
    }
    // Most MCP clients cannot reasonably know Palette's auth-context headers.
...
fn oauth_challenge(resource_metadata_url: &str) -> String {
    format!(
        "Bearer resource_metadata=\"{resource_metadata_url}\", error=\"insufficient_scope\", error_description=\"Authorize Palette MCP access to continue\""
    )
}

fn has_presented_credentials(headers: &HeaderMap) -> bool {
    headers.contains_key(http::header::AUTHORIZATION) || headers.contains_key("x-palette-api-key")
}
...
mod tests {
    ...
    fn initialize_result_advertises_client_safe_instructions() {
        ...
        assert_eq!(result["serverInfo"]["name"], "palette-mcp");
    ...
    fn tool_to_json_advertises_mcp_oauth_scopes_when_enabled() {
        let tool = tools()
            .iter()
            .find(|tool| tool.name == "spans.get")
            .expect("spans.get tool present");
        let without = tool_to_json(tool, false);
        assert!(
    ...
    fn oauth_scope_map_matches_non_obvious_api_scopes() {
        let cases = [
            ("providerSecrets.list", "GET", "admin"),
            ...
            ("alerts.evaluate", "POST", "trace:read"),
        ];
