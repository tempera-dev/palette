...
mod tests {
    ...
    fn experiment_usage_keeps_zero_cost_cached_calls_for_audit() -> anyhow::Result<()> {
        let report = ExperimentRunReport {
            experiment_run_id: palette_core::ExperimentRunId::new("run-1")?,
            tenant_id: TenantId::new("tenant")?,
            project_id: ProjectId::new("project")?,
            ...
            case_scores: vec![CaseExperimentScore {
                ...
                baseline_judge_call_id: Some(palette_core::JudgeCallId::new("judge-call-a")?),
                candidate_judge_call_id: Some(palette_core::JudgeCallId::new("judge-call-b")?),
                baseline_cached: Some(false),
                candidate_cached: Some(true),
            }],
            comparison: palette_eval::ExperimentComparison {
                sample_size: 1,
                baseline_mean: 0.0,
                candidate_mean: 1.0,
                delta: 1.0,
                ...
                ci_high: 1.0,
                p_value: 1.0,
                decision: palette_eval::GateDecision::Pass,
            ...
            decision: palette_eval::GateDecision::Pass,
            gate_policy: palette_eval::GatePolicy::default(),
            created_at: Utc::now(),
        };
    ...
    async fn backfills_rollup_for_pre_existing_ledger_on_open() -> anyhow::Result<()> {
        ...
        path.push(format!("palette-usage-backfill-{}.sqlite", Uuid::new_v4()));
