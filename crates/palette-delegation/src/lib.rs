//! palette-delegation — A2A and inter-agent delegation traces with trust
...
//! crate cannot edit `palette-schema`, the span kind and attribute keys live here
...
///
/// A production deployment would promote this to a `palette-schema` span kind;
...
///
/// `palette_schema::RedactionClass` derives only `Eq`/`PartialEq` (no `Ord`), so
/// it cannot live directly in a `BTreeSet`. This crate cannot edit
/// `palette-schema`, so `DataClass` adds a total order (by sensitivity rank) and
...
mod tests {
    use super::*;
    use palette_core::{ArtifactId, Sha256Hash};
