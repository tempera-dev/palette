...
fn tool_set_equals_spec_v1_operations() {
    ...
    // Sanity: the spec covers 47 /v1 operations.
...
async fn initialize_and_tools_list_over_mcp_route() {
    ...
    // 47 spec-derived tools + the synthetic `help` meta tool.
...
async fn tools_list_exposes_output_schema_and_annotations() {
    ...
    // These list endpoints return top-level JSON arrays, which MCP forbids as
...
async fn help_overview_lists_every_spec_tool() {
    ...
    assert_eq!(
        structured["toolCount"], 47,
        "overview covers all 47 spec tools"
    );
