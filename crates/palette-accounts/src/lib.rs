...
//! `(tenant, project, environment)`-scoped API keys in `palette-security` /
//! `palette-auth`. A person registers with an email + password, signs in to get
...
//!   `palette-security`) — slow on purpose, salted per password.
...
fn dummy_password_hash() -> &'static str {
    ...
    DUMMY
        .get_or_init(|| {
            hash_password("palette-accounts timing-equalization dummy").unwrap_or_default()
        })
        .as_str()
}
