...
use palette_experiments::{ExperimentRunReport, SqliteExperimentStore};
use palette_gates::{GateDefinition, GateRunReport, SqliteGateStore};
use palette_human::{
...
use palette_replay::{ReplayMode, plan_replay};
use palette_sandbox::WasmEvaluatorRuntime;
use palette_schema::{
...
use palette_search::{SearchIndex, SearchRequest, SearchResponse, TantivySearchIndex};
use palette_secrets::{EncryptedSqliteProviderSecretStore, SecretKeyring};
use palette_security::{ApiScope, api_key_id_from_secret, verify_webhook};
use palette_store::{
...
async fn api_ingest_store_eval_gate_and_replay_are_integrated() {
    ...
    verify_webhook(
        ...
        delivery
            ...
            .unwrap_or_else(|| panic!("missing palette-signature")),
        now,
...
async fn buffered_ingest_drains_scoped_trace_writes_through_api() {
    ...
    let body = to_bytes(response.into_body(), 1024 * 1024)
        .await
        .unwrap_or_else(|err| panic!("{err}"));
    let outcome: palette_ingest::IngestOutcome =
        serde_json::from_slice(&body).unwrap_or_else(|err| panic!("{err}"));
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("authorization", format!("Bearer {trace_secret}"))
                .header("x-palette-project-id", "project")
                .header("x-palette-environment-id", "prod")
                .body(Body::empty())
                .unwrap_or_else(|err| panic!("{err}")),
        )
            ...
            "scopeSpans": [{
                ...
                "spans": [{
                    ...
                    "attributes": [
                        ...
                        {"key": "palette.run_id", "value": {"stringValue": "strict-auth-json"}}
                    ],
                    "events": [
                        {
                            "name": "palette.input",
