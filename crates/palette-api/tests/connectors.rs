...
//! Palette wiring (router → auth → handler → connector trait → response) with no
//! network. The live wire contract is covered by `palette-composio`'s
...
async fn connect_returns_login_link_with_project_entity() {
    ...
    assert!(
        fake.calls()
            .iter()
            .any(|c| c == "connect:github:palette:acme:proj")
    );
}
...
async fn invokes_tool_and_scopes_entity_per_project() {
    ...
    assert!(fake.calls().iter().any(|c| {
        c.starts_with("execute:GITHUB_CREATE_AN_ISSUE:palette:acme:proj:")
...
async fn invokes_read_only_tool_without_explicit_allow() {
    ...
    assert!(
        calls
            .iter()
            .any(|c| c.starts_with("execute:GITHUB_GET_REPOSITORY:palette:acme:proj:"))
    );
}
...
async fn eval_run_key_denies_high_risk_tool_without_allow_policy_and_audits() {
    ...
    let (status, _) = send_with_headers(
        ...
        &[
            ...
            ("x-palette-environment-id", "prod".to_string()),
        ],
    )
    .await;
