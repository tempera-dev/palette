//! `palette-bench` — criterion benchmarks and load-test fixtures for Palette.
//!
//! # Purpose
//!
...
//! SLOs in CI*.
//!
//! Nothing in this crate is wired into `paletted` or any runtime path.  It exists
...
//!   Metronome CI gate that runs `cargo bench -p palette-bench`.
...
const NORMALIZER_VERSION: &str = "palette-bench-fixture-v1";
...
fn raw_envelope_for(config: &SpanFixtureConfig, span: &CanonicalSpan) -> RawEnvelope {
    ...
    RawEnvelope {
        ...
        source_schema_url: Some("palette://bench/native/v1".to_string()),
