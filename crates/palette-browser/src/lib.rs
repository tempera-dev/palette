//! `palette-browser` — the contract for browser-driving agents in Palette.
//!
...
//!   (`palette-browser-{playwright,webdriver,cdp}`) implements.
...
//!   (mirrors `palette-store-conformance`), plus [`MockDriver`] so downstream
...
pub const FIXTURE_KNOWN_SELECTOR: &str = "#palette-known";
/// Selector deliberately absent from the conformance fixture page.
pub const FIXTURE_MISSING_SELECTOR: &str = "#palette-missing";
...
pub const CONFORMANCE_FIXTURE_HTML: &str = concat!(
    "<!doctype html><html><head><title>Palette Conformance Fixture</title></head>",
    "<body><button id=\"palette-known\">ok</button>",
    "<script>console.log(\"palette-fixture-ready\");</script>",
    "</body></html>",
);
...
pub const FIXTURE_CONSOLE_MESSAGE: &str = "palette-fixture-ready";
...
///
/// ```rust,no_run
/// use palette_browser::{BrowserDriver, BrowserEngine, MockDriver, UrlPolicy};
/// # async fn example() {
...
impl MockDriver {
    ...
    pub fn with_conformance_fixture() -> Self {
        ...
        Self {
            ...
            url: String::new(),
            title: Some("Palette Conformance Fixture".to_string()),
            selectors,
            policy: UrlPolicy::allow_all(),
        }
    }
