...
//! [`palette:scorer/deterministic-scorer`](../wit/scorer.wit) world. The world
...
use palette_eval::{EvalError, EvaluationCase, EvaluatorSpec, ScoreResult, evaluate_deterministic};
    ...
    pub fn evaluate_bytes(
        ...
        Ok(ScoreResult {
            ...
            evidence: serde_json::json!({
                ...
                "world": "palette:scorer/deterministic-scorer",
...
/// Bridge that runs a `palette-eval` deterministic scorer through the WASI
...
/// `palette-eval` are *actually executed* through the component sandbox rather
...
pub struct DeterministicScorerRun {
    ...
    /// Score produced by the native `palette_eval::evaluate_deterministic`.
...
impl WasmEvaluatorRuntime {
    /// Execute a deterministic evaluator from `palette-eval` two ways — once by
...
mod tests {
    use super::*;
    use palette_eval::EvaluationCase;
    use serde_json::json;
    ...
    fn component_model_evaluator_scores_case_json() {
        ...
        assert_eq!(
            ...
            json!("palette:scorer/deterministic-scorer")
        );
    }

    #[test]
    fn runs_palette_eval_deterministic_scorer_through_component_runtime() {
        use palette_eval::{EvaluatorKind, EvaluatorSpec};
        use palette_schema::EvaluatorLane;

        // A real deterministic evaluator defined in palette-eval.
        ...
        // palette-eval reference implementation.
        assert_eq!(
