...
mod tests {
    use super::*;
    use palette_eval::{ExperimentComparison, StatisticalTest};

    #[tokio::test]
    async fn gate_run_fails_regression_and_persists_report() -> anyhow::Result<()> {
        let gates = SqliteGateStore::in_memory()?;
        let experiments = palette_experiments::SqliteExperimentStore::in_memory()?;
    ...
    fn experiment_report(
        ...
        Ok(ExperimentRunReport {
            ...
            project_id,
            dataset_id,
            dataset_version_id: palette_core::DatasetVersionId::new("version")?,
