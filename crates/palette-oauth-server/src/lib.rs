...
//!
//! A self-contained axum [`Router`] (the same pattern as `palette-mcp`) that
//! exposes the OAuth endpoints and is merged into `paletted` alongside the API
...
pub const SESSION_COOKIE: &str = "palette_session";
...
/// Build the OAuth HTTP router. Mirrors `palette_mcp::router`: self-contained,
...
async fn register(
    ...
    let registration = ClientRegistration {
        client_name: req
            .client_name
            .unwrap_or_else(|| "palette-client".to_string()),
...
async fn authorize(
    ...
    let client_id = match palette_core::OAuthClientId::new(params.client_id.clone()) {
        Ok(id) => id,
        Err(_) => {
            return oauth_error(
                StatusCode::BAD_REQUEST,
...
async fn token(
    ...
    let client_id = match palette_core::OAuthClientId::new(client_id_str) {
        Ok(id) => id,
...
<head>
  ...
  <title>Allow access to Palette</title>
  <style>
...
fn scope_label(scope: &str) -> String {
    match scope {
        "mcp:invoke" => "Use the Palette MCP tools".to_string(),
...
mod tests {
    ...
    fn test_state() -> OAuthServerState {
        OAuthServerState {
            ...
            api_keys: Some(Arc::new(ok(palette_auth::SqliteApiKeyStore::in_memory()))),
    ...
    async fn authorize_rejects_unsafe_registered_redirect_uri_without_redirecting() {
        let state = test_state();
        let now = Utc::now();
        let client_id = ok(palette_core::OAuthClientId::new("unsafe-client"));
        ok(state
            .oauth
    ...
    async fn authorize_rejects_client_with_unsupported_scope() {
        ...
        ok(state
            .accounts
            .put_membership(palette_accounts::OrgMembership {
                organization_id: ok(OrganizationId::new("demo")),
                user_id: user.user_id.clone(),
                role: palette_accounts::OrgRole::Member,
                created_at: now,
            })
            .await);
    ...
    async fn authorize_admin_scope_consent_marks_elevated_permission() {
        ...
        ok(state
            ...
            .put_membership(palette_accounts::OrgMembership {
                organization_id: ok(OrganizationId::new("demo")),
                user_id: user.user_id.clone(),
                role: palette_accounts::OrgRole::Admin,
                created_at: now,
            })
            .await);
        let registered = ok(state
    ...
    async fn authorize_omitted_scope_narrows_broad_client_to_member_safe_default() {
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp).await;
        assert!(body.contains("Use the Palette MCP tools"));
    ...
    async fn authorize_without_scope_ids_shows_human_consent_then_issues_code() {
        ...
        ok(state
            ...
            .put_membership(palette_accounts::OrgMembership {
                ...
                user_id: user.user_id.clone(),
                role: palette_accounts::OrgRole::Owner,
                created_at: now,
            })
            .await);
        let session = ok(state
        ...
        let body = body_text(resp).await;
        assert!(body.contains("Allow Cursor to use Palette?"));
        assert!(body.contains("Use the Palette MCP tools"));
        assert!(body.contains("Read traces and evaluation data"));
