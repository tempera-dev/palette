...
//!
//! [`BrowserToolProxy`] wraps any [`palette_browser::BrowserDriver`] and turns
//! each step into Palette's canonical record:
//!
...
use palette_replay::{ReplayEvent, ReplayEventKind, SqliteReplayStore};
use palette_schema::{
...
pub enum CaptureError {
    ...
    Driver(#[from] palette_browser::BrowserError),
...
pub struct BrowserToolProxy<D, A> {
    ...
    ctx: CaptureContext,
    engine: palette_browser::BrowserEngine,
    ...
    A: ArtifactStore,
{
    ...
    fn build_span(&mut self, parts: SpanParts) -> Result<CanonicalSpan, CaptureError> {
        ...
        self.span_seq += 1;
        let span_id = palette_core::SpanId::new(parts.span_id)
...
/// shape consumed by the browser evaluators in `palette-eval`.
...
mod tests {
    ...
    /// mirroring what `palette-otlp` produces for an ingested external agent run.
