...
fn judge_json_hash<T: Serialize>(value: &T) -> Result<Sha256Hash, JudgeBrokerError> {
    palette_core::sha256_json_hash(value).map_err(|err| JudgeBrokerError::Store(err.to_string()))
}
...
mod tests {
    ...
    use palette_secrets::{PutProviderSecretRequest, SqliteProviderSecretStore};
