//! Integration tests for `palette-billing`:
...
use palette_core::{Money, OrganizationId, ProjectId, TenantId};
...
async fn concurrent_plan_change_has_one_winner_no_lost_update() -> anyhow::Result<()> {
    ...
    let conflicts = [&res_a, &res_b]
        .iter()
        .filter(|r| matches!(r, Err(palette_store::StoreError::Conflict(_))))
        .count();
