...
//! 1. **Signature** — verified via `palette-security` (HMAC + replay window).
...
pub enum StripeError {
    ...
    Malformed(String),
    #[error("store error: {0}")]
    Store(#[from] palette_store::StoreError),
...
mod tests {
    ...
    fn fixed_now() -> palette_core::Timestamp {
    ...
    async fn usage_push_is_idempotent_on_stripe() -> anyhow::Result<()> {
        ...
        let invoice = crate::Invoice {
            ...
            line_items: vec![crate::InvoiceLineItem {
                meter: Some(palette_usage::UsageMeter::JudgeCostMicros),
