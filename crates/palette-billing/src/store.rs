...
//! implementation mirroring `palette-usage`'s `SqliteUsageLedger` and the
//! `palette-store-sql` idiom (an `Arc<Mutex<Connection>>`, JSON row payloads,
...
use palette_core::OrganizationId;
...
/// payloads as [`Money`](palette_core::Money) (integer micros); SQL only carries
/// keys and status.
#[async_trait]
