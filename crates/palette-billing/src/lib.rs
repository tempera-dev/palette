//! `palette-billing` — Bandwidth (ARCHITECTURE §20.7 #5.8 / §21.7 / R15).
//!
//! Metered-usage billing built on the same primitives as the rest of the
//! platform: integer-micros [`Money`](palette_core::Money) for every monetary
//! computation (never `f64`), the usage ledger ([`palette_usage`]) as the source
//! of metered quantities, and the signed-webhook primitives in
//! [`palette_security`] for Stripe webhook verification.
//!
...
//!   but not applied.
//! * **Overflow-safe** — all sums use [`Money::try_add`](palette_core::Money::try_add)
//!   and checked integer math, surfacing [`BillingError::Overflow`] instead of
//!   wrapping or panicking.

use palette_core::{Currency, Money, MoneyError, OrganizationId, Timestamp};
...
use palette_usage::{UsageLedgerStore, UsageMeter};

/// Identifier for a billing [`Plan`]. A non-empty, whitespace-free string, like
/// the identifier newtypes in [`palette_core`].
#[derive(
...
pub struct BillingScope {
    pub org_id: OrganizationId,
    pub tenant_id: palette_core::TenantId,
    pub project_id: palette_core::ProjectId,
}
...
pub struct Billing<C: palette_core::Clock = palette_core::SystemClock> {
    store: std::sync::Arc<dyn BillingStore>,
    usage: std::sync::Arc<dyn UsageLedgerStore>,
    clock: C,
}

impl Billing<palette_core::SystemClock> {
    ...
    pub fn new(
        ...
        Self {
            store,
            usage,
            clock: palette_core::SystemClock,
        }
    }
}

impl<C: palette_core::Clock> Billing<C> {
