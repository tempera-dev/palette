...
//!
//! Palette ingests OTLP/native payloads into a richer canonical model
//! ([`palette_schema::CanonicalSpan`]) that records provenance the open standards
...
//! referenced by [`palette_schema::CanonicalSpan::raw_ref`]; reproducibility and
//! re-normalization (`palette-replay::reproject`) must read *that*, not an export.
...
const TENANT_METADATA_KEY: &str = "x-palette-tenant-id";
const PROJECT_METADATA_KEY: &str = "x-palette-project-id";
const ENVIRONMENT_METADATA_KEY: &str = "x-palette-environment-id";
...
/// Rust capture layer. Kept in sync with `palette_browser::semconv`; hardcoded
/// here to avoid a dependency edge into `palette-otlp`.
    ...
    for resource_spans in &request.resource_spans {
        ...
        for attr in &resource.attributes {
            ...
            match attr.key.as_str() {
                "palette.tenant_id" if hints.tenant_id.is_none() => {
                    hints.tenant_id = Some(value.to_string());
                }
                "palette.project_id" if hints.project_id.is_none() => {
                    hints.project_id = Some(value.to_string());
                }
                "palette.environment_id" | "deployment.environment.name"
...
fn convert_span(
    ...
    let input = attributes
        .remove("input.value")
        .or_else(|| span_event_payload(&span.events, "palette.input"));
    let output = attributes
        .remove("output.value")
        .or_else(|| span_event_payload(&span.events, "palette.output"));
    ...
    if temporal_span_kind(&attributes, &span.name).is_some() {
        attributes
            .entry("palette.framework".to_string())
    ...
    events
        ...
        .and_then(|event| {
            event.attributes.iter().find_map(|attr| {
                if attr.key != "palette.payload_json" {
                    return None;
                }
...
fn infer_agent_span_kind(attrs: &CanonicalAttrs, name: &str, otel_kind: i32) -> AgentSpanKind {
    ...
    // `palette.span.kind=llm.call` and `browser.action`, and the declared kind
    // is correct.
    if let Some(value) = attrs
        .get("openinference.span.kind")
        .or_else(|| attrs.get("palette.span.kind"))
...
/// (`span.attributes`) but, by construction, CANNOT carry Palette-internal
...
pub fn canonical_span_to_otlp(span: &palette_schema::CanonicalSpan) -> Span {
...
mod tests {
    ...
    fn lossy_canonical_span() -> palette_schema::CanonicalSpan {
        use palette_schema::{ArtifactRef, CANONICAL_SCHEMA_VERSION, CanonicalSpan};
        CanonicalSpan {
            ...
            raw_ref: ArtifactRef {
                artifact_id: palette_core::ArtifactId::new("raw-artifact")
                    .unwrap_or_else(|err| panic!("{err}")),
                uri: "artifact://tenant/project/raw-artifact".to_string(),
                sha256: palette_core::Sha256Hash::new("rawhash")
                    .unwrap_or_else(|err| panic!("{err}")),
                size_bytes: 128,
    ...
    fn normalizer_accepts_every_canonical_span_kind() {
        // Guard tying palette-otlp <-> palette-schema: every span kind in the
        ...
        // a canonical kind to palette-schema without teaching the normalizer, this
        // fails (it would silently fall through to AgentStep otherwise).
        for wire in palette_schema::conventions::span_kinds() {
    ...
    fn browser_kind_and_status_precedence() {
        use palette_schema::{AgentSpanKind, SpanStatus};
        ...
        // Kind: an explicit `palette.span.kind` wins over the `browser.action`
    ...
    fn decodes_real_otlp_export_request_and_maps_agent_kinds() {
        ...
        assert_eq!(raw_request.normalizer_version, "palette-otlp-v1");
    ...
    fn decodes_collector_otlp_json_and_extracts_scope_hints() {
                ...
                "scopeSpans": [{
                    ...
                    "spans": [{
                        ...
                        "attributes": [
                            ...
                            {"key": "palette.step_seq", "value": {"intValue": "7"}}
                        ],
                        "events": [
                            {
                                "name": "palette.input",
        ...
        assert_eq!(converted[0].attributes["palette.step_seq"], json!(7));
        assert_eq!(converted[0].input, Some(json!({"prompt": "hello"})));
        assert_eq!(converted[0].output, Some(json!({"answer": "world"})));
    }

    #[test]
    fn maps_every_canonical_palette_span_kind_from_otlp_attrs() {
        let cases = [
