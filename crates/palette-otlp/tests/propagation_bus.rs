...
//! spawned async task, end to end through `palette_bus::InMemoryBus`.
//!
//! Palette's `BusMessage` payload is opaque bytes, so the producer serializes the
