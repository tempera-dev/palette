...
//! Raw envelopes and canonical spans are persisted into the `palette.raw_envelopes`
//! / `palette.spans` tables defined there with the full canonical object kept in
...
//! `palette.spans` columns at write time, so `query_runs` aggregates run
...
impl ClickHouseTraceStore {
    ...
    async fn existing_raw_keys(
        ...
        let tuples = candidates
            ...
            .join(",");
        let sql = format!(
            "SELECT DISTINCT tenant_id, project_id, idempotency_key FROM palette.raw_envelopes WHERE (tenant_id, project_id, idempotency_key) IN ({tuples}) FORMAT JSONEachRow"
        );
    ...
    async fn existing_span_keys(&self, candidates: &[SpanKey]) -> StoreResult<BTreeSet<SpanKey>> {
        ...
        let tuples = candidates
            ...
            .join(",");
        let sql = format!(
            "SELECT DISTINCT tenant_id, project_id, trace_id, span_id, seq FROM palette.spans WHERE (tenant_id, project_id, trace_id, span_id, toString(seq)) IN ({tuples}) FORMAT JSONEachRow"
        );
        let body = self.query_raw(&sql).await?;
        let mut found = BTreeSet::new();
        for line in body.lines().filter(|line| !line.trim().is_empty()) {
    ...
    async fn get_trace_with_project(
        ...
        let sql = format!(
            "SELECT span_json FROM palette.spans WHERE {} ORDER BY seq ASC, start_time ASC FORMAT JSONEachRow",
            predicates.join(" AND ")
        );
...
impl TraceStore for ClickHouseTraceStore {
    async fn write_batch(&self, batch: Arc<CanonicalTraceBatch>) -> StoreResult<WriteAck> {
        ...
        if !raw_rows.is_empty() {
            self.insert(
                "INSERT INTO palette.raw_envelopes FORMAT JSONEachRow",
                raw_rows,
            )
            .await?;
        }
        ...
        if !span_rows.is_empty() {
            self.insert("INSERT INTO palette.spans FORMAT JSONEachRow", span_rows)
                .await?;
        }
    ...
    async fn get_raw_envelope(
        ...
        let sql = format!(
            "SELECT raw_json FROM palette.raw_envelopes WHERE tenant_id = '{}' AND project_id = '{}' AND idempotency_key = '{}' LIMIT 1 FORMAT JSONEachRow",
    ...
    async fn query_spans(
        ...
        let sql = format!(
            "SELECT span_json FROM palette.spans WHERE {} ORDER BY start_time DESC, seq ASC FORMAT JSONEachRow",
            predicates.join(" AND ")
        );
        let spans = self.span_json_query(&sql).await?;
