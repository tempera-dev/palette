...
//!
//! ```sh
//! cargo test -p palette-store-sql --test pg_trace_store -- --ignored
//! ```

#![cfg(feature = "postgres")]

use palette_store_conformance::assert_trace_store_conformance;
use palette_store_sql::PgTraceStore;
use testcontainers::{
...
async fn pg_trace_store_conforms() {
    let container = GenericImage::new("postgres", "16-alpine")
        ...
        .with_env_var("POSTGRES_USER", "palette")
        .with_env_var("POSTGRES_PASSWORD", "palette")
        .with_env_var("POSTGRES_DB", "palette")
        .start()
        .await
    ...
    let connection_string =
        format!("host={host} port={port} user=palette password=palette dbname=palette");
