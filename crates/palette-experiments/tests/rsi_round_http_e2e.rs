...
//! through the **real** `palette-judge` HTTP provider seam
...
use palette_eval::{GateDecision, GatePolicy, VarianceReduction};
...
const CANDIDATE_MARKER: &str = "PALETTE-CANDIDATE";
...
const REWRITE: &str = "You are PALETTE-CANDIDATE, a meticulous factual assistant. \
...
fn provider_config(endpoint: String) -> HttpJudgeProviderConfig {
    HttpJudgeProviderConfig {
        ...
        max_cost: palette_core::Money::usd_micros(0),
...
async fn run_with_proposer(
    ...
    retry: RetryPolicy,
) -> Result<palette_experiments::OptimizationOutcome, OptimizerError> {
...
/// `palette-experiments` unit test proves flip the gate, so the outcome is
...
) -> palette_experiments::OptimizationOutcome {
    let state = MockState::new(ProposerBehavior::Rewrite, EvalPolicy::Unused);
    let endpoint = spawn(state, Wire::Anthropic).await;
    let provider = AnthropicJudgeProvider::new(provider_config(endpoint));
    let mut cfg = round_config(OptimizerStrategy::LlmRewrite);
...
async fn cuped_difficulty_covariate_resolves_the_round_over_real_http() {
    ...
    assert_eq!(evaluation.gate.test, palette_eval::StatisticalTest::PairedT);
    assert!(
