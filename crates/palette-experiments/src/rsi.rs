...
use palette_core::DatasetCaseId;
...
use palette_stats::{GapAssessment, assess_generalization_gap};
...
pub enum RsiError {
    ...
    Fingerprint {
        ...
        #[source]
        source: palette_core::JsonHashError,
    },
    ...
    #[error(transparent)]
    Eval(#[from] palette_eval::EvalError),
    /// The generalization-gap statistics failed.
    #[error(transparent)]
    Stats(#[from] palette_stats::StatsError),
}
...
) -> Result<f64, palette_eval::EvalError> {
...
mod tests {
    use super::*;
    use palette_core::{DatasetId, EnvironmentId, ProjectId, SpanId, TenantId, Timestamp, TraceId};
    ...
    fn snapshot(n: usize) -> DatasetVersionSnapshot {
        ...
        let corpus_root =
            palette_datasets::corpus_root_for_cases(&cases).unwrap_or_else(|err| panic!("{err}"));
        DatasetVersionSnapshot {
            tenant_id: TenantId::new("t").unwrap_or_else(|err| panic!("{err}")),
            project_id: ProjectId::new("p").unwrap_or_else(|err| panic!("{err}")),
            dataset_id: DatasetId::new("d").unwrap_or_else(|err| panic!("{err}")),
            version_id: palette_core::DatasetVersionId::new("v1")
