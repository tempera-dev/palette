...
use palette_datasets::DatasetVersionSnapshot;
use palette_eval::{
...
pub trait AgentAdapter: Send + Sync {
    async fn run_case(
        &self,
        case: palette_datasets::DatasetCase,
...
/// the `palette-stats` confidence interval already implemented here
...
/// text — without depending on the richer (unmerged) `palette-scenarios`
...
/// substitute for `palette-scenarios` clustering, not a reimplementation of it.
...
/// `palette-scenarios` clustering (unmerged PR #470), not a dependency on it.
///
...
impl ProposalContext {
    ...
    /// which avoids a dependency on `palette-eval` and the resulting crate cycle.
    ///
...
/// [`TextGenerator`] (the plain text-generation seam in `palette-judge` — NOT the
...
/// Test gate (§21.3) + the palette-stats CI before it can be accepted. Proposal
        ...
        Ok(vec![CandidateChange {
            ...
            rationale: format!(
                "reflective LLM-rewrite scaffold (§21.3) over {} failing example(s); no model \
                 ...
                 palette-stats CI before acceptance.",
...
impl AsyncProposalStrategy for LlmRewrite {
    async fn propose_async(
        ...
        Ok(vec![CandidateChange {
            ...
            rationale: format!(
                "reflective LLM rewrite via the palette-judge text-generation seam over {} failing \
                 example(s) (model {}); a proposal only — must clear the held-out Test gate + \
                 palette-stats CI before acceptance. Proposal is not acceptance: the gate decides.",
                ...
                candidates.push(CandidateChange {
                    ...
                    rationale: format!(
                        "deterministic param-grid point (§6.1 model-params lever) proposed for \
                         goal: {}; a proposal only — the held-out Test gate + palette-stats CI \
                         decide acceptance.",
                        ctx.goal
                    ),
...
pub struct CaseScore {
    ...
    /// see [`palette_stats::cuped_adjust`]). `None` disables CUPED for the case; the
...
///
/// Both come straight from `palette-eval` / `palette-stats`; this function does no
...
fn gate_candidate(
    ...
    // (1) Real held-out Test gate — reused verbatim from palette-eval, and now
    ...
    // (2) Real anti-overfit guardrail — reused verbatim from palette-stats.
...
mod tests {
    ...
    use palette_eval::{EvalError, EvaluatorKind, StatisticalTest};
    use palette_judge::{JudgeAuditRecord, JudgeBrokerError};
    use serde_json::json;
    ...
    impl TextGenerator for FakeRewriteGenerator {
        async fn generate(
            &self,
            req: palette_judge::GenerationRequest,
            _credentials: ProviderCredentials,
        ) -> palette_judge::JudgeProviderResult<palette_judge::GenerationResponse> {
            ...
            Ok(palette_judge::GenerationResponse {
    ...
    fn cuped_round_falls_back_when_covariate_is_partial() {
        ...
        assert_eq!(evaluated.gate.test, palette_eval::StatisticalTest::PairedT);
    }
    ...
    async fn round_drives_deterministic_param_search_through_the_gate() {
        ...
        impl TextGenerator for PanicGenerator {
            async fn generate(
                &self,
                _req: palette_judge::GenerationRequest,
                _credentials: ProviderCredentials,
            ) -> palette_judge::JudgeProviderResult<palette_judge::GenerationResponse> {
    ...
    fn passing_eval(p_value: f64) -> CandidateEvaluation {
        CandidateEvaluation {
            ...
            gate: ExperimentComparison {
                ...
                test: palette_eval::StatisticalTest::PairedT,
                adjusted_alpha: 0.05,
                mde: None,
                required_n: None,
            },
