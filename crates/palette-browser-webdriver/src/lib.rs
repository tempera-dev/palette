//! WebDriver-backed [`palette_browser::BrowserDriver`] backend.
//!
...
//!
//! Grounding contract (see [`palette_browser::BrowserDriver`]): an action whose
...
mod tests {
    ...
    fn locator_maps_selector_to_css() {
        ...
        match locator_for("#palette-known") {
            Locator::Css(value) => assert_eq!(value, "#palette-known"),
    ...
    fn action_selectors_route_through_grounding() {
        ...
        assert_eq!(click.selector(), Some("#palette-known"));
    ...
    /// --port=4444`). It serves [`palette_browser::CONFORMANCE_FIXTURE_HTML`] on a
    /// local TCP port, then runs the shared conformance suite.
    ///
    /// Run with, e.g.:
    /// `WEBDRIVER_URL=http://localhost:4444 cargo test -p palette-browser-webdriver \
    ...
    async fn live_conformance() {
        ...
        let server = tokio::spawn(async move {
            let body = palette_browser::CONFORMANCE_FIXTURE_HTML;
            let response = format!(
        ...
        palette_browser::assert_browser_driver_conformance(&mut driver, &base_url).await;
