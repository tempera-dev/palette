...
pub trait ArtifactStore: Send + Sync {
    async fn put_bytes(
        ...
        mime_type: &str,
        redaction_class: palette_schema::RedactionClass,
        bytes: &[u8],
...
/// order of [`palette_schema::roll_up_runs`]. This is the §8.1-compliant
...
/// [`palette_schema::roll_up_runs`] and [`palette_schema::filter_run_summaries`]
...
/// Mirrors `palette_schema`'s private `run_duration_ms`: clamps a non-negative
...
/// Mirrors `palette_schema`'s private `merge_cost` fold over the run's per-span
...
/// Mirrors `palette_schema`'s private `push_model`: distinct models by
...
/// Mirrors `palette_schema`'s private `push_release_id`: distinct release ids,
