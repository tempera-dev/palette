//! Playwright-backed [`palette_browser::BrowserDriver`] backend.
//!
...
//! a missing selector surfaces as a [`StepStatus`](palette_browser::StepStatus)
//! `Error` outcome — never a [`BrowserError`].
//!
//! Must pass [`palette_browser::assert_browser_driver_conformance`]; the live
