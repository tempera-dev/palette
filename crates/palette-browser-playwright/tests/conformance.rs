...
//!
//! ```text
//! cd crates/palette-browser-playwright/runner && npm install
//! cargo test -p palette-browser-playwright -- --ignored
//! ```
//!
//! It serves [`palette_browser::CONFORMANCE_FIXTURE_HTML`] from a throwaway
//! single-purpose HTTP server on `127.0.0.1` and runs the cross-backend
//! [`palette_browser::assert_browser_driver_conformance`] suite against the
...
use palette_browser_playwright::{PlaywrightConfig, PlaywrightDriver};
