...
use palette_core::{ProjectId, SpanId, TenantId, TraceId};
use palette_schema::{CanonicalSpan, RedactionClass};
use palette_store::{IntoStoreResult, StoreError, StoreResult, TraceStore};
...
fn body_indexable(span: &CanonicalSpan, body_ref: Option<&palette_schema::ArtifactRef>) -> bool {
...
mod tests {
    ...
    fn fixture_span_with_project(
        ...
        CanonicalSpan {
            ...
            raw_ref: palette_schema::ArtifactRef {
                artifact_id: palette_core::ArtifactId::new("artifact")
                    .unwrap_or_else(|err| panic!("{err}")),
                uri: "artifact://tenant/project/artifact".to_string(),
                sha256: palette_core::Sha256Hash::new("hash").unwrap_or_else(|err| panic!("{err}")),
                size_bytes: 0,
                mime_type: "application/json".to_string(),
                redaction_class: palette_schema::RedactionClass::Internal,
            },
        }
    }
}
