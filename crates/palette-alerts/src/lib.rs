...
impl AlertEngine {
    ...
    pub fn evaluate(
        ...
        headers.insert("palette-signature".to_string(), signature.header_value());
        headers.insert("palette-alert-policy".to_string(), policy.policy_id.clone());
...
/// [`palette_schema::TraceCompletionState`] flips to a terminal value, at which
...
/// caller reports a terminal [`palette_schema::TraceCompletionState`] (e.g.
...
/// `palette_ingest::trace_completion_state` evaluation rather than by wall-clock
...
fn completion_is_terminal(state: &palette_schema::TraceCompletionState) -> bool {
    !matches!(state, palette_schema::TraceCompletionState::Open)
}
...
impl TailBuffer {
    ...
    pub fn offer(
        &mut self,
        span: CanonicalSpan,
        completion: &palette_schema::TraceCompletionState,
    ) -> TailBufferOutcome {
...
mod tests {
    use super::*;
    use palette_core::{Money, TokenCounts};
    ...
    fn slack_channel_formats_block_kit_with_severity_score_and_trace_button() {
        ...
        assert_eq!(
            button["url"].as_str(),
            Some("https://palette.test/traces/trace")
        );
    }
    ...
    fn slack_channel_trims_trace_deep_link_button_url() {
        ...
        let mut input = fixture_alert_input(Utc::now());
        input.links.trace_url = "  https://palette.test/traces/trace?from=alert  ".to_string();
        ...
        assert_eq!(
            button["url"].as_str(),
            Some("https://palette.test/traces/trace?from=alert")
        );
    }
    ...
    fn fixture_alert_input(now: Timestamp) -> AlertInput {
        AlertInput {
            ...
            links: AlertLinks {
                trace_url: "https://palette.test/traces/trace".to_string(),
                cluster_url: Some("https://palette.test/clusters/cluster".to_string()),
                dataset_url: Some("https://palette.test/datasets/dataset".to_string()),
                gate_url: Some("https://palette.test/gates/gate".to_string()),
            },
            now,
        }
    }
    ...
    fn fixture_trace(
        ...
        TraceView {
            ...
            spans: vec![CanonicalSpan {
                ...
                tenant_id: tenant.clone(),
                project_id: ProjectId::new("project").unwrap_or_else(|err| panic!("{err}")),
                environment_id: palette_core::EnvironmentId::new("prod")
                    .unwrap_or_else(|err| panic!("{err}")),
                trace_id: TraceId::new("trace").unwrap_or_else(|err| panic!("{err}")),
                span_id: palette_core::SpanId::new("span").unwrap_or_else(|err| panic!("{err}")),
                parent_span_id: None,
                seq: 1,
                ...
                raw_ref: ArtifactRef {
                    artifact_id: palette_core::ArtifactId::new("raw")
                        .unwrap_or_else(|err| panic!("{err}")),
                    uri: "artifact://tenant/project/raw".to_string(),
                    sha256: palette_core::Sha256Hash::new("ab".repeat(32))
                        .unwrap_or_else(|err| panic!("{err}")),
                    size_bytes: 2,
    ...
    fn fixture_span(
        ...
        CanonicalSpan {
            ...
            span_id: palette_core::SpanId::new(span_id).unwrap_or_else(|err| panic!("{err}")),
            parent_span_id: None,
            seq: 1,
            kind: AgentSpanKind::AgentRun,
            name: "run".to_string(),
    ...
    fn tail_buffer_holds_spans_until_completion_then_samples_whole_trace() {
        use palette_schema::TraceCompletionState;
