//! Langfuse export → Palette canonical span normalization.
//!
//! Converts a [Langfuse] trace/observations export into Palette's canonical span
...
//! their traces into Palette as canonical spans WITH the raw envelope retained
...
/// `palette_temporal::temporal_history_to_raw_ingest`.
...
fn trace_span_id(trace_id: &str) -> Result<SpanId, palette_core::IdError> {
    SpanId::new(format!("lf-trace-{trace_id}"))
}

fn observation_span_id(obs_id: &str) -> Result<SpanId, palette_core::IdError> {
...
mod tests {
    ...
    fn raw_bytes_are_retained_losslessly() {
        let scope = TenantScope::new(
            palette_core::TenantId::new("t1").unwrap_or_else(|e| panic!("{e}")),
            palette_core::ProjectId::new("p1").unwrap_or_else(|e| panic!("{e}")),
            palette_core::EnvironmentId::new("e1").unwrap_or_else(|e| panic!("{e}")),
        );
