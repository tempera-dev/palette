    ...
    S: MetadataStore,
{
    ...
    store
        ...
        .unwrap_or_else(|err| panic!("{err}"));
    store
        .put_environment(palette_store::EnvironmentMetadata {
            tenant_id: tenant.clone(),
            project_id: project.clone(),
...
fn fixture_rollup_batch(
    ...
    let span = |span_id: &str,
        ...
        attributes.insert("palette.release_id".to_string(), json!(release));
        CanonicalSpan {
            schema_version: palette_schema::CANONICAL_SCHEMA_VERSION,
...
fn span_ids(spans: &[palette_schema::SpanSummary]) -> Vec<&str> {
    spans.iter().map(|span| span.span_id.as_str()).collect()
}

fn run_trace_ids(runs: &[palette_schema::RunSummary]) -> Vec<&str> {
