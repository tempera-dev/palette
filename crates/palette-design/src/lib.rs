//! # palette-design — the eval/gate pre-registration manifest
//!
//! Palette's load-bearing scientific invariant (`ARCHITECTURE.md` §1 #9–#11,
...
//! `palette-stats`; the *engine wiring* (routing the gate through the design's
...
use palette_core::{JsonHashError, Sha256Hash, sha256_json_hash};
...
/// for fitting and tuning. When `palette-datasets` gains a first-class per-case
...
/// `palette-stats` dispatch by metric family + satisfied assumptions; the explicit
/// variants pin a test when the analyst has already decided. These mirror the
/// `palette_stats::TestKind` variants the gate will execute (the explicit
...
/// one for `E[d]` — see `palette_stats::cuped_paired_t_test`); it is unbiased for
...
pub enum VarianceReduction {
    ...
    /// `palette_stats::cuped_paired_t_test`). The covariate MUST also be independent
