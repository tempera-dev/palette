...
///
/// Palette delivers webhooks with at-least-once semantics: a single logical
...
pub const WEBHOOK_IDEMPOTENCY_KEY_HEADER: &str = "palette-idempotency-key";
...
pub fn webhook_idempotency_key(
    ...
    Ok(format!(
        "bt_whk_{}",
        palette_core::lower_hex(&mac.finalize().into_bytes())
    ))
}
...
mod tests {
    ...
    fn webhook_idempotency_key_is_stable_per_delivery_and_unique_across_deliveries() {
        ...
        assert_eq!(WEBHOOK_IDEMPOTENCY_KEY_HEADER, "palette-idempotency-key");
    }
