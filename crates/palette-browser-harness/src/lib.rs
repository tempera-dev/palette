...
//! [`StepTriple`]s, scores them with the browser evaluators in `palette-eval`,
...
//! [`palette_experiments::AgentAdapter`], so a browser agent plugs directly into
//! `run_agent_experiment` / `evaluate_gate` like any other agent.

use palette_browser::{BrowserAction, BrowserDriver, BrowserError, LlmDecision, StepTriple};
use palette_browser_capture::{browser_trace, browser_trace_from_spans};
...
use palette_experiments::{AgentAdapter, AgentAdapterError, AgentRunOutput, HarnessContext};
use palette_schema::{CanonicalSpan, EvaluatorLane};
...
mod tests {
    use super::*;
    use palette_browser::{BrowserEngine, FIXTURE_KNOWN_SELECTOR, MockDriver};
    ...
    fn harness_context() -> HarnessContext {
        HarnessContext {
            tenant_id: palette_core::TenantId::new("tenant").unwrap_or_else(|err| panic!("{err}")),
            project_id: palette_core::ProjectId::new("project")
                .unwrap_or_else(|err| panic!("{err}")),
            dataset_id: palette_core::DatasetId::new("dataset")
                .unwrap_or_else(|err| panic!("{err}")),
            dataset_version_id: palette_core::DatasetVersionId::new("v1")
                .unwrap_or_else(|err| panic!("{err}")),
            agent_release_id: palette_core::AgentReleaseId::new("agent")
                .unwrap_or_else(|err| panic!("{err}")),
        }
    }
    ...
    fn dataset_case(input: Value) -> DatasetCase {
        DatasetCase {
            ...
            dataset_id: palette_core::DatasetId::new("dataset")
                .unwrap_or_else(|err| panic!("{err}")),
            case_id: palette_core::DatasetCaseId::new("case").unwrap_or_else(|err| panic!("{err}")),
            source_trace_id: palette_core::TraceId::new("trace")
                .unwrap_or_else(|err| panic!("{err}")),
            source_span_id: palette_core::SpanId::new("span").unwrap_or_else(|err| panic!("{err}")),
            source_environment_id: palette_core::EnvironmentId::new("prod")
                .unwrap_or_else(|err| panic!("{err}")),
            input,
            output: json!(null),
            reference: None,
    ...
    /// Build a `tool.call` `CanonicalSpan` exactly as `palette-otlp` produces it
    /// for an ingested `browser-use`/Stagehand step (browser.* attributes).
    fn ingested_span(seq: u64, matched: bool) -> CanonicalSpan {
        use palette_browser::semconv;
        use palette_schema::{ArtifactRef, CANONICAL_SCHEMA_VERSION, RedactionClass, SpanStatus};
        ...
        let artifact = ArtifactRef {
            artifact_id: palette_core::ArtifactId::new("a").unwrap_or_else(|err| panic!("{err}")),
            uri: "artifact://t/p/a".to_string(),
            sha256: palette_core::Sha256Hash::new("h").unwrap_or_else(|err| panic!("{err}")),
            size_bytes: 0,
        ...
        CanonicalSpan {
            ...
            tenant_id: palette_core::TenantId::new("t").unwrap_or_else(|err| panic!("{err}")),
            project_id: palette_core::ProjectId::new("p").unwrap_or_else(|err| panic!("{err}")),
            environment_id: palette_core::EnvironmentId::new("e")
                .unwrap_or_else(|err| panic!("{err}")),
            trace_id: palette_core::TraceId::new("tr").unwrap_or_else(|err| panic!("{err}")),
            span_id: palette_core::SpanId::new(format!("s{seq}"))
                .unwrap_or_else(|err| panic!("{err}")),
            parent_span_id: None,
            seq,
            kind: palette_schema::AgentSpanKind::ToolCall,
    ...
    fn ingested_regressed_run_is_gated_as_regression() {
        ...
        // the canonical form palette-otlp emits) are projected to browser_steps,
