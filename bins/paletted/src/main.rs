...
use palette_bus::{DeadLetter, DurableBus, InMemoryBus, SqliteDurableBus};
use palette_calibration::SqliteCalibrationStore;
use palette_composio::{ConnectorToolPolicy, HttpComposioClient};
use palette_core::{IdempotencyKey, Money, Page, PageRequest, ProjectId, TenantId, TraceId};
...
use palette_oauth::SqliteOAuthStore;
use palette_oauth_server::OAuthServerState;
use palette_otlp::{OtlpGrpcTraceService, TraceServiceServer};
use palette_prompts::SqlitePromptRegistry;
...
use palette_search::{SearchIndex, TantivySearchIndex, TraceIngestedSearchProcessor};
use palette_secrets::{EncryptedSqliteProviderSecretStore, SecretKeyring};
use palette_store::{ArtifactStore, StoreError, StoreResult, TraceStore};
...
#[command(
    name = "paletted",
    about = "All-in-one Palette agent observability server"
)]
struct Args {
    #[command(subcommand)]
    ...
    #[arg(long, env = "PALETTE_ISSUER_URL")]
    ...
    #[arg(long, env = "PALETTE_LOGIN_URL")]
    ...
    #[arg(long, env = "PALETTE_OAUTH_RESOURCE", default_value = "palette")]
    ...
    #[arg(
        long,
        env = "PALETTE_ARTIFACT_MAX_BYTES",
    ...
    #[arg(long, env = "PALETTE_PER_PROJECT_EVENT_QUOTA")]
    per_project_event_quota: Option<u64>,
    #[arg(long, env = "PALETTE_QUOTA_WINDOW_SECONDS", default_value_t = 60)]
    quota_window_seconds: i64,
    #[arg(long, env = "PALETTE_QUOTA_DB_PATH")]
    ...
    #[arg(
        long,
        env = "PALETTE_ALLOW_INSECURE_LOCAL_BIND",
        default_value_t = false
    )]
    allow_insecure_non_loopback: bool,
    #[arg(long, env = "PALETTE_PROVIDER_SECRET_KEY")]
    ...
    #[arg(long, env = "PALETTE_CONNECTOR_ALLOW_TOOLS", value_delimiter = ',')]
    ...
    #[arg(
        long,
        env = "PALETTE_TRACE_WRITE_DRAIN_INTERVAL_MS",
        default_value_t = 1000
    )]
    trace_write_drain_interval_ms: u64,
    #[arg(long, env = "PALETTE_TRACE_WRITE_MAX_ATTEMPTS", default_value_t = 3)]
    ...
    #[arg(
        long,
        env = "PALETTE_MAX_RAW_PAYLOAD_BYTES",
    ...
    #[arg(
        long,
        env = "PALETTE_INLINE_PAYLOAD_BYTES",
    ...
    #[arg(long, env = "PALETTE_MAX_ATTRIBUTES", default_value_t = 128)]
    ...
    #[arg(long, env = "PALETTE_TRACE_IDLE_TIMEOUT_SECONDS", default_value_t = 60)]
    ...
    #[arg(long, env = "PALETTE_TRACE_LATE_WINDOW_SECONDS", default_value_t = 10)]
    trace_late_window_seconds: i64,
    #[arg(
        long,
        env = "PALETTE_TRACE_INGESTED_DRAIN_INTERVAL_MS",
        default_value_t = 1000
    )]
    trace_ingested_drain_interval_ms: u64,
    #[arg(long, hide = true, env = "PALETTE_TEST_TRACE_WRITE_LEASE_MARKER")]
    test_trace_write_lease_marker: Option<PathBuf>,
    #[arg(long, hide = true, env = "PALETTE_TEST_TRACE_WRITE_HOLD_PATH")]
    test_trace_write_hold_path: Option<PathBuf>,
    #[arg(long, hide = true, env = "PALETTE_TEST_TRACE_INGESTED_LEASE_MARKER")]
    test_trace_ingested_lease_marker: Option<PathBuf>,
    #[arg(long, hide = true, env = "PALETTE_TEST_TRACE_INGESTED_HOLD_PATH")]
    test_trace_ingested_hold_path: Option<PathBuf>,
    #[arg(long, hide = true, env = "PALETTE_TEST_TRACE_INGESTED_FAIL_WHILE_PATH")]
    test_trace_ingested_fail_while_path: Option<PathBuf>,
    #[arg(
        long,
        hide = true,
        env = "PALETTE_TEST_TRACE_STORE_FAIL_WRITE_WHILE_PATH"
    )]
    test_trace_store_fail_write_while_path: Option<PathBuf>,
    #[arg(long, hide = true, env = "PALETTE_TEST_HTTP_TRACE_STORE_URL")]
    test_http_trace_store_url: Option<String>,
    /// Opt in to anonymous self-host usage telemetry. Off by default (R12.5):
    /// a self-hosted paletted makes no outbound telemetry call unless this is set.
    #[arg(long, env = palette_core::SelfHostTelemetryConfig::ENV_VAR)]
        ...
        && !args.allow_insecure_non_loopback
    {
        anyhow::bail!(
            "--auth-mode local may only bind HTTP to loopback addresses; got --addr {}. \
             ...
             --allow-insecure-non-loopback (env PALETTE_ALLOW_INSECURE_LOCAL_BIND) to opt \
...
async fn main() -> anyhow::Result<()> {
    ...
    // to disabled and paletted makes no outbound telemetry call.
    let telemetry = palette_core::SelfHostTelemetryConfig::new(args.self_host_telemetry);
    ...
    if matches!(args.auth_mode, AuthModeArg::Local) {
        eprintln!("============================================================");
        eprintln!("WARNING: paletted is running in INSECURE --auth-mode local.");
    ...
    ingest_policy.validate().context(
        "invalid ingest policy; check PALETTE_MAX_RAW_PAYLOAD_BYTES, \
                  PALETTE_INLINE_PAYLOAD_BYTES, PALETTE_MAX_ATTRIBUTES, \
                  PALETTE_TRACE_IDLE_TIMEOUT_SECONDS, PALETTE_TRACE_LATE_WINDOW_SECONDS",
    )?;
    eprintln!(
    ...
    let ingest = IngestService::new(artifacts, traces.clone(), bus, ingest_policy)
        ...
        .with_importer(std::sync::Arc::new(MeteredImporter::new(
            palette_temporal::TemporalHistoryImporter,
            "temporal-history-import-v1",
            metrics.clone(),
        )))
        .with_importer(std::sync::Arc::new(MeteredImporter::new(
            palette_langfuse::LangfuseImporter,
            palette_langfuse::LANGFUSE_CONTRACT,
            metrics.clone(),
        )));
    ...
    let otlp_default_scope = palette_core::TenantScope::new(
        palette_core::TenantId::new(args.default_tenant_id.clone())?,
        palette_core::ProjectId::new(args.default_project_id.clone())?,
        palette_core::EnvironmentId::new(args.default_environment_id.clone())?,
    );
    ...
    // 501 and paletted runs with zero third-party cloud dependency (OSS default).
    ...
    let api_key_store: Option<Arc<dyn palette_auth::ApiKeyStore>> =
        if matches!(args.auth_mode, AuthModeArg::Required) {
    ...
    // paletted sqlite migration list above.
    ...
    let app = router(state.clone())
        .merge(palette_mcp::router(state))
        ...
        .merge(palette_oauth_server::router(oauth_state));
    ...
    let grpc_server = async move {
        Server::builder()
            ...
            .await
            .context("serve paletted otlp grpc")
    };
...
fn migrate_local_sqlite_stores(paths: &[PathBuf]) -> anyhow::Result<()> {
    ...
    for path in unique_paths {
        migrate_local_paletted_sqlite(&path)
...
fn spawn_queue_stats_sampler(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ticker.tick().await;
            // Unfiltered, deployment-wide DLQ — every tenant, so `palette_dlq_depth`
...
impl<I: SourceImporter> SourceImporter for MeteredImporter<I> {
    ...
    fn normalize(
        &self,
        scope: &palette_core::TenantScope,
        raw_bytes: &[u8],
...
mod artifact_cap_tests {
    ...
    fn artifact_max_bytes_has_a_runtime_default_and_cli_override() {
        let default_args = Args::try_parse_from(["paletted"])
            .unwrap_or_else(|err| panic!("parse default args: {err}"));
        assert_eq!(default_args.artifact_max_bytes, DEFAULT_ARTIFACT_MAX_BYTES);

        let override_args = Args::try_parse_from(["paletted", "--artifact-max-bytes", "4096"])
...
mod auth_default_tests {
    ...
    // #127: paletted must require auth out of the box. With no auth flags the
    ...
    fn auth_mode_local_allows_loopback_http_bind() {
        ...
        let args = Args::parse_from(["paletted", "--auth-mode", "local", "--addr", "[::1]:8080"]);
        let result = validate_auth_mode_bind(&args);
    ...
    fn auth_mode_local_rejects_public_http_bind() {
        for addr in ["0.0.0.0:8080", "[::]:8080", "192.0.2.10:8080"] {
            let args = Args::parse_from(["paletted", "--auth-mode", "local", "--addr", addr]);
