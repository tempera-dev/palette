//! Optional thin "query/API" role binary (R1.2).
//!
...
//! `thin-bins` cargo feature so an operator who wants to scale the read/query
//! API independently can, but nobody is forced into a multi-service deployment.
//!
...
//!
//! ```sh
//! cargo run -p paletted --features thin-bins --bin palette-queryd
//! ```

fn main() {
    eprintln!(
        "palette-queryd is an optional thin query/API-role binary (R1.2). The \
         ...
         operators who explicitly want to scale the read API separately."
    );
}
