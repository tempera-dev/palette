...
//! contract, so it is intentionally registered outside `palette_api::router`.
...
/// the main app alongside `palette_api::router` and the MCP router.
...
mod tests {
    ...
    async fn metrics_endpoint_renders_prometheus_text() {
        ...
        assert!(text.contains("palette_trace_writes_total{result=\"success\"} 5"));
    }
    ...
    async fn middleware_records_latency_for_matched_route() {
        ...
        assert!(out.contains("palette_http_request_duration_seconds"));
