...
//!
//! The all-in-one `paletted` is the only mandatory binary. Optional thin role
...
#[test]
fn default_build_ships_only_the_all_in_one_paletted() {
    let manifest = read("Cargo.toml");

    // The default feature set must be empty so nothing pulls in the thin bins.
    assert!(
        manifest.contains("default = []"),
        "paletted default features must be empty so the all-in-one is the only default build"
    );
    assert!(
        manifest.contains("thin-bins = []"),
        "paletted must declare an opt-in `thin-bins` feature"
    );

    // The optional thin role binaries must be feature-gated, never default.
    for thin in ["palette-ingestd", "palette-queryd"] {
...
fn thin_role_sources_point_back_at_the_all_in_one() {
    // The thin bins are stubs that document the role and defer to `paletted`,
    // so the supported, tested path stays the single-process server.
    for (src, role) in [
        ("src/bin/palette-ingestd.rs", "ingest"),
        ("src/bin/palette-queryd.rs", "query"),
    ] {
        let body = read(src);
        assert!(
            body.contains("R1.2"),
        ...
        assert!(
            body.contains("all-in-one `paletted`"),
            "{src} ({role}) must point operators back at the all-in-one paletted"
        );
        assert!(
...
fn default_compose_does_not_mandate_thin_bins_or_external_services() {
    // The default self-host compose path must run one Palette process, not a
