//! Offline self-host test for R1.3: OSS runs without Palette Cloud.
//!
...
//! service or a `127.0.0.1` health probe, nothing points at a public Palette
...
fn repo_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(|p| p.parent())
        .unwrap_or_else(|| panic!("repo root is two levels above bins/paletted"))
        .to_path_buf()
}
...
/// Hosts that would indicate a self-host phoning home to Palette Cloud. None of
/// these may appear anywhere in the default compose file.
const FORBIDDEN_OUTBOUND_HOSTS: &[&str] = &[
    "api.palette.dev",
    "app.palette.dev",
    "cloud.palette.dev",
    "telemetry.palette.dev",
    "palette.cloud",
];
...
fn default_compose_makes_no_outbound_calls_except_configured_providers() {
    ...
    // 1. No service points at a Palette Cloud / telemetry host.
    for host in FORBIDDEN_OUTBOUND_HOSTS {
        assert!(
            !compose.contains(host),
            "offline self-host compose must not reference Palette Cloud host {host}"
        );
    }

    // 2. Every wired URL is container-local (service DNS) or a loopback probe.
    //    The OTLP exporter endpoint the example tools use targets `paletted`, not
    //    an external collector.
    for local in [
        "http://paletted:8080",
        "http://paletted:4317",
        "OTEL_EXPORTER_OTLP_ENDPOINT: http://paletted:4317",
    ] {
        assert!(
    ...
    //    profiles, so the default `up` path is one Palette process with no extra
    ...
    // 4. paletted does not depend on any external service in the default path.
    assert!(
        !compose_service_block(&compose, "paletted").contains("depends_on:"),
        "paletted must not depend on external services in the offline default path"
    );
}

#[test]
fn paletted_self_host_telemetry_is_opt_out_so_no_outbound_call_by_default() {
    ...
    let default = palette_core::SelfHostTelemetryConfig::default();
    ...
    assert!(
        !compose.contains("PALETTE_SELF_HOST_TELEMETRY"),
...
fn smoke_compose_script_keeps_runtime_loop_local() {
    ...
    // local compose stack without introducing Palette Cloud or deploy surfaces.
