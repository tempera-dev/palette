...
#[tokio::test]
async fn paletted_accepts_otlp_http_and_grpc_and_makes_traces_queryable() -> anyhow::Result<()> {
    let _guard = live_smoke_guard().await;
    let tempdir = tempfile::tempdir()?;
    let addrs = free_addrs(2)?;
    let http_addr = addrs[0];
    let grpc_addr = addrs[1];
    let _server = PalettedChild::spawn(tempdir.path(), http_addr, grpc_addr)?;
    ...
    let http_export = otlp_smoke_export(http_trace, http_span, "paletted http smoke");
    reqwest::Client::new()
        .post(format!(
    ...
    let grpc_export = otlp_smoke_export(grpc_trace, grpc_span, "paletted grpc smoke");
    let mut client = TraceServiceClient::connect(grpc_url).await?;
    let mut request = TonicRequest::new(grpc_export);
    request
        .metadata_mut()
        .insert("x-palette-tenant-id", metadata_value("demo")?);
    request
        .metadata_mut()
        .insert("x-palette-project-id", metadata_value("demo")?);
    request
        .metadata_mut()
        .insert("x-palette-environment-id", metadata_value("local")?);
...
const TEMPORAL_HISTORY_FIXTURE: &str =
    include_str!("../../../crates/palette-temporal/tests/fixtures/order_workflow_history.json");

#[tokio::test]
async fn paletted_imports_temporal_history_into_queryable_trace() -> anyhow::Result<()> {
    let _guard = live_smoke_guard().await;
    let tempdir = tempfile::tempdir()?;
    let addrs = free_addrs(2)?;
    let http_addr = addrs[0];
...
#[tokio::test]
async fn paletted_quota_is_shared_across_two_replicas_and_resets_on_window() -> anyhow::Result<()> {
    let _guard = live_smoke_guard().await;
    let tempdir = tempfile::tempdir()?;
    ...
    let options = PalettedSpawnOptions {
        per_project_event_quota: Some(1),
        quota_window_seconds: Some(quota_window_seconds),
        quota_db_path: Some(quota_path),
        ..PalettedSpawnOptions::default()
    };
    let _replica_a = PalettedChild::spawn_with_options(
    ...
    let _replica_b = PalettedChild::spawn_with_options(
...
#[tokio::test]
async fn paletted_consumer_kill_restart_dlq_replay_recovers_trace_ingested_work()
-> anyhow::Result<()> {
    let _guard = live_smoke_guard().await;
    let tempdir = tempfile::tempdir()?;
    let data_dir = tempdir.path().join("paletted");
    ...
    let mut first = PalettedChild::spawn_with_options(
        &data_dir,
        first_http,
        first_grpc,
        PalettedSpawnOptions {
    ...
    let _second = PalettedChild::spawn_with_options(
        &data_dir,
        second_http,
        second_grpc,
        PalettedSpawnOptions {
...
#[tokio::test]
async fn paletted_trace_write_kill_replay_preserves_buffered_trace() -> anyhow::Result<()> {
    let _guard = live_smoke_guard().await;
    let tempdir = tempfile::tempdir()?;
    let data_dir = tempdir.path().join("paletted");
...
#[tokio::test]
async fn paletted_external_trace_store_kill_replays_buffered_trace() -> anyhow::Result<()> {
    let _guard = live_smoke_guard().await;
    let tempdir = tempfile::tempdir()?;
    let data_dir = tempdir.path().join("paletted");
    ...
    let http_addr = addrs[0];
    let grpc_addr = addrs[1];
    let _server = PalettedChild::spawn_with_options(
        &data_dir,
        http_addr,
        grpc_addr,
        PalettedSpawnOptions {
...
#[tokio::test]
async fn paletted_storage_failure_accounts_every_event_without_silent_drop() -> anyhow::Result<()> {
    let _guard = live_smoke_guard().await;
    let tempdir = tempfile::tempdir()?;
    let data_dir = tempdir.path().join("paletted");
    let fail_path = tempdir.path().join("trace-store-fail");
...
struct PalettedChild {
    child: Child,
}

#[derive(Clone, Default)]
struct PalettedSpawnOptions {
...
impl PalettedChild {
    ...
    fn spawn_with_options(
        ...
        http_addr: SocketAddr,
        grpc_addr: SocketAddr,
        options: PalettedSpawnOptions,
    ) -> anyhow::Result<Self> {
        let mut command = Command::new(env!("CARGO_BIN_EXE_paletted"));
        command
            .arg("--addr")
            ...
            .arg("--data-dir")
            .arg(data_dir)
            // #127: paletted now requires auth by default. This smoke harness
...
async fn wait_for_health(http_url: &str) -> anyhow::Result<()> {
    ...
    loop {
        ...
        if tokio::time::Instant::now() >= deadline {
            anyhow::bail!("paletted did not become healthy at {http_url}");
        }
    ...
    ExportTraceServiceRequest {
        resource_spans: vec![ResourceSpans {
            resource: Some(Resource {
                attributes: vec![otel_kv("service.name", otel_string("paletted-live-smoke"))],
                dropped_attributes_count: 0,
                entity_refs: Vec::new(),
            }),
            scope_spans: vec![ScopeSpans {
                scope: Some(InstrumentationScope {
                    name: "paletted-live-smoke".to_string(),
