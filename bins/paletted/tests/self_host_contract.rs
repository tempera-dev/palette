...
fn self_host_files_define_gate_two_compose_surface() {
    ...
    for service in ["postgres:", "nats:", "minio:", "paletted:", "dashboard:"] {
        assert!(
    ...
    for service in ["postgres", "nats", "minio"] {
        assert!(
            ...
            "docker-compose.yml {service} must be opt-in until paletted uses it at runtime"
        );
    }
    assert!(
        !compose_service_block(&compose, "paletted").contains("depends_on:"),
        "paletted must not depend on unused external services in the default local compose path"
    );
    assert!(compose.contains("dashboard-e2e:"));
    assert!(compose.contains("target: e2e"));
    assert!(compose.contains("profiles: [\"clickhouse\"]"));
    assert!(compose.contains("http://paletted:8080"));
    assert!(compose.contains("http://paletted:4317"));
    assert!(compose.contains("${PALETTE_HTTP_PORT:-8080}:8080"));
    assert!(compose.contains("${PALETTE_DASHBOARD_PORT:-3000}:3000"));
    ...
    assert!(dockerfile.contains("FROM rust-deps AS paletted-builder"));
    assert!(dockerfile.contains("FROM rust-deps AS palettectl-builder"));
    ...
    for service in ["postgres", "nats", "minio"] {
        assert!(
            ...
            "docker-compose.prebuilt.yml {service} must be opt-in until paletted uses it at runtime"
        );
    }
    assert!(
        !compose_service_block(&prebuilt_compose, "paletted").contains("depends_on:"),
        "paletted must not depend on unused external services in the default prebuilt compose path"
    );
    ...
    assert!(!prebuilt_compose.contains("PALETTE_POSTGRES_PORT"));
    assert!(!prebuilt_compose.contains("PALETTE_NATS_PORT"));
    assert!(!prebuilt_compose.contains("PALETTE_MINIO_PORT"));
    ...
    assert!(image_workflow.contains("Build palettectl tools target"));
    assert!(image_workflow.contains("target: tools"));
    assert!(image_workflow.contains("push: false"));
    assert!(image_workflow.contains("cache-from: type=gha,scope=paletted-${{ matrix.suffix }}"));
    assert!(
        image_workflow.contains("cache-to: type=gha,mode=max,scope=paletted-${{ matrix.suffix }}")
    );
    assert!(
        image_workflow
            .contains("cache-to: type=gha,mode=max,scope=palettectl-tools-${{ matrix.suffix }}")
    );
    assert!(image_workflow.contains("ghcr.io/${{ github.repository }}/paletted:main"));
    ...
    assert!(gate2_workflow.contains("cargo test -p paletted --test self_host_contract"));
    assert!(gate2_workflow.contains("cargo test -p paletted --test gate2_outside_validator"));
    ...
    assert!(!frontend_workflow.contains("PALETTE_GATE2_SKIP_BROWSER"));
    ...
    assert!(browser_proof_workflow.contains("PALETTE_GATE2_SKIP_PLAYWRIGHT_INSTALL: \"1\""));
    assert!(!browser_proof_workflow.contains("PALETTE_GATE2_SKIP_BROWSER"));
    ...
    for proof in [
        "paletted_accepts_otlp_http_and_grpc_and_makes_traces_queryable",
        "paletted_quota_is_shared_across_two_replicas_and_resets_on_window",
        "paletted_consumer_kill_restart_dlq_replay_recovers_trace_ingested_work",
        "paletted_trace_write_kill_replay_preserves_buffered_trace",
        "paletted_external_trace_store_kill_replays_buffered_trace",
        "paletted_storage_failure_accounts_every_event_without_silent_drop",
    ] {
        assert!(
    ...
    assert!(gate0_contract.contains("cargo\", \"tree\", \"-p\", \"palette-store"));
    assert!(gate0_contract.contains("palette-store must stay trait/types-only"));
    assert!(gate0_contract.contains("palette-store-conformance"));
    assert!(gate0_contract.contains("palette-store-memory"));
    assert!(gate0_contract.contains("palette-store-sql"));
    assert!(gate0_contract.contains("palette-store-obj"));
    ...
    assert!(
        gate0_contract
            .contains("cargo\", \"test\", \"-p\", \"palette-core\", \"-p\", \"palette-schema")
    );
    ...
    assert!(gate0_contract.contains("palette-schema must own"));
...
fn agent_span_kind_taxonomy_contracts_stay_in_sync() {
    let root = repo_root();
    let schema = read(root.join("crates/palette-schema/src/lib.rs"));
    ...
    assert_contains_in_order(
        &schema,
        "palette-schema AgentSpanKind::as_str",
    ...
    for kind in CANONICAL_AGENT_SPAN_KINDS {
        ...
        assert!(
            ...
            "palette-schema AgentSpanKind::parse must accept {kind} and {snake_case}"
        );
    }
...
fn migrations_cover_trace_store_and_queue_contracts() {
    let root = repo_root();
    let sqlite = read(root.join("migrations/sqlite/0001_local_paletted.sql"));
    let postgres = read(root.join("migrations/postgres/0001_initial.sql"));
    for table in [
        "raw_envelopes",
        "spans",
    ...
    assert!(clickhouse.contains("CREATE TABLE IF NOT EXISTS palette.raw_envelopes"));
    assert!(clickhouse.contains("CREATE TABLE IF NOT EXISTS palette.spans"));
    ...
    assert!(!clickhouse.contains("CREATE TABLE IF NOT EXISTS palette.trace_runs"));
...
fn clean_clone_smoke_uses_stock_otel_and_browser_visible_trace() {
    ...
    assert!(compose_script.contains("compose up -d --build paletted dashboard"));
    assert!(!compose_script.contains("compose up -d --build postgres nats minio"));
    assert!(compose_script.contains("compose run --rm palettectl"));
    ...
    assert!(proof_script.contains("palettectl\" smoke"));
    ...
    assert!(proof_script.contains("PALETTE_GATE2_SKIP_PLAYWRIGHT_INSTALL"));
    ...
    assert!(stopwatch_script.contains("PALETTE_GATE2_RUN_ID"));
    ...
    assert!(stopwatch_script.contains("proof-image paletted"));
    assert!(stopwatch_script.contains("proof-image dashboard-e2e"));
    assert!(stopwatch_script.contains("startup_args=(up -d --build paletted dashboard)"));
    assert!(
        stopwatch_script
            .contains("startup_args=(up -d --pull \"$prebuilt_pull_policy\" paletted dashboard)")
    );
    ...
    assert!(stopwatch_script.contains("PALETTE_GATE2_WRITE_PROOF"));
    assert!(stopwatch_script.contains("PALETTE_GATE2_BROWSER_PROOF"));
    assert!(stopwatch_script.contains("PALETTE_GATE2_RECORD_DEMO"));
    assert!(stopwatch_script.contains("PALETTE_GATE2_RECORD_MODE=compose"));
    assert!(stopwatch_script.contains("PALETTE_GATE2_OUTSIDE_WRAPPER=\"$outside_wrapper\""));
    assert!(stopwatch_script.contains("PALETTE_GATE2_COMPOSE_LOGS"));
    assert!(stopwatch_script.contains("PALETTE_GATE2_TERMINAL_LOG"));
    ...
    assert!(stopwatch_script.contains("PALETTE_E2E_QUICKSTART_TRACE_ID"));
    assert!(stopwatch_script.contains("PALETTE_E2E_QUICKSTART_RELEASE"));
    ...
    assert!(stopwatch_script.contains("PALETTE_GATE2_PUBLIC_DASHBOARD_BASE"));
    ...
    assert!(stopwatch_script.contains("PALETTE_GATE2_REUSE"));
    ...
    assert!(stopwatch_script.contains("PALETTE_GATE2_PULL_POLICY"));
    assert!(stopwatch_script.contains("--pull \"$prebuilt_pull_policy\""));
    assert!(stopwatch_script.contains("PALETTE_GATE2_POST_SLO_TIMEOUT_SECONDS"));
    ...
    assert!(stopwatch_script.contains("Palette image reference"));
    ...
    assert!(stopwatch_script.contains("Palette image digest"));
    ...
    assert!(outside_run.contains("PALETTE_GATE2_EXPECTED_ORIGIN"));
    assert!(outside_run.contains("PALETTE_GATE2_WRITE_PROOF=1"));
    assert!(outside_run.contains("PALETTE_GATE2_BROWSER_PROOF=1"));
    assert!(outside_run.contains("PALETTE_GATE2_RECORD_DEMO=1"));
    assert!(outside_run.contains("PALETTE_GATE2_REUSE"));
    assert!(outside_run.contains("PALETTE_GATE2_LOCAL_BUILD"));
    assert!(outside_run.contains("PALETTE_DASHBOARD_PORT"));
    assert!(outside_run.contains("PALETTE_GATE2_OUTSIDE_WRAPPER=1"));
    assert!(outside_run.contains("require_unset PALETTED_IMAGE"));
    assert!(outside_run.contains("require_unset PALETTE_DASHBOARD_IMAGE"));
    assert!(outside_run.contains("require_unset PALETTE_DASHBOARD_E2E_IMAGE"));
    assert!(outside_run.contains("require_unset PALETTE_OTEL_PYTHON_IMAGE"));
    assert!(outside_run.contains("require_unset PALETTE_GATE2_RUN_ID"));
    assert!(outside_run.contains("fresh per-run quickstart release ID"));
    assert!(outside_run.contains("PALETTE_GATE2_REGISTRY_FIXTURE_UNSAFE_FOR_TESTS"));
    assert!(outside_run.contains("outside evidence must validate against public GHCR"));
    assert!(outside_run.contains("the wrapper pins paletted to the checked-out commit SHA"));
    assert!(outside_run.contains("require_unset PALETTE_GATE2_STOPWATCH_PROOF"));
    assert!(outside_run.contains("require_unset PALETTE_GATE2_RECORD_VIDEO"));
    assert!(outside_run.contains("require_unset PALETTE_GATE2_RECORD_NOTES"));
    assert!(outside_run.contains("require_unset PALETTE_GATE2_COMPOSE_LOGS"));
    assert!(outside_run.contains("require_unset PALETTE_GATE2_TERMINAL_LOG"));
    ...
    assert!(outside_run.contains("require_unset_or_value KEEP_PALETTE_COMPOSE 1"));
    ...
    assert!(outside_run.contains("default palette-stopwatch Compose project"));
    assert!(outside_run.contains("export KEEP_PALETTE_COMPOSE=1"));
    assert!(
        outside_run
            .contains("export PALETTE_GATE2_COMPOSE_LOGS=docs/demos/gate2-outside-compose.log")
    );
    assert!(
        outside_run
            .contains("export PALETTE_GATE2_TERMINAL_LOG=docs/demos/gate2-outside-terminal.log")
    );
    assert!(outside_run.contains("tee \"$PALETTE_GATE2_TERMINAL_LOG\""));
    ...
    assert!(outside_local_preflight.contains("require_unset_or_value PALETTE_GATE2_REUSE 0"));
    assert!(outside_local_preflight.contains("require_unset_or_value PALETTE_DASHBOARD_PORT 3000"));
    assert!(outside_local_preflight.contains("require_unset PALETTED_IMAGE"));
    assert!(outside_local_preflight.contains("require_unset PALETTE_GATE2_COMPOSE_LOGS"));
    assert!(outside_local_preflight.contains("require_unset PALETTE_GATE2_TERMINAL_LOG"));
    assert!(outside_local_preflight.contains("PALETTE_GATE2_EXPECTED_COMMIT"));
    ...
    assert!(outside_local_preflight.contains("current directory already contains ./palette"));
    assert!(outside_local_preflight.contains("If this is a stale Palette Gate 2 run"));
    assert!(outside_local_preflight.contains("docker-compose.prebuilt.yml -p palette-stopwatch"));
    assert!(outside_local_preflight.contains("label=com.docker.compose.project=palette-stopwatch"));
    ...
    for image in ["paletted", "dashboard", "dashboard-e2e", "otel-python"] {
        assert!(
            gate2_proof_contract.contains(&format!("image_name=\"{image}\"")),
            "Gate 2 proof contract must define image {image}"
        );
    }
    assert!(outside_validator.contains("PALETTE_GATE2_REUSE=1"));
    assert!(outside_validator.contains("PALETTE_DASHBOARD_PORT="));
    ...
    assert!(outside_validator.contains("(\"Compose project\", \"palette-stopwatch\")"));
    assert!(
        outside_validator
    ...
    assert!(gate2_proof_contract.contains("PALETTE_GATE2_EXPECTED_COMMIT=\"$sha\""));
    assert!(gate2_proof_contract.contains("cd ./palette"));
    assert!(!gate2_proof_contract.contains("cd palette"));
    assert!(
        gate2_proof_contract.contains("PALETTE_GATE2_CLONE_STARTED_EPOCH=\"$t\" {PUBLIC_GIT_ENV}")
    );
    ...
    assert!(gate2_proof_contract.contains("(8080, \"paletted HTTP\", \"PALETTE_HTTP_PORT\")"));
    assert!(gate2_proof_contract.contains("(4317, \"OTLP gRPC\", \"PALETTE_OTLP_GRPC_PORT\")"));
    assert!(gate2_proof_contract.contains("(3000, \"dashboard\", \"PALETTE_DASHBOARD_PORT\")"));
    ...
    assert!(public_handoff.contains("env[\"PALETTE_GATE2_CLONE_STARTED_EPOCH\"]"));
    assert!(public_handoff.contains("run_with_manual_checkpoint_confirmation("));
    ...
    assert!(outside_proof.contains("Run `cd ./palette`"));
    assert!(outside_proof.contains("stay in the `palette/`\nclone"));
    assert!(
        !outside_proof.contains("cd ./palette\ngit add docs/demos/gate2-outside-person-proof.md")
    );
    ...
    assert!(outside_proof.contains("Palette image reference"));
    ...
    assert!(outside_proof.contains("Palette image digest"));
    ...
    assert!(outside_proof.contains("Prior Palette repo exposure"));
    ...
    assert!(readme.contains("default compose proof starts `paletted` and the dashboard"));
    ...
    assert!(readme.contains("PALETTE_GATE2_EXPECTED_COMMIT=\"$sha\""));
    assert!(readme.contains("GIT_CONFIG_COUNT=0 git rev-parse HEAD"));
    assert!(readme.contains("cd ./palette"));
    assert!(readme.contains("run `cd ./palette`"));
    assert!(readme.contains("from the same `palette/` clone"));
    assert!(!readme.contains("cd ./palette\ngit add docs/demos/gate2-outside-person-proof.md"));
    ...
    assert!(readme.contains("PALETTE_GATE2_CLONE_STARTED_EPOCH"));
    ...
    assert!(readme.contains("removes any previous Palette stopwatch project"));
    ...
    assert!(readme.contains("pins `paletted`, `dashboard`, `dashboard-e2e`, and `otel-python`"));
    assert!(readme.contains("current-SHA"));
    assert!(readme.contains("`paletted`, `dashboard`, `dashboard-e2e`, and `otel-python` GHCR"));
    ...
    assert!(readme.contains("PALETTE_GATE2_RUN_ID"));
    assert!(readme.contains("fresh per-run quickstart release ID"));
    assert!(readme.contains("Palette image service rows"));
    ...
    assert!(
        requirements
            .contains("structured Palette image service rows plus `proof-image` digest rows")
    );
    ...
    assert!(quickstart.contains("palette.release_id"));
    assert!(quickstart.contains("PALETTE_GATE2_RUN_ID"));
    assert!(!quickstart.contains("palettectl"));
    ...
    assert!(!python.contains("palettectl"));

    let record_script = read(root.join("web/dashboard/tests/e2e/record-gate2-demo.mjs"));
    assert!(record_script.contains("recordVideo"));
    assert!(record_script.contains("PALETTE_GATE2_RECORD_MODE"));
    assert!(record_script.contains("PALETTE_GATE2_OUTSIDE_WRAPPER"));
    assert!(record_script.contains("PALETTE_GATE2_PUBLIC_DASHBOARD_BASE"));
    assert!(record_script.contains("PALETTE_E2E_QUICKSTART_RELEASE"));
    ...
    let compose_recording_fixture =
        root.join("bins/paletted/tests/fixtures/gate2-compose-browser-demo.webm");
    ...
    assert!(quickstart_e2e.contains("PALETTE_E2E_QUICKSTART_RELEASE"));
    ...
    assert!(quickstart_e2e.contains("hello from Palette"));
    ...
    assert!(readme.contains("stop or move that app instead of setting alternate Palette ports"));
    assert_eq!(
        readme
            .matches("stop or move that app instead of setting alternate Palette ports")
            .count(),
        2,
        "both README clean-clone paths must tell outside runners to free non-Palette default-port owners"
    );
    ...
    assert!(runner_card.contains("PALETTE_GATE2_CLONE_STARTED_EPOCH"));
    assert!(runner_card.contains("`ffprobe` (installed by common `ffmpeg` packages)"));
    assert!(runner_card.contains("clean stale Palette containers"));
    assert!(runner_card.contains("stop/move\nthe reported non-Palette app"));
    assert!(runner_card.contains("Do not set alternate Palette\nports"));
...
fn repo_root() -> PathBuf {
    ...
    let Some(root) = manifest.parent().and_then(|path| path.parent()) else {
        panic!("paletted manifest must live under bins/paletted");
    };
    root.to_path_buf()
}
