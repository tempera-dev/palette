...
const OUTSIDE_RUN_ATTESTATION: &str = "I attest that I am not a Palette project maintainer, I received no step-by-step help beyond public repository instructions, I used a fresh clone, and I completed the Gate 2 flow unaided.";
...
fn gate2_outside_validator_resolves_default_template_from_script_path() {
    ...
    let output = Command::new("bash")
        ...
        .env_remove("PALETTE_GATE2_OUTSIDE_PROOF")
        .output()
...
fn gate2_outside_docs_use_fail_fast_clone_command() {
    ...
    assert!(canonical_outside_command().contains("cd ./palette"));
    assert!(!canonical_outside_command().contains("cd palette"));
    assert!(
        canonical_outside_command()
            .contains("PALETTE_GATE2_CLONE_STARTED_EPOCH=\"$t\" GIT_CONFIG_GLOBAL=/dev/null")
    );
    ...
    assert!(readme.contains(r#"--relationship "external evaluator; no Palette project role""#));
    assert!(!readme.contains(r#"--relationship "external evaluator; no Palette maintainer role""#));
    ...
    assert!(proof_template.contains("Run `cd ./palette`"));
    assert!(
        proof_template
            .contains("cd ./palette\nscripts/generate-gate2-outside-proof.py --print-command")
    );
    assert!(proof_template.contains("stay in the `palette/`\nclone"));
    assert!(
        !proof_template.contains("cd ./palette\ngit add docs/demos/gate2-outside-person-proof.md")
    );
    ...
    assert!(
        proof_template.contains(r#"--relationship "external evaluator; no Palette project role""#)
    );
    assert!(
        !proof_template
            .contains(r#"--relationship "external evaluator; no Palette maintainer role""#)
    );
...
fn gate2_outside_generator_accepts_prior_exposure_none_and_defaults_run_date() {
    ...
    let generated_text = fs::read_to_string(&generated)
        .unwrap_or_else(|err| panic!("read {}: {err}", generated.display()));
    assert!(generated_text.contains("- Prior Palette repo exposure: none"));
...
fn gate2_outside_readiness_rejects_github_https_origin_for_wrong_repo() {
    ...
    git_success(
        ...
        &[
            ...
            "add",
            "origin",
            "https://github.com/jadenfix/not-palette",
        ],
    );
...
fn gate2_outside_readiness_rejects_default_service_dependency_on_profiled_deps() {
    ...
    assert_failure(
        output,
        "docker-compose.prebuilt.yml default/timed service paletted must not depend on profiled third-party service(s): postgres",
    );
}
...
fn gate2_public_handoff_verifier_clears_post_slo_timeout_override() {
    ...
    let output = run_public_handoff_with_fixture_env(
        ...
        &fixture_head,
        registry.path(),
        ("PALETTE_GATE2_POST_SLO_TIMEOUT_SECONDS", "1"),
    );
...
fn gate2_public_handoff_verifier_full_run_rejects_noncanonical_fixture_source() {
    ...
    assert!(
        !clone_parent.path().join("palette").exists(),
...
fn gate2_public_handoff_verifier_full_run_accepts_rewritten_canonical_fixture() {
    ...
    command
        ...
        .env("PALETTE_GATE2_FIXTURE_FULL_RUN", "1")
    ...
    let checks_clone_dir = clone_parent.path().join("palette-checks");
    assert!(
    ...
    assert!(
        bash_log.contains("mktemp \"${TMPDIR:-/tmp}/palette-gate2-preflight.XXXXXX\""),
        "full-run fixture should create a temp preflight script through bash -lc\n{bash_log}"
    );
    assert!(bash_log.contains("-o \"$preflight\" && PALETTE_GATE2_EXPECTED_COMMIT=\""));
...
"#,
    ...
    assert!(!curl_log.contains("/main/scripts/gate2-outside-local-preflight.sh"));
    assert!(
        !clone_parent.path().join("palette-checks").exists(),
...
fn gate2_public_handoff_full_run_has_local_runtime_preflight_contract() {
    ...
    assert!(proof_contract.contains("PALETTE_GATE2_EXPECTED_COMMIT=\"$sha\""));
    ...
    assert!(proof_contract.contains("(8080, \"paletted HTTP\", \"PALETTE_HTTP_PORT\")"));
    assert!(proof_contract.contains("(4317, \"OTLP gRPC\", \"PALETTE_OTLP_GRPC_PORT\")"));
    assert!(proof_contract.contains("(3000, \"dashboard\", \"PALETTE_DASHBOARD_PORT\")"));
    ...
    assert!(script.contains("cleaning the palette-stopwatch Compose project"));
    assert!(script.contains("preflight_full_run_runtime(args)"));
    assert!(script.contains("\"palette-checks\" if args.full_run else \"palette\""));
    assert!(script.contains("full_clone_dir, full_temp_owner, full_clone_started_epoch"));
    assert!(script.contains("expected_commit, \"palette\""));
...
fn gate2_outside_local_preflight_rejects_existing_clone_destination_before_docker() {
    ...
    fs::create_dir(parent.path().join("palette"))
        .unwrap_or_else(|err| panic!("create existing palette clone dir: {err}"));

    let output = run_outside_local_preflight_with_runtime(&runtime, None, Some(parent.path()));

    assert_failure(output, "current directory already contains ./palette");
    let docker_log = fs::read_to_string(&runtime.docker_log).unwrap_or_default();
    assert!(
        docker_log.is_empty(),
...
fn gate2_outside_local_preflight_rejects_alternate_port_before_docker() {
    ...
    let output = run_outside_local_preflight_with_runtime(
        &runtime,
        Some(("PALETTE_DASHBOARD_PORT", "13000")),
        None,
    );

    assert_failure(output, "PALETTE_DASHBOARD_PORT must be unset or '3000'");
    let docker_log = fs::read_to_string(&runtime.docker_log).unwrap_or_default();
    assert!(
        docker_log.is_empty(),
...
fn gate2_outside_local_preflight_rejects_image_override_before_docker() {
    ...
    let output = run_outside_local_preflight_with_runtime(
        &runtime,
        Some(("PALETTED_IMAGE", "ghcr.io/example/paletted:test")),
        None,
    );

    assert_failure(
        output,
        "PALETTED_IMAGE must be unset for outside-person evidence",
    );
    let docker_log = fs::read_to_string(&runtime.docker_log).unwrap_or_default();
    assert!(
        docker_log.is_empty(),
...
print(module.occupied_port_message(3000, "dashboard", "PALETTE_DASHBOARD_PORT"))
"#,
        )
        .arg(root.join("scripts/check-gate2-public-handoff.py"))
        .current_dir(&root)
    ...
    assert!(
        stdout.contains("do not set PALETTE_DASHBOARD_PORT"),
...
module.os.environ["PALETTE_GATE2_FIXTURE_FULL_RUN"] = "1"
...
fn gate2_stopwatch_service_image_digest_rejects_wrong_service_repo_digest() {
    let output = run_service_image_digest_fixture("dashboard", PALETTE_IMAGE_DIGEST);
...
fn gate2_stopwatch_outside_next_steps_separate_dashboard_targets() {
    ...
    assert!(script.contains("PALETTE_GATE2_COMPOSE_LOGS"));
    assert!(script.contains("PALETTE_GATE2_TERMINAL_LOG"));
    ...
    assert!(script.contains("After the one-liner exits, run 'cd ./palette'"));
...
fn gate2_outside_wrapper_rejects_alternate_dashboard_port() {
    let output = run_outside_wrapper_dry_run(Some(("PALETTE_DASHBOARD_PORT", "13080")));
...
fn gate2_outside_wrapper_rejects_post_slo_timeout_override() {
    let output =
        run_outside_wrapper_dry_run(Some(("PALETTE_GATE2_POST_SLO_TIMEOUT_SECONDS", "120")));

    assert_failure(
        output,
        "PALETTE_GATE2_POST_SLO_TIMEOUT_SECONDS must be unset or '300'",
    );
}
...
fn gate2_outside_wrapper_rejects_registry_fixture_override() {
    ...
    assert_failure(
        output,
        "PALETTE_GATE2_REGISTRY_FIXTURE_UNSAFE_FOR_TESTS must be unset for outside-person evidence",
    );
}
...
fn gate2_outside_wrapper_rejects_artifact_path_override() {
    ...
    assert_failure(
        output,
        "PALETTE_GATE2_RECORD_VIDEO must be unset for outside-person evidence",
    );
}
...
fn gate2_outside_wrapper_rejects_compose_logs_override() {
    ...
    assert_failure(
        output,
        "PALETTE_GATE2_COMPOSE_LOGS must be unset for outside-person evidence",
    );
}

#[test]
fn gate2_outside_wrapper_rejects_compose_project_override() {
    let output = run_outside_wrapper_dry_run(Some(("COMPOSE_PROJECT_NAME", "palette-alt")));
...
fn gate2_outside_wrapper_rejects_terminal_log_override() {
    ...
    assert_failure(
        output,
        "PALETTE_GATE2_TERMINAL_LOG must be unset for outside-person evidence",
    );
}
...
fn gate2_outside_wrapper_rejects_compose_cleanup_override() {
    let output = run_outside_wrapper_dry_run(Some(("KEEP_PALETTE_COMPOSE", "0")));

    assert_failure(output, "KEEP_PALETTE_COMPOSE must be unset or '1'");
}
...
fn gate2_outside_wrapper_rejects_missing_clone_timer_for_real_run() {
    ...
    assert_failure(
        output,
        "PALETTE_GATE2_CLONE_STARTED_EPOCH must be set before git clone",
    );
}
...
fn gate2_outside_wrapper_rejects_clone_timer_after_first_local_reflog() {
    ...
    assert_failure(
        output,
        "PALETTE_GATE2_CLONE_STARTED_EPOCH must be captured before git clone",
    );
    assert!(
        !fixture
            .path()
...
fn gate2_outside_wrapper_rejects_missing_ffprobe_before_stopwatch() {
    ...
    command
        .env("PATH", path_dir.path())
        .env("PALETTE_GATE2_CLONE_STARTED_EPOCH", clone_started);
    let output = command
        .output()
...
fn gate2_outside_validator_rejects_placeholder_compose_images_excerpt() {
    ...
    replace(
        ...
        "- `docker compose images` excerpt: paletted and dashboard images present\n",
    );
...
fn gate2_outside_validator_rejects_noncanonical_compose_project() {
    ...
    assert_failure(
        output,
        "Compose project in stopwatch proof must be 'palette-stopwatch'",
    );
}
...
fn gate2_outside_validator_rejects_missing_prior_exposure() {
    ...
    replace(
        &fixture.proof_path,
        "- Prior Palette repo exposure: no prior exposure",
    ...
    assert_failure(
        output,
        "unresolved required fields: Prior Palette repo exposure",
    );
}
...
fn gate2_outside_validator_accepts_prior_exposure_none() {
    ...
    replace(
        &fixture.proof_path,
        "- Prior Palette repo exposure: no prior exposure",
        "- Prior Palette repo exposure: none",
    );
...
fn gate2_outside_validator_rejects_registry_fixture_env_without_test_marker() {
    ...
    assert_failure(
        output,
        "PALETTE_GATE2_REGISTRY_FIXTURE_UNSAFE_FOR_TESTS is only allowed for diagnostic or temporary generator validation",
    );
}
...
fn gate2_outside_validator_rejects_bare_image_digest() {
    ...
    assert_failure(
        output,
        "Palette image digest in outside-person proof must be a GHCR repo digest for paletted",
    );
}
...
fn gate2_outside_validator_rejects_compose_env_in_recording_notes() {
    ...
    append(
        &fixture.notes_path,
        "\nRecording note: COMPOSE_PROJECT_NAME=palette-alt\n",
    );
...
fn gate2_outside_validator_rejects_symlink_recording_artifact() {
    ...
    symlink(
        repo_root().join("bins/paletted/tests/fixtures/gate2-compose-browser-demo.webm"),
        &fixture.recording_path,
    )
...
fn gate2_outside_validator_rejects_image_digest_mismatch() {
    ...
    assert_failure(
        output,
        "palette image digest mismatch between proof artifacts",
    );
}
...
fn gate2_outside_validator_rejects_registry_unbound_image_digest_at_closure() {
    ...
    assert_failure(
        output,
        "Palette image digest in outside-person proof must match public GHCR manifest digest",
    );
}
...
Compose project: palette-stopwatch\n\
Startup mode: prebuilt-image\n\
Command: docker compose -f docker-compose.prebuilt.yml -p palette-stopwatch logs --no-color --timestamps\n\
\n\
2026-06-20T12:00:01Z palette-stopwatch-paletted-1 started\n\
2026-06-20T12:00:02Z palette-stopwatch-dashboard-1 started\n"
}
...
fn run_validator_without_untracked_artifact_escape(proof_path: &Path) -> Output {
    ...
    Command::new("bash")
        ...
        .current_dir(root)
        .env("PALETTE_GATE2_OUTSIDE_PROOF", proof_path)
        .env_remove("PALETTE_GATE2_ALLOW_UNTRACKED_ARTIFACTS")
        .output()
        .unwrap_or_else(|err| panic!("run Gate 2 outside proof validator: {err}"))
}
...
case "$*" in
  ...
  "context inspect --format {{{{.Endpoints.docker.Host}}}}")
    ...
    exit 0
    ;;
  "compose -f docker-compose.prebuilt.yml -p palette-stopwatch down -v --remove-orphans")
    exit 0
    ;;
  *)
...
fn registry_digest_for_image(image: &str) -> &'static str {
    match image {
        "paletted" => "sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
...
fn recording_bytes() -> Vec<u8> {
    fs::read(repo_root().join("bins/paletted/tests/fixtures/gate2-compose-browser-demo.webm"))
...
fn write_public_handoff_fixture_repo() -> TempDir {
    ...
    copy_fixture_file_as(
        &root,
        fixture.path(),
        "bins/paletted/tests/fixtures/gate2-compose-browser-demo.webm",
...
After the one-liner exits, run 'cd ./palette' from the parent shell if your prompt is not already inside the clone.
Generate the completed proof from this prefilled command:
python3 scripts/generate-gate2-outside-proof.py --stopwatch-proof docs/demos/gate2-compose-stopwatch.md --print-command
...
fi
{
  ...
  echo "write=${PALETTE_GATE2_WRITE_PROOF:-unset}"
  echo "browser=${PALETTE_GATE2_BROWSER_PROOF:-unset}"
  echo "record=${PALETTE_GATE2_RECORD_DEMO:-unset}"
  echo "post_slo=${PALETTE_GATE2_POST_SLO_TIMEOUT_SECONDS:-unset}"
  echo "reuse=${PALETTE_GATE2_REUSE:-unset}"
  echo "local_build=${PALETTE_GATE2_LOCAL_BUILD:-unset}"
  echo "pull_policy=${PALETTE_GATE2_PULL_POLICY:-unset}"
  echo "keep=${KEEP_PALETTE_COMPOSE:-unset}"
	  echo "outside_wrapper=${PALETTE_GATE2_OUTSIDE_WRAPPER:-unset}"
	  echo "compose_logs=${PALETTE_GATE2_COMPOSE_LOGS:-unset}"
	  echo "terminal_log=${PALETTE_GATE2_TERMINAL_LOG:-unset}"
	  echo "dry=${PALETTE_GATE2_OUTSIDE_RUN_DRY_RUN:-unset}"
  echo "expected_origin=${PALETTE_GATE2_EXPECTED_ORIGIN:-unset}"
  echo "clone_started=${PALETTE_GATE2_CLONE_STARTED_EPOCH:-unset}"
  echo "dashboard_port=${PALETTE_DASHBOARD_PORT:-unset}"
  echo "fixture_full_run=${PALETTE_GATE2_FIXTURE_FULL_RUN:-unset}"
