...
use palette_api::openapi::urlencode;
use palette_audit::{
...
use palette_bus::{BusMessage, DurableBus, SqliteDurableBus};
...
use palette_gates::{GateDefinition, GateStore, InconclusivePolicy, SqliteGateStore, run_gate};
use palette_human::{
...
use palette_otlp::{encode_export_trace_request, export_to_raw_trace_ingest_request};
use palette_replay::{
...
#[derive(Debug, Parser)]
#[command(name = "palettectl", about = "Palette local development and CI helper")]
struct Args {
    /// Base URL of the Palette API for remote commands.
    #[arg(
        long,
        global = true,
        env = "PALETTE_BASE_URL",
        default_value = "http://127.0.0.1:8080"
    )]
    base_url: String,
    /// API key sent as `Authorization: Bearer <key>` on remote requests.
    #[arg(long, global = true, env = "PALETTE_API_KEY")]
...
enum Command {
    ...
    /// Call any Palette API endpoint by its OpenAPI operationId.
    ///
    ...
    Api {
        /// OpenAPI operationId, e.g. `traces.list`.
        operation_id: String,
    ...
    /// `palette_experiments::run_optimization_round`: it builds an
    ...
    /// palette-experiments. A live run against real providers + a real candidate
    ...
    /// canonical Palette trace (via the same `IngestService::ingest_native` path
    ...
    /// palette-judge text-generation seam) AND scores every candidate by being
        ...
        Command::AlertFixture { data_dir } => {
            ...
            let alert = AlertEngine::new().evaluate(
                &AlertPolicy {
                    ...
                    endpoint_url: "https://example.test/palette-webhook".to_string(),
...
async fn run_local_quickstart(
    ...
    zero_code_env
        ...
        .insert("PALETTE_API_KEY".to_string(), json!(api_key.secret.clone()));
...
fn zero_code_otlp_env(
    ...
    if otlp_grpc_url.is_some() {
        headers.push(format!("x-palette-tenant-id={tenant_id}"));
        headers.push(format!("x-palette-project-id={project_id}"));
        headers.push(format!("x-palette-environment-id={environment_id}"));
    }
    if auth_configured {
        headers.push("x-palette-api-key=${PALETTE_API_KEY}".to_string());
    }

    let mut env = serde_json::Map::new();
    env.insert("PALETTE_TENANT_ID".to_string(), json!(tenant_id));
    env.insert("PALETTE_PROJECT_ID".to_string(), json!(project_id));
    env.insert("PALETTE_ENVIRONMENT_ID".to_string(), json!(environment_id));
    env.insert(
...
async fn emit_remote_grpc(
    ...
    request
        .metadata_mut()
        .insert("x-palette-tenant-id", metadata_value(tenant_id)?);
    request
        .metadata_mut()
        .insert("x-palette-project-id", metadata_value(project_id)?);
    request
        .metadata_mut()
        .insert("x-palette-environment-id", metadata_value(environment_id)?);
...
async fn wait_for_remote_trace(
    ...
    loop {
        let trace = with_bearer(client.get(&url), api_key)
            .header("x-palette-project-id", project_id)
            .header("x-palette-environment-id", environment_id)
            .send()
            .await
            .context("read smoke trace")?
            .error_for_status()
...
fn resolve_operation(
    ...
    palette_api::openapi::operations(spec)
        .into_iter()
    ...
    ExportTraceServiceRequest {
        resource_spans: vec![ResourceSpans {
            resource: Some(Resource {
                attributes: vec![otel_kv("service.name", otel_string("palettectl-smoke"))],
                dropped_attributes_count: 0,
                entity_refs: Vec::new(),
            }),
            scope_spans: vec![ScopeSpans {
                scope: Some(InstrumentationScope {
                    name: "palettectl".to_string(),
                ...
                spans: vec![Span {
                    ...
                    flags: 0,
                    name: "palettectl otlp smoke".to_string(),
...
/// path (good only on the optimization split) is unit-tested in palette-experiments.
...
/// optional `PALETTE_ANTHROPIC_BASE_URL` override.
///
...
fn anthropic_provider_config() -> HttpJudgeProviderConfig {
    ...
    if let Some(base_url) = std::env::var("PALETTE_ANTHROPIC_BASE_URL")
        .ok()
...
async fn run_rsi_round_live(
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "rsi-round needs a real Anthropic key: set ANTHROPIC_API_KEY (BYOK). For a \
                 no-network demo use `palettectl rsi-round-fixture`."
            )
        })?;
    ...
    // can point it at a mock server with `PALETTE_ANTHROPIC_BASE_URL` to exercise
...
/// Emit a completed RSI optimization `round_report` as ONE canonical Palette
/// trace into the `--data-dir` store, then read it back to prove it landed —
/// dogfooding the exact ingest path `smoke` uses.
///
/// Construction mirrors [`palette_ingest::smoke_trace`]: an [`IngestService`] is
...
fn calibration_fixture_case(
    ...
    Ok(DatasetCase {
        ...
        case_id: palette_core::DatasetCaseId::new(case_id)?,
        ...
        normalizer_version: "palette-calibration-fixture-v1".to_string(),
...
impl TraceStore for UnavailableTraceStore {
    ...
    ) -> palette_store::StoreResult<WriteAck> {
    ...
    async fn get_trace(
        ...
        _tenant: TenantId,
        _trace: TraceId,
    ) -> palette_store::StoreResult<TraceView> {
        Err(StoreError::Backend("trace store unavailable".to_string()))
    }
    ...
    ) -> palette_store::StoreResult<Option<RawEnvelope>> {
        Err(StoreError::Backend("trace store unavailable".to_string()))
    }
    ...
    async fn query_runs(
        ...
        _filter: RunFilter,
        _page: PageRequest,
    ) -> palette_store::StoreResult<Page<RunSummary>> {
        Err(StoreError::Backend("trace store unavailable".to_string()))
    }
    ...
    async fn query_spans(
        ...
        _filter: SpanFilter,
        _page: PageRequest,
    ) -> palette_store::StoreResult<Page<SpanSummary>> {
        Err(StoreError::Backend("trace store unavailable".to_string()))
    }
}
