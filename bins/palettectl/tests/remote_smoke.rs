...
async fn remote_smoke_http_reports_query_lag_under_oss_slo() -> anyhow::Result<()> {
    ...
    let _server = PalettedChild::spawn(tempdir.path().to_path_buf(), http_addr, grpc_addr)?;
    let http_url = format!("http://{http_addr}");
    wait_for_health(&http_url).await?;

    let smoke = run_palettectl_smoke(&http_url, None, None)?;
...
async fn ingest_test_reports_trace_and_zero_code_env() -> anyhow::Result<()> {
    ...
    let output = run_palettectl_ingest_test(&http_url)?;
    ...
    assert_eq!(env["PALETTE_TENANT_ID"], "demo");
    assert_eq!(env["PALETTE_PROJECT_ID"], "demo");
    assert_eq!(env["PALETTE_ENVIRONMENT_ID"], "local");
...
async fn remote_smoke_grpc_reports_query_lag_under_oss_slo() -> anyhow::Result<()> {
    ...
    let smoke = run_palettectl_smoke(&http_url, Some(&grpc_url), None)?;
    assert_eq!(smoke["mode"], "remote");
...
async fn remote_smoke_http_strict_auth_reads_back_trace() -> anyhow::Result<()> {
    ...
    let _server = PalettedChild::spawn_with_auth_mode(
    ...
    let smoke = run_palettectl_smoke(&http_url, None, Some(&api_key))?;
    assert_eq!(smoke["mode"], "remote");
    assert_eq!(smoke["protocol"], "http");
    assert_eq!(smoke["source"], "otlp");
    assert_eq!(smoke["trace_span_count"], 1);
...
fn run_palettectl_ingest_test(http_url: &str) -> anyhow::Result<serde_json::Value> {
...
impl PalettedChild {
    fn spawn(
        ...
        // paletted defaults to --auth-mode required (b728b9e / #127); most of this
    ...
    fn spawn_with_auth_mode(
        ...
        let child = Command::new(paletted_bin()?)
            .arg("--addr")
...
fn run_palettectl_smoke(
    http_url: &str,
    otlp_grpc_url: Option<&str>,
    api_key: Option<&str>,
) -> anyhow::Result<serde_json::Value> {
    let mut command = Command::new(env!("CARGO_BIN_EXE_palettectl"));
...
fn create_trace_smoke_key(data_dir: &Path) -> anyhow::Result<String> {
    ...
    assert!(
        output.status.success(),
        "palettectl api-key-create failed\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
...
fn paletted_bin() -> anyhow::Result<PathBuf> {
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_paletted") {
    ...
    let candidate = debug_dir.join(format!("paletted{}", std::env::consts::EXE_SUFFIX));
    if candidate.exists() {
        return Ok(candidate);
    }
    let status = Command::new("cargo")
        ...
        .status()?;
    if !status.success() {
        anyhow::bail!("cargo build -p paletted failed with {status}");
    }
    Ok(candidate)
}
