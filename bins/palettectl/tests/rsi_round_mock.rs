//! Binary-level E2E for the LIVE `palettectl rsi-round` command, driven against an
...
//! pointed at a mock via `PALETTE_ANTHROPIC_BASE_URL`, the actual shipped binary
...
const REWRITE: &str =
    "You are PALETTE-CANDIDATE, a meticulous factual assistant. Answer correctly and cite sources.";
...
async fn run_rsi_round(endpoint: &str) -> Value {
    ...
    let output = tokio::task::spawn_blocking(move || {
        std::process::Command::new(env!("CARGO_BIN_EXE_palettectl"))
            .arg("rsi-round")
            .env("ANTHROPIC_API_KEY", "sk-mock-fake-key")
            .env("PALETTE_ANTHROPIC_BASE_URL", endpoint)
            .output()
            .expect("spawn palettectl rsi-round")
    })
    .await
